...
A line is expressed in the form Ax + By + C = 0. Lines can
be "normalized" to a canonical form by insisting that the
y-coefficient equal 1 if it is non-zero. Otherwise, we set
the x-coefficient to 1. If B is non-zero, then we have the
common case where the slope = -A after normalization.
