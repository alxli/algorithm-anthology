...
- `tangent(c, p, &l1, &l2)` determines the line(s) tangent to circle `c` that pass through point
  `p`, returning $-1$ if there is no tangent line because `p` is strictly inside `c`,
...
- `intersection(c, l, &p, &q)` determines the intersection between the circle `c` and line
  `l`, returning $-1$ if there is no intersection, $0$ if the line has one intersection point
  because the line is tangent (in which case it will be stored into pointer `p` if it's not
...
- `intersection(c1, c2, &p, &q)` determines the intersection points between two circles
  `c1` and `c2`, returning $-2$ if circle `c2` completely encloses circle `c1`, $-1$ if
