...
- `tangent(c, p, &l1, &l2)` determines the line(s) tangent to circle $c$ that pass
...
- `intersection(c, l, &p, &q)` determines the intersection between the circle $c$
  and line $l$, returning $-1$ if there is no intersection, 0 if the line has one
...
- `intersection(c1, c2, &p, &q)` determines the intersection points between two
  circles $c_1$ and $c_2$, returning $-2$ if circle $c_2$ completely encloses circle $c_1$,
  $-1$ if circle $c_1$ completely encloses circle $c_2$, 0 if the circles are completely
