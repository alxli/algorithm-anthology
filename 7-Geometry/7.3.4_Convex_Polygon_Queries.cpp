...
- `point_in_convex_polygon(poly, p, include_boundary = true)` returns whether point `p` lies inside
  the convex polygon `poly`. The polygon must be in counter-clockwise order, with no repeated final
  vertex. Points on the boundary count as inside unless `include_boundary` is set to `false`.
