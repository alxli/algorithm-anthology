...
`PointI` from 7.1.1, or any struct with numeric `.x` and `.y` fields. All operations use comparisons
only and are exact for integer-coordinate points.
...
bool point_in_rectangle(const Pt &p, const Pt &v, T w, T h) {
