...
Provides a modular integer value type together with lazy factorial-based combinatorics tables. Both
are common contest helpers for dynamic programming, counting, polynomial operations, and any
calculation whose answers are taken modulo some number such as $10^9 + 7$. Where the helpers of
section 6.3.1 suit a few scattered operations, a value type is what makes an entire solution read in
ordinary arithmetic notation while every operation reduces silently.

The modulus is a template parameter rather than a member, which is a deliberate speed choice: a
compile-time constant lets the compiler replace each division by a multiplication and a shift, while
a modulus only known at run time forces a hardware divide costing several times as much. Storing it
in a `static` member instead makes the same class work with a runtime modulus at that price, and is
worth doing when the modulus is read from the input. A modulus above roughly $3 \times 10^9$ needs
the `mulmod()` of section 6.3.1 in place of the plain products used here.
...
`Mint`. It assumes the modulus is prime and all requested factorials are invertible. Growing the
tables costs one modular inverse rather than one per entry: after extending the factorials by
repeated multiplication, a single inversion of the largest factorial seeds a backward pass in which
each inverse factorial follows from the next by one multiplication.
...
Tables pay off once a solution asks for many values against one fixed modulus, which is the usual
counting or dynamic programming case. For a handful of values, or for a modulus that changes between
calls, the table-free routines of section 6.2.1 compute the same quantities in O(min(k, n - k)) per
