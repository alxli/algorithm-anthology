...
the pattern required of a prime proves that $n$ is composite.
...
Pollard's rho iterates a pseudorandom map modulo $n$; two iterates that collide modulo a hidden
prime factor reveal that factor through a GCD with their difference. The complete factorization
routine first removes small prime factors with a cached linear sieve, then uses Miller-Rabin and
Pollard's rho on the remaining cofactor. Factorizations remain sorted and compressed as (`prime`,
`exponent`) pairs.
