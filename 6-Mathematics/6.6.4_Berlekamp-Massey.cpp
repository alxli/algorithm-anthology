...
correction was needed. The order $L$ never decreases, and $2L$ terms suffice to determine a
...
An order-$L$ recurrence generally needs at least $2L$ terms to be determined, so the result is most
trustworthy when `s` is comfortably longer than twice the returned length. If the returned $L$ is
close to $n / 2$, the sequence is under-determined and the recurrence may be a spurious overfit.
...
- `berlekamp_massey(s)` returns the coefficients of the shortest recurrence that the sequence `s`
  ...
  vector is returned when `s` is entirely zero (no recurrence is needed). Entries are reduced modulo
  `MOD` internally.
