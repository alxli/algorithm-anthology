...
not one, since many shapes share the same preorder. Two encodings below rebuild a tree: a pair of
traversals, and a single preorder that records the null children.

Every routine here that rebuilds a tree allocates each node with `new` and returns raw pointers.
Thus the caller owns the resulting tree and should eventually delete its nodes in long-running
programs. Contest programs commonly omit this cleanup when the tree is needed until program
termination, since the operating system reclaims the process's memory on exit.
...
A pair of traversals usually is an encoding: an inorder together with either a preorder or a
postorder pins the shape down, because the inorder splits the values into a left group and a right
group once the root is known, and the other traversal is what identifies that root. Preorder puts
the root first, postorder puts it last, and each recursion then knows exactly how many values belong
to each side.
...
Time Complexity:
- O(n) expected per call, where $n$ is the number of nodes, from the hash lookups of inorder
  positions.

Space Complexity:
- O(n) for the allocated tree nodes.
- O(n) auxiliary for the position map, and O(h) auxiliary stack space, where $h$ is the height of
  the tree.
...
- `serialize(root, delim = '#')` returns the tree as a whitespace-separated preorder string, writing
  `delim` for each null child. The delimiter must not be whitespace, and streamed values must be
  non-empty and free of `delim` or whitespace.
- `deserialize(s, delim = '#')` rebuilds the tree that `serialize()` produced using `delim`. The
  value type must support stream input from the token written by its stream output operation.

Time Complexity:
- O(n) per call to `serialize()` and `deserialize()`, where $n$ is the number of nodes.

Space Complexity:
- O(n) for the allocated tree nodes, and O(h) auxiliary stack space, where $h$ is the height of the
  tree.
