...
- O(alpha(n) log n) per call to `is_united()` and `unite()`, where $n$ is the number of
  elements that have been added via `make_set()` so far, and $\alpha(n)$ is the extremely
