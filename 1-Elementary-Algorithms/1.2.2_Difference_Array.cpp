...
immediately adding `delta` to every element of the inclusive range `[lo, hi]`, add `delta` at
`diff[lo]` and subtract it at `diff[hi + 1]`; a final prefix sum reconstructs the updated values.
The idea extends to two dimensions: a rectangle update touches just the four corner cells of a
