...
- `find_cycle_floyd(f, x0)` repeatedly applies `f` starting from `x0` and returns the reached cycle
  as a tuple (`entry`, `start`, `length`), where `entry` is the first cycle value, `start` is the
  number of applications of `f` needed to reach it from `x0`, and `length` is the number of distinct
  values in the cycle.
