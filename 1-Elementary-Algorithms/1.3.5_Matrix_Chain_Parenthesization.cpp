...
For a chain of matrices, where matrix $i$ has $d_i$ rows and $d_{i+1}$ columns, $dp(l, r)$ is the
minimum cost of multiplying matrices $l$ through $r$. Trying every final split $k$ gives the
recurrence $dp(l, r) = \min_{l \le k < r}(dp(l, k) + dp(k + 1, r) + d_l d_{k+1} d_{r+1})$. The final
product multiplies a $d_l$-by-$d_{k+1}$ matrix with a $d_{k+1}$-by-$d_{r+1}$ matrix. Processing
intervals in increasing order of length ensures that both subintervals have already been solved. The
same pattern applies whenever a contiguous interval is formed by combining two smaller intervals.

- `matrix_chain_order(dim)` returns a pair (`operations`, `parenthesization`) containing the minimum
  number of scalar multiplications and one optimal parenthesization. Matrix `i` has dimension
  `dim[i]` by `dim[i + 1]`; all dimensions must be positive.
  ...
  for (int length = 2; length <= n; length++) {
    for (int lo = 0; lo + length <= n; lo++) {
      ...
      for (int k = lo; k < hi; k++) {
        int64_t candidate =
            dp[lo][k] + dp[k + 1][hi] +
            static_cast<int64_t>(dim[lo]) * dim[k + 1] * dim[hi + 1];  // Overflow warning.
