...
- `majority_element(lo, hi)` returns an iterator to the first occurrence of the majority element of
  $[`lo`, `hi`)$, or `hi` if no majority element exists. Requires `operator==` on the value type.
...
It majority_element(It lo, It hi) {
  ...
  for (It it = lo; it != hi; ++it) {
    n++;  // Tally the size to avoid an extra O(n) std::distance() pass for ForwardIterators.
    if (count == 0) {
      candidate = it;
      count = 1;
