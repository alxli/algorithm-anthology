...
- `max_subset_sum_at_most(lo, hi, v)` returns the maximum sum of any subset of the range `[lo, hi)`
  that does not exceed `v`. Values may be negative. The range is supplied as random-access
  iterators, and 64-bit integers are used in intermediate calculations to avoid overflow.
