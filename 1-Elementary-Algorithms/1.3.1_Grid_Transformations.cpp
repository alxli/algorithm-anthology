...
Grid transpose(const Grid &a) {
  ...
  int cols = rows == 0 ? 0 : static_cast<int>(a[0].size());
...
Grid rotate(const Grid &a, int degrees = 90) {
  ...
  Grid res(rotation % 180 == 0 ? rows : cols, std::vector<int>(rotation % 180 == 0 ? cols : rows));
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      if (rotation == 90) {
...
Grid &rotate_in_place(Grid &a, int degrees = 90) {
  assert(degrees % 90 == 0);
  int rotation = (degrees % 360 + 360) % 360;
  assert(rotation % 180 == 0 || a.empty() || a.size() == a[0].size());
  if (rotation == 90) {
