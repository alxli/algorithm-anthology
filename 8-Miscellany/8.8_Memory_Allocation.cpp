...
Simple bump allocators for local contest code. These are useful when creating many small objects and
individual `new`/`delete` overhead is measurable. They allocate from one fixed buffer and do not
reclaim individual objects. The global `operator new` override sends all dynamic allocations through
a static buffer. This is the shortest version, but it affects the whole program and should be used
only when you are sure the fixed buffer is large enough.
...
int main() {
  ...
  std::size_t before_vector = bump_pos;
  std::vector<int, BumpAllocator<int>> v;
  for (int i = 0; i < 1000; i++) {
    v.push_back(i);
  }
  assert(bump_pos < before_vector);
  assert(v.size() == 1000);
  assert(v[123] == 123);
  assert(v.back() == 999);
  return 0;
