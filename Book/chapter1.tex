...
\begin{compactitem}
  ...
  \item \inlinecode{largest\_histogram\_rectangle(heights)} returns the maximum area and its inclusive left and right endpoints in a histogram where each bar has width $1$.
...
\section{\texorpdfstring{Grid Algorithms}{1.3 Grid Algorithms}}
...
Traverses a rectangular grid as an implicit unweighted graph: each unblocked cell is a node joined to its unblocked four-way orthogonal neighbors. Depth-first flood fill finds connected regions, while breadth-first search finds shortest distances because every move has unit length. Seeding the BFS with several cells computes the distance to the nearest source in one pass.

\begin{compactitem}
  \item \inlinecode{flood\_fill(blocked, r, c)} returns all unblocked cells reachable from starting cell $({\inlinecodemath{r}}, {\inlinecodemath{c}})$, which must lie within the grid. If it is blocked, the returned vector is empty; otherwise, its size is the area of the containing component.
  \item \inlinecode{grid\_components(blocked)} returns the connected components of cells marked zero, with each cell represented by a pair $({\inlinecodemath{r}}, {\inlinecodemath{c}})$. Movement is allowed in the four orthogonal directions.
  \item \inlinecode{grid\_distances(blocked, sources)} returns an $R$ by $C$ matrix of shortest distances from the nearest source. Blocked and unreachable cells contain $-1$. Every source must be an unblocked cell represented by a pair $({\inlinecodemath{r}}, {\inlinecodemath{c}})$. Passing one source gives ordinary single-source shortest paths.
  \item \inlinecode{shortest\_grid\_path(blocked, source, target)} returns one shortest path from the unblocked cell \inlinecode{source} to \inlinecode{target}, including both endpoints, or an empty vector if \inlinecode{target} is blocked or unreachable. Both endpoints must lie within the grid.
\end{compactitem}

\textbf{Time Complexity}: $O(R \cdot C)$ per call, where $R$ and $C$ are the grid dimensions.

\textbf{Space Complexity}: $O(R \cdot C)$ auxiliary, including the recursion stack used by \inlinecode{grid\_components()} and $O(R \cdot C)$ for the returned result.
...
For path-counting, \inlinecode{dp[r][c]} stores the number of ways to reach cell $(r, c)$. For minimum-cost paths, \inlinecode{dp[r][c]} stores the cheapest cost to reach that cell, and a predecessor direction reconstructs one optimal path. The implementation retains only the current row of path costs while storing all predecessor directions. The helpers below assume movement only down or right.
...
\section{\texorpdfstring{Greedy and Scheduling}{1.4 Greedy and Scheduling}}
...
\section{\texorpdfstring{Streaming and Randomized Algorithms}{1.5 Streaming and Randomized Algorithms}}
...
Overflow warning: All accumulated costs and tiling counts must fit in \inlinecode{int64\_t}.
...
\section{\texorpdfstring{Miscellaneous Problems}{1.7 Miscellaneous Problems}}
