...
\end{lstlisting}
Comb sort is an improved bubble sort. While bubble sort compares only adjacent elements, comb sort compares elements separated by a fixed gap, decreasing that gap after every pass. Large gaps move small values near the end toward the front quickly, avoiding the slow movement of such values in bubble sort. The shrink factor $1.3$ is a common empirical choice.
...
\begin{compactitem}
  ...
  \item \inlinecode{nth\_element2(lo, nth, hi)} rearranges $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ in-place around the 0-based rank represented by iterator \inlinecode{nth}. This requires random-access iterators and \inlinecode{operator\textless{}} on the value type.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{CoordinateCompressor\textless{}T\textgreater{}(lo, hi)} constructs a compressor from the half-open iterator range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$. Alternatively, use the default constructor and register values with \inlinecode{add(x)}, then call \inlinecode{build()} once after all values are registered and before the first query.
...
All prefix sums and intermediate inclusion-exclusion results must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  ...
  \item $O(m \cdot n)$ for the table returned by \inlinecode{prefix\_sums\_2d(a)}.
...
All stored differences and reconstructed values must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  ...
  \item $O(m \cdot n)$ for storage and $O(m \cdot n)$ for the grid returned by \inlinecode{DifferenceArray2D::build()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{longest\_at\_most\_k\_distinct(a, k)} returns $({\inlinecodemath{length}}, {\inlinecodemath{lo}}, {\inlinecodemath{hi}})$, the maximum length and inclusive endpoints of a contiguous subarray containing at most \inlinecode{k} distinct values. If \inlinecode{k} is nonpositive, it returns the empty subarray.
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary for \inlinecode{largest\_rectangle()}.
...
\begin{compactitem}
  \item \inlinecode{max\_subarray\_sum(lo, hi)} returns the sum and inclusive endpoints $({\inlinecodemath{sum}}, {\inlinecodemath{begin}}, {\inlinecodemath{end}})$ for the maximal-sum subarray of $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$, where \inlinecode{lo} and \inlinecode{hi} are random-access iterators to numeric types. This implementation requires operators \inlinecode{+} and \inlinecode{\textless{}} on the iterators' value type. By convention, the empty subarray is allowed, so an input range containing only negative values returns sum $0$ and endpoints $[0, -1]$.
  \item \inlinecode{max\_submatrix\_sum(a)} returns the sum and inclusive boundaries $({\inlinecodemath{sum}}, {\inlinecodemath{r1}}, {\inlinecodemath{c1}}, {\inlinecodemath{r2}}, {\inlinecodemath{c2}})$ for the largest rectangular submatrix of a matrix \inlinecode{a} with $m$ rows and $n$ columns. This implementation requires operators \inlinecode{+} and \inlinecode{\textless{}} on the matrix value type. By convention, the empty submatrix is allowed, so a matrix containing only negative values returns sum $0$ with empty row and column intervals.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{find\_cycle\_floyd(f, x0)} repeatedly applies \inlinecode{f} starting from \inlinecode{x0} and returns the reached cycle as a tuple $({\inlinecodemath{entry}}, {\inlinecodemath{start}}, {\inlinecodemath{length}})$, where \inlinecode{entry} is the first cycle value, \inlinecode{start} is its index in the sequence, and \inlinecode{length} is the number of distinct values in the cycle.
...
\begin{compactitem}
  ...
  \item $O(t)$ auxiliary and $O(c)$ for the returned \inlinecode{used} from \inlinecode{min\_coin\_change(coins, target)}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{max\_subset\_sum\_at\_most(lo, hi, v)} returns a pair $({\inlinecodemath{sum}}, {\inlinecodemath{items}})$ containing that maximum sum and the selected item indices relative to \inlinecode{lo}. Values may be negative. The range is supplied as random-access iterators, and \inlinecode{v} must be nonnegative.
...
All subset sums and intermediate differences in \inlinecode{max\_subset\_sum\_at\_most()} must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  ...
  \item \inlinecode{min\_grid\_path\_cost(cost)} returns a pair $({\inlinecodemath{sum}}, {\inlinecodemath{path}})$ containing the minimum cost of a down/right path from the upper-left cell to the lower-right cell and the cells of one optimal path in order. If the grid is empty, \inlinecode{sum} is $0$ and \inlinecode{path} is empty.
\end{compactitem}

Path counts and path costs must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  ...
  \item $O(R \cdot C)$ auxiliary and $O(R + C)$ for the returned path from \inlinecode{min\_grid\_path\_cost(cost)}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{schedule\_intervals(intervals)} returns one maximum-size compatible subset of intervals, in the order they are selected, as original indices into an input vector of \inlinecode{Interval} with fields \inlinecode{start} and \inlinecode{finish}. Every interval must satisfy \inlinecode{start \textless{} finish}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{partition\_intervals(intervals)} returns a pair $({\inlinecodemath{rooms}}, {\inlinecodemath{room}})$, where \inlinecode{rooms} is the minimum number of rooms and \inlinecode{room[i]} is the assigned room for input interval \inlinecode{i}. Each \inlinecode{PartitionInterval} has fields \inlinecode{start} and \inlinecode{finish}, which must satisfy \inlinecode{start \textless{} finish}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{select\_deadline\_jobs(jobs)} returns a pair $({\inlinecodemath{profit}}, {\inlinecodemath{slot}})$ containing that maximum profit and \inlinecode{slot[i]}, the assigned time slot of input job \inlinecode{i}, or $-1$ if that job is not selected, for a vector of \inlinecode{Job} with fields \inlinecode{deadline} and \inlinecode{profit}. Deadlines are positive integers, and assigned slots are numbered starting from $1$. Jobs with nonpositive profit are not selected.
\end{compactitem}

The total profit must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  \item \inlinecode{select\_on\_time\_jobs(jobs)} returns the selected jobs as original input indices in a feasible earliest-deadline-first execution order, for an input vector of \inlinecode{TimedJob} with fields \inlinecode{duration} and \inlinecode{deadline}. Durations and deadlines must be nonnegative integers.
\end{compactitem}

The total duration must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  \item \inlinecode{fisher\_yates\_shuffle(lo, hi, rng)} randomly shuffles the range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ using the uniform random bit generator \inlinecode{rng}. The range must provide random-access iterators.
\end{compactitem}
...
Each class maintains its reservoir incrementally; call \inlinecode{add(x)} once per stream element in any order, then call \inlinecode{get()} to retrieve the result.
...
\begin{compactitem}
  ...
  \item \inlinecode{get()} returns the current sample. For \inlinecode{ReservoirSampleOne}, \inlinecode{get()} requires at least one \inlinecode{add()} call; for \inlinecode{ReservoirSampleK}, it returns the full reservoir, which may contain fewer than \inlinecode{k} elements if the stream was shorter.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to \inlinecode{ReservoirSampleOne::add()} and $O(1)$ amortized per call to \inlinecode{ReservoirSampleK::add()}.
...
\begin{compactitem}
  ...
  \item $O(k)$ storage for \inlinecode{ReservoirSampleK}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{majority\_element(lo, hi)} returns an iterator to the first occurrence of the majority element of $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$, or \inlinecode{hi} if no majority element exists. Requires \inlinecode{operator==} on the value type.
\end{compactitem}
...
Maintains the mean and variance of a stream of numbers in one pass using Welford's algorithm. The naive formula $\operatorname{Var}(X) = E[X^2] - E[X]^2$ can lose precision when the two terms are large and close together. Welford instead maintains the current mean $\bar{x}_n$ (stored as \inlinecode{avg}) and the unnormalized second central moment $M_{2,n} = \sum_{i=1}^{n}(x_i - \bar{x}_n)^2$ (stored as \inlinecode{m2}). Each new value shifts the running mean by its deviation divided by the new count, then adds the product of its deviations from the old and new means to \inlinecode{m2}. Population and sample variance follow by dividing \inlinecode{m2} by $n$ or $n - 1$.
...
\begin{compactitem}
  ...
  \item \inlinecode{variance\_population()} returns population variance, dividing by $n$, or $0$ if the summary is empty.
  \item \inlinecode{variance\_sample()} returns sample variance, dividing by $n - 1$, or $0$ if fewer than two values have been added.
\end{compactitem}

\textbf{Time Complexity}: $O(1)$ per call to \inlinecode{add(x)} and each query.

\textbf{Space Complexity}: $O(1)$ storage.
...
\begin{compactitem}
  \item \inlinecode{frequent\_candidates(lo, hi, k)} returns a hash table of candidate values to their residual counters. \inlinecode{k} must be at least $2$, and the value type must support equality and \inlinecode{std::hash}. Use \inlinecode{k = 2} for the Boyer-Moore majority-candidate special case.
\end{compactitem}

\Needspace*{3\baselineskip}
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ expected per call: a full-table decrement cancels $k$ occurrences and therefore happens at most $\lfloor n/k \rfloor$ times, so all decrement sweeps take $O(n)$ total. Hash-table operations take $O(1)$ expected time each.
  \item $O(n \cdot k)$ in the collision-heavy worst case for the hash table.
\end{compactitem}

\textbf{Space Complexity}: $O(1)$ auxiliary and $O(k)$ for the returned candidates.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(b)$ worst case per call to \inlinecode{popcount(x)}, \inlinecode{parity(x)}, \inlinecode{ctz(x)}, \inlinecode{ffs(x)}, \inlinecode{clz(x)}, and \inlinecode{for\_each\_set\_bit(x, f)}, where $b$ is \inlinecode{MASK\_BITS}.
...
\begin{compactitem}
  ...
  \item \inlinecode{next\_popcount(x)} returns the smallest integer greater than \inlinecode{x} with the same number of set bits (Gosper's hack), for \inlinecode{x \textgreater{} 0} when that next mask is representable in \inlinecode{mask\_t}.
...
\begin{compactitem}
  \item \inlinecode{assignment\_min\_cost(cost)} returns a pair $({\inlinecodemath{sum}}, {\inlinecodemath{job}})$, where \inlinecode{sum} is the minimum cost of assigning each worker to a distinct job, and \inlinecode{job[i]} is the job assigned to worker \inlinecode{i}. The input is a square matrix with one row per worker and one column per job.
  \item \inlinecode{minimum\_set\_cover(sets, universe\_size)} returns a pair $({\inlinecodemath{count}}, {\inlinecodemath{chosen}})$, where \inlinecode{count} is the minimum number of sets needed to cover all elements in $[0, u)$, where $u$ is \inlinecode{universe\_size}, and \inlinecode{chosen} contains one optimal list of set indices. If no cover exists, \inlinecode{count} is $-1$ and \inlinecode{chosen} is empty. Each input set is represented as a bitmask.
...
All accumulated costs and tiling counts must fit in \inlinecode{int64\_t}.
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(2^{n})$ auxiliary and $O(n)$ for the returned assignment from \inlinecode{assignment\_min\_cost()}.
  \item $O(2^{u})$ auxiliary and $O(s)$ for the returned indices from \inlinecode{minimum\_set\_cover()}.
...
\begin{compactitem}
  ...
  \item $O(\log b)$ per call to \inlinecode{inverse\_gray\_code(g)}, where $b$ is \inlinecode{MASK\_BITS}.
...
\begin{compactitem}
  \item \inlinecode{XorBasis()} constructs an empty basis over \inlinecode{mask\_t}, which is \inlinecode{uint64\_t} by default.
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary and $O(n)$ for the returned array from each convolution.
\end{compactitem}
