...
\begin{compactitem}
  \item \inlinecode{max\_subset\_sum\_at\_most(lo, hi, target)} returns a pair (\inlinecode{sum}, \inlinecode{items}) containing that maximum sum and the selected item indices relative to \inlinecode{lo}. Values may be negative. The range is supplied as random-access iterators, and \inlinecode{target} must be nonnegative.
