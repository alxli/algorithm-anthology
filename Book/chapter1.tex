...
\textbf{Time Complexity}: $O(n \cdot 2^{n/2})$ per call to \inlinecode{max\_subset\_sum\_at\_most()}, where $n$ is the distance between \inlinecode{lo} and \inlinecode{hi}.
...
\textbf{Time Complexity}: $O(nk)$ expected for this compact hash-table implementation, where $n$ is the number of stream values.
