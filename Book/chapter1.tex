...
Both implementations below take the range as ForwardIterators and require \inlinecode{operator\textless{}} on the value type.
...
\section{\texorpdfstring{Cycle Detection}{1.5 Cycle Detection}}
...
\begin{compactitem}
  \item \inlinecode{find\_cycle\_floyd(f, x0)} returns the cycle as a \inlinecode{(position, length)} pair, where \inlinecode{position} is the index at which the cycle begins and \inlinecode{length} is the number of elements in it.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{find\_cycle\_brent(f, x0)} returns the cycle as a \inlinecode{(position, length)} pair, where \inlinecode{position} is the index at which the cycle begins and \inlinecode{length} is the number of elements in it.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{popcount(x)} returns the number of set bits, analogous to \inlinecode{\_\_builtin\_popcount()}.
  \item \inlinecode{ctz(x)} returns the number of trailing 0-bits (undefined for 0), analogous to \inlinecode{\_\_builtin\_ctz()}.
  \item \inlinecode{clz(x)} returns the number of leading 0-bits (undefined for 0), analogous to \inlinecode{\_\_builtin\_clz()}.
  \item \inlinecode{is\_power\_of\_two(x)} returns whether \inlinecode{x} has exactly one set bit.
  \item \inlinecode{floor\_pow2(x)} returns the largest power of 2 that is $\leq$ \inlinecode{x} (for \inlinecode{x} $> 0$).
  \item \inlinecode{ceil\_pow2(x)} returns the smallest power of 2 that is $\geq$ \inlinecode{x} (for \inlinecode{x} $> 0$).
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(b)$ worst case per call to \inlinecode{popcount(x)}, \inlinecode{ctz(x)}, \inlinecode{clz(x)}, and \inlinecode{for\_each\_set\_bit(x, f)}, where $b$ is the bit width of the mask (32 here).
