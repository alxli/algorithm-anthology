...
These functions are equivalent to \inlinecode{std::rotate()}, taking three iterators \inlinecode{lo}, \inlinecode{mid}, and \inlinecode{hi} (\inlinecode{lo} $\leq$ \inlinecode{mid} $\leq$ \inlinecode{hi}) to perform a left rotation on the range \inlinecode{[lo, hi)}. After the function call, \inlinecode{[lo, hi)} will consist of the concatenation of elements originally in \inlinecode{[mid, hi)} + \inlinecode{[lo, mid)}. That is, the range \inlinecode{[lo, hi)} will be rearranged in such a way that the element at \inlinecode{mid} becomes the first element of the new range and the element at \inlinecode{mid - 1} becomes the last element, all while preserving the relative ordering of elements within the two rotated subarrays.
...
\begin{compactitem}
  \item \inlinecode{inversions(lo, hi)} uses merge sort to return the number of inversions given two random-access iterators as a range \inlinecode{[lo, hi)}. The input range will be sorted after the function call. Optionally, a comparison function object specifying a strict weak ordering may be specified to replace the default \inlinecode{operator\textless{}}.
...
\begin{compactitem}
  \item \inlinecode{CoordinateCompressor(lo, hi)} constructs a compressor over the values of \inlinecode{[lo, hi)}, ready for querying. Alternatively, use the default constructor and register values with \inlinecode{add(x)}, then call \inlinecode{build()} once after all values are registered and before the first query.
...
Partially sort a range so that one chosen position ends up holding its final sorted element, like \inlinecode{std::nth\_element()}.

\begin{compactitem}
  \item \inlinecode{nth\_element2(lo, nth, hi)} rearranges the range \inlinecode{[lo, hi)} such that the value pointed to by \inlinecode{nth} is the element that would be there if the range were fully sorted. Furthermore, the range is partitioned such that no value in \inlinecode{[lo, nth)} compares greater than the value pointed to by \inlinecode{nth} and no value in \inlinecode{(nth, hi)} compares less. Requires \inlinecode{operator\textless{}} on the iterator's value type.
\end{compactitem}

The pivot is chosen uniformly at random, and the partition step groups equal keys together so arrays with many duplicate values remain efficient. After each partition, the search continues in only the side that contains \inlinecode{nth}, shrinking the expected remaining work geometrically for a linear average.
...
\begin{compactitem}
  ...
  \item \inlinecode{range\_sum(pref, lo, hi)} returns the sum of the inclusive range \inlinecode{[lo, hi]}.
  \item \inlinecode{prefix\_sums\_2d(a)} returns a two-dimensional prefix sum table for matrix \inlinecode{a}.
  \item \inlinecode{rectangle\_sum(pref, r1, c1, r2, c2)} returns the sum of the rectangle with top left at \inlinecode{(r1, c1)} and bottom right at \inlinecode{(r2, c2)}.
\end{compactitem}
...
Applies many range-add updates to an array in linear total time using a difference array. Instead of immediately adding \inlinecode{delta} to every element of the inclusive range \inlinecode{[lo, hi]}, add \inlinecode{delta} at \inlinecode{diff[lo]} and subtract it at \inlinecode{diff[hi + 1]}; a final prefix sum reconstructs the updated values. The idea extends to two dimensions: a rectangle update touches just the four corner cells of a difference grid by inclusion-exclusion, and a final two-dimensional prefix sum reconstructs the grid.

\begin{compactitem}
  \item \inlinecode{DifferenceArray(n)} constructs an initially zero array of size \inlinecode{n}.
  \item \inlinecode{add(lo, hi, delta)} adds \inlinecode{delta} to every position in the inclusive range \inlinecode{[lo, hi]}.
  ...
  \item \inlinecode{add(r1, c1, r2, c2, delta)} adds \inlinecode{delta} to every cell of the rectangle with top left at \inlinecode{(r1, c1)} and bottom right at \inlinecode{(r2, c2)}, inclusive.
...
\begin{compactitem}
  \item \inlinecode{find\_cycle\_floyd(f, x0)} returns the reached cycle as a pair \inlinecode{(start, length)}, where \inlinecode{start} is the smallest index $i$ such that $x_i$ is in the cycle, and \inlinecode{length} is the number of distinct values in the cycle.
...
Both functions take a vector \inlinecode{a} of $n$ comparable values and a window length $k \in [1, n]$, returning a vector of $n - k + 1$ results, one per window in left-to-right order.
...
\begin{compactitem}
  \item \inlinecode{AggregateQueue()} constructs an empty queue.
...
\begin{compactitem}
  \item \inlinecode{previous\_less(a)} returns, for each \inlinecode{i}, the largest index \inlinecode{j \textless{} i} with \inlinecode{a[j] \textless{} a[i]}, or $-1$ if there's no such index.
  \item \inlinecode{next\_less(a)} returns, for each \inlinecode{i}, the smallest index \inlinecode{j \textgreater{} i} with \inlinecode{a[j] \textless{} a[i]}, or \inlinecode{n} if there's no such index.
  \item \inlinecode{previous\_greater(a)} returns, for each \inlinecode{i}, the largest index \inlinecode{j \textless{} i} with \inlinecode{a[j] \textgreater{} a[i]}, or $-1$ if there's no such index.
  \item \inlinecode{next\_greater(a)} returns, for each \inlinecode{i}, the smallest index \inlinecode{j \textgreater{} i} with \inlinecode{a[j] \textgreater{} a[i]}, or \inlinecode{n} if there's no such index.
  \item \inlinecode{largest\_rectangle(heights)} returns the maximum area of an axis-aligned rectangle that fits under the given histogram, where bar \inlinecode{i} has width 1 and height \inlinecode{heights[i]}.
\end{compactitem}

\textbf{Time Complexity}: $O(n)$ per call to all functions, where $n$ is the size of the input.

\textbf{Space Complexity}: $O(n)$ auxiliary heap space for the stack and result.
...
\begin{compactitem}
  ...
  \item \inlinecode{add(x)} inserts one copy of value \inlinecode{x} if \inlinecode{x} $\in$ \inlinecode{[0, n]}.
...
Given a range \inlinecode{[lo, hi)}, determine a longest subsequence of the range such that all of its elements are in strictly ascending order. The subsequence is not necessarily contiguous or unique, so only one such answer will be found. This implementation requires random-access iterators and \inlinecode{operator\textless{}} on the value type.
...
\begin{compactitem}
  \item \inlinecode{longest\_increasing\_subsequence(lo, hi)} returns a longest strictly increasing subsequence of \inlinecode{[lo, hi)} as a \inlinecode{std::vector} of the selected values, in order.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{max\_subset\_sum\_at\_most(lo, hi, v)} returns the maximum sum of any subset of the range \inlinecode{[lo, hi)} that does not exceed \inlinecode{v}. Values may be negative. The range is supplied as random-access iterators, and 64-bit integers are used in intermediate calculations to avoid overflow.
  \item \inlinecode{max\_subset\_sum\_bounded(lo, hi, target)} returns the maximum sum of any subset of the range \inlinecode{[lo, hi)} that does not exceed \inlinecode{target}. All weights and \inlinecode{target} must be nonnegative integers. This is faster than meet-in-the-middle when the largest weight is small relative to $2^{n/2}$.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{max\_zero\_submatrix(a)} returns the area of the largest all-zero rectangular submatrix of \inlinecode{a}, a two-dimensional vector of bool.
\end{compactitem}

\textbf{Time Complexity}: $O(m \cdot n)$ per call to \inlinecode{max\_zero\_submatrix()}, where $m$ is the number of rows and $n$ is the number of columns in the matrix.

\textbf{Space Complexity}: $O(n)$ auxiliary heap space, where $n$ is the number of columns in the matrix.
...
\begin{compactitem}
  \item \inlinecode{weighted\_interval\_scheduling(intervals)} returns the maximum total weight of a compatible subset from an input vector of \inlinecode{WeightedInterval} with fields \inlinecode{start}, \inlinecode{finish}, and \inlinecode{weight}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{max\_submatrix\_sum(a, \&r1, \&c1, \&r2, \&c2)} returns the largest sum of any rectangular submatrix for a matrix \inlinecode{a} with $m$ rows and $n$ columns. This implementation requires operators \inlinecode{+} and \inlinecode{\textless{}} to be defined on the iterators' value type. Optionally, four \inlinecode{int} pointers may be passed to store the boundary indices of the resulting subarray, with \inlinecode{(r1, c1)} specifiying the top-left index and \inlinecode{(r2, c2)} specifying the bottom-right index. By convention, the empty submatrix is allowed, so an input matrix consisting of only negative values returns 0 with an empty result interval.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{majority(lo, hi)} returns an iterator to the first occurrence of the majority element of the range \inlinecode{[lo, hi)}, or \inlinecode{hi} if no majority element exists. Requires \inlinecode{operator==} on the value type.
\end{compactitem}
...
Maintain a multiset of non-negative $b$-bit integers as a binary trie, storing each value as a root-to-leaf path of its bits from the most significant (bit $b - 1$) down to the least significant. Every node keeps a count of the stored values passing through it, which supports deletion and counting queries, and lets the trie answer XOR-extremal queries by a greedy walk down the bits.

The classic application is the maximum-XOR query: to maximize \inlinecode{x XOR y} over all stored \inlinecode{y}, walk from the most significant bit. At each step, descend toward the child whose bit differs from that of \inlinecode{x} whenever such a branch exists, since setting a higher bit of the result always dominates any combination of lower bits. The minimum-XOR query is the mirror image, preferring the matching bit.
...
Values must lie in \inlinecode{[0, 2\textasciicircum{}BITS)}; the template parameters select the unsigned word type \inlinecode{U} and the bit width \inlinecode{BITS}, which must be less than the width of \inlinecode{U} (for example, 30 with \inlinecode{uint32\_t} or 62 with \inlinecode{uint64\_t}). Nodes are kept in a pool indexed by integer, where index 0 is the root and also serves as the null child.

\begin{compactitem}
  \item \inlinecode{BinaryTrie()} constructs an empty multiset.
...
\textbf{Space Complexity}: $O({\inlinecodemath{BITS}} \cdot n)$ for storage, where $n$ is the number of distinct values inserted so far.
...
Bitwise convolution combines two arrays indexed by masks, grouping pairs of masks by a bitwise operation instead of by ordinary addition. For arrays \inlinecode{a} and \inlinecode{b} over an \inlinecode{n}-bit universe, the XOR convolution \inlinecode{c[m]} is the sum of \inlinecode{a[x]*b[y]} over all \inlinecode{x \textasciicircum{} y = m}. OR and AND convolution replace \inlinecode{x \textasciicircum{} y} with \inlinecode{x | y} or \inlinecode{x \& y}. These appear in subset DP, counting pairs by bitwise result, and algebra over the Boolean hypercube.
