...
\begin{compactitem}
  ...
  \item \inlinecode{min\_cost\_grid\_path(cost)} returns a pair (\inlinecode{min\_cost}, \inlinecode{path}) containing the minimum cost of a down/right path from the upper-left cell to the lower-right cell and the cells of one optimal path in order. If the grid is empty, \inlinecode{min\_cost} is $0$ and \inlinecode{path} is empty.
\end{compactitem}

Path counts and path costs must fit in \inlinecode{int64\_t}.

\textbf{Time Complexity}: $O(R \cdot C)$ per call to \inlinecode{count\_grid\_paths(blocked)} and \inlinecode{min\_cost\_grid\_path(cost)}, where $R$ and $C$ are the grid dimensions.

\Needspace*{3\baselineskip}
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(C)$ auxiliary for \inlinecode{count\_grid\_paths(blocked)}.
  \item $O(R \cdot C)$ auxiliary and $O(R + C)$ for the returned path from \inlinecode{min\_cost\_grid\_path(cost)}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{schedule\_intervals(intervals)} returns one maximum-size compatible subset of intervals, in the order they are selected, as original indices into an input vector of \inlinecode{Interval} with fields \inlinecode{start} and \inlinecode{finish}. Every interval must satisfy \inlinecode{start} \textless{} \inlinecode{finish}.
\end{compactitem}
