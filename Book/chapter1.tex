...
Given a range \inlinecode{[lo, hi)} of integers, return the maximum sum of any subset of the range that is at most a given integer \inlinecode{v}. This is a common meet-in-the-middle version of subset sum: enumerate subset sums for each half, sort one side, and binary search for the best compatible partner.

\begin{compactitem}
  \item \inlinecode{max\_subset\_sum\_at\_most(lo, hi, v)} returns the maximum sum of any subset of \inlinecode{[lo, hi)} that does not exceed \inlinecode{v}.
\end{compactitem}
