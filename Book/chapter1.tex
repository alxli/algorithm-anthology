...
\begin{compactitem}
  \item \inlinecode{nth\_element2(lo, nth, hi)} rearranges the range [\inlinecode{lo}, \inlinecode{hi}) such that the value pointed to by \inlinecode{nth} is the element that would be there if the range were fully sorted. Furthermore, the range is partitioned such that no value in [\inlinecode{lo}, \inlinecode{nth}) compares greater than the value pointed to by \inlinecode{nth} and no value in (\inlinecode{nth}, \inlinecode{hi}) compares less. This requires \inlinecode{operator\textless{}} defined on the iterator's value type.
\end{compactitem}
