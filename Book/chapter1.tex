...
\begin{compactitem}
  ...
  \item \inlinecode{compress2(lo, hi)} achieves the same result by inserting all values into a balanced binary search tree (\inlinecode{std::map}), which automatically removes duplicate values and supports efficient lookups of the compressed values.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{max\_zero\_submatrix(matrix)} returns the area of the largest all-zero rectangular submatrix of \inlinecode{matrix}, a two-dimensional vector of bool.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{maximize\_on\_time\_jobs(jobs)} returns the maximum number of jobs that can finish by their deadlines. Note that the input will be sorted by deadline after the call.
\end{compactitem}
...
Integers double as fixed-size sets of bits, where bit \inlinecode{b} (counting from 0 at the least significant end) is ``present'' when it is set to 1. Treating an integer as a bitmask makes set membership, insertion, deletion, and iteration into single machine instructions, which is why bitmasks are the backbone of subset dynamic programming and many low-level tricks. The helpers below operate on \inlinecode{unsigned} masks; to use 64-bit masks, replace \inlinecode{unsigned} with \inlinecode{unsigned long long}, the literal \inlinecode{1u} with \inlinecode{1ull}, and the \inlinecode{1u \textless{}\textless{} 31} inside \inlinecode{clz()} with \inlinecode{1ull \textless{}\textless{} 63}.

\begin{compactitem}
  \item \inlinecode{test\_bit(x, b)} returns whether bit \inlinecode{b} of \inlinecode{x} is set.
  \item \inlinecode{set\_bit(x, b)}, \inlinecode{clear\_bit(x, b)}, and \inlinecode{toggle\_bit(x, b)} return \inlinecode{x} with bit \inlinecode{b} respectively forced to 1, forced to 0, or flipped.
  ...
  \item \inlinecode{for\_each\_set\_bit(x, f)} calls \inlinecode{f(b)} once for each set bit position \inlinecode{b} of \inlinecode{x}, in increasing order.
\end{compactitem}
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item O(2\textasciicircum{}\inlinecode{popcount(m)}) per call to \inlinecode{submasks(m)}.
...
The \inlinecode{k}-th Gray code is obtained from the ordinary binary value \inlinecode{k} by \inlinecode{k \textasciicircum{} (k \textgreater{}\textgreater{} 1)}, a bijection on \inlinecode{[0, 2\textasciicircum{}n)}. Its inverse recovers the rank of a given Gray code, which tells how many flips into the sequence a mask sits.
