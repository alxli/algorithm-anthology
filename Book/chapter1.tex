...
All prefix sums, target differences, inclusion-exclusion results, and returned counts must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  ...
  \item \inlinecode{min\_length\_at\_least(a, target)} returns a tuple (\inlinecode{length}, \inlinecode{lo}, \inlinecode{hi}), the minimum length and inclusive endpoints of a contiguous subarray with sum at least \inlinecode{target}, or length $-1$ if none exists. Values in \inlinecode{a} must be nonnegative. If \inlinecode{target} is nonpositive, it returns the empty subarray.
  \item \inlinecode{longest\_at\_most\_k\_distinct(a, k)} returns a tuple (\inlinecode{length}, \inlinecode{lo}, \inlinecode{hi}), the maximum length and inclusive endpoints of a contiguous subarray containing at most \inlinecode{k} distinct values. If \inlinecode{k} is nonpositive, it returns the empty subarray.
...
\begin{compactitem}
  \item \inlinecode{max\_subarray\_sum(lo, hi)} returns the sum and inclusive endpoints (\inlinecode{sum}, \inlinecode{begin}, \inlinecode{end}) for the maximal-sum subarray of $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$, where \inlinecode{lo} and \inlinecode{hi} are random-access iterators to numeric types. This implementation requires operators \inlinecode{+} and \inlinecode{\textless{}} on the iterators' value type. By convention, the empty subarray is allowed, so an input range containing only negative values returns sum $0$ and endpoints $[0, -1]$.
  \item \inlinecode{max\_circular\_subarray\_sum(lo, hi)} returns a tuple (\inlinecode{sum}, \inlinecode{begin}, \inlinecode{end}) containing the maximal circular-subarray sum and its inclusive endpoints. If \inlinecode{begin \textless{}= end}, the result is an ordinary range; if \inlinecode{begin \textgreater{} end}, it wraps from \inlinecode{begin} through the end of the input and continues through \inlinecode{end}. The empty subarray is allowed under the same convention as \inlinecode{max\_subarray\_sum()}.
  \item \inlinecode{max\_product\_subarray(lo, hi)} returns a tuple (\inlinecode{product}, \inlinecode{begin}, \inlinecode{end}) containing the product and inclusive endpoints of a nonempty maximal-product subarray, or product $0$ and endpoints $[0, -1]$ for an empty input range.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to \inlinecode{max\_subarray\_sum()}, \inlinecode{max\_circular\_subarray\_sum()}, and \inlinecode{max\_product\_subarray()}, where $n$ is the distance between \inlinecode{lo} and \inlinecode{hi}.
...
\begin{compactitem}
  \item \inlinecode{find\_cycle\_floyd(f, x0)} repeatedly applies \inlinecode{f} starting from \inlinecode{x0} and returns the reached cycle as a tuple (\inlinecode{entry}, \inlinecode{start}, \inlinecode{length}), where \inlinecode{entry} is the first cycle value, \inlinecode{start} is its index in the sequence, and \inlinecode{length} is the number of distinct values in the cycle.
...
\begin{compactitem}
  \item \inlinecode{max\_subset\_sum\_at\_most(lo, hi, v)} returns a pair (\inlinecode{sum}, \inlinecode{items}) containing that maximum sum and the selected item indices relative to \inlinecode{lo}. Values may be negative. The range is supplied as random-access iterators, and \inlinecode{v} must be nonnegative.
...
\begin{compactitem}
  ...
  \item \inlinecode{min\_grid\_path\_cost(cost)} returns a pair (\inlinecode{sum}, \inlinecode{path}) containing the minimum cost of a down/right path from the upper-left cell to the lower-right cell and the cells of one optimal path in order. If the grid is empty, \inlinecode{sum} is $0$ and \inlinecode{path} is empty.
\end{compactitem}
...
Solves matrix-chain multiplication as a canonical example of interval dynamic programming. Matrix multiplication is associative, but multiplying an $a$-by-$b$ matrix with a $b$-by-$c$ matrix costs $abc$ scalar multiplications, so the choice of parenthesization can greatly affect the total cost.

For a chain of matrices, $dp(l,r)$ is the minimum cost of multiplying matrices $l$ through $r$. Trying every final split $k$ gives the recurrence $dp(l,r) = \min_{l \le k < r}(dp(l,k) + dp(k+1,r) + d_l d_{k+1} d_{r+1})$. Processing intervals in increasing order of length ensures that both subintervals have already been solved. The same pattern applies whenever a contiguous interval is formed by combining two smaller intervals.
...
The minimum cost and all intermediate products must fit in \inlinecode{int64\_t}.

\textbf{Time Complexity}: $O(n^{3})$ per call, where $n$ is the number of matrices.
...
\begin{compactitem}
  ...
  \item $O(n \cdot k)$ in the collision-heavy worst case for \inlinecode{frequent\_candidates()}.
\end{compactitem}
...
Maintains summary statistics as values arrive without storing the stream externally. Welford's algorithm computes the mean and variance using constant storage, while a pair of heaps maintains the exact median using linear storage.
...
For the median, a max-heap stores the lower half of the values and a min-heap stores the upper half. The heaps are balanced so that the lower heap has either the same number of values as the upper heap or one extra value. Their roots are therefore the middle value or values of the stream.
...
\begin{compactitem}
  ...
  \item \inlinecode{lower\_median()} returns the lower of the two middle values, or the middle value for an odd count. The tracker must be nonempty.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to \inlinecode{OnlineStatistics::add()} and each \inlinecode{OnlineStatistics} query.
  \item $O(\log n)$ per call to \inlinecode{OnlineMedian::add()}, where $n$ is the number of values stored.
  \item $O(1)$ per \inlinecode{OnlineMedian} query.
\end{compactitem}

\Needspace*{3\baselineskip}
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(1)$ for \inlinecode{OnlineStatistics}.
  \item $O(n)$ for \inlinecode{OnlineMedian}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{assignment\_min\_cost(cost)} returns a pair (\inlinecode{sum}, \inlinecode{job}), where \inlinecode{sum} is the minimum cost of assigning each worker to a distinct job, and \inlinecode{job[i]} is the job assigned to worker \inlinecode{i}. The input is a square matrix with one row per worker and one column per job.
  \item \inlinecode{minimum\_set\_cover(sets, universe\_size)} returns a pair (\inlinecode{count}, \inlinecode{chosen}), where \inlinecode{count} is the minimum number of sets needed to cover all elements in $[0, u)$, where $u$ is \inlinecode{universe\_size}, and \inlinecode{chosen} contains one optimal list of set indices. If no cover exists, \inlinecode{count} is $-1$ and \inlinecode{chosen} is empty. Each input set is represented as a bitmask.
