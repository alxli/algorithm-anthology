...
2 3 3 4 5 6 6 7 9
\end{exampleoutput}
\subsection{Dutch National Flag}
...
\begin{compactitem}
  ...
  \item \inlinecode{range\_sum(pref, l, r)} returns the sum of the half-open range \inlinecode{[l, r)}.
...
Applies many range-add updates to an array in linear total time using a difference array. Instead of immediately adding \inlinecode{delta} to every element of \inlinecode{[l, r)}, add \inlinecode{delta} at \inlinecode{diff[l]} and subtract it at \inlinecode{diff[r]}; a final prefix sum reconstructs the updated values.

\begin{compactitem}
  \item \inlinecode{difference\_array(n)} constructs an initially zero array of size \inlinecode{n}.
  \item \inlinecode{add(l, r, delta)} adds \inlinecode{delta} to every position in the half-open range \inlinecode{[l, r)}.
  \item \inlinecode{build()} returns the final array after all range updates.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to \inlinecode{add(l, r, delta)}.
...
\begin{compactitem}
  ...
  \item \inlinecode{dynamic\_mex(n)} maintains counts of values in \inlinecode{[0, n]}, enough to track the MEX of a multiset of at most \inlinecode{n} relevant nonnegative values.
...
\begin{compactitem}
  ...
  \item $O(\log n)$ per call to \inlinecode{add(x)}, \inlinecode{remove(x)}, and \inlinecode{get()} for \inlinecode{dynamic\_mex}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{interval\_t} stores \inlinecode{start}, \inlinecode{finish}, and the original \inlinecode{id}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{weighted\_interval\_t} stores \inlinecode{start}, \inlinecode{finish}, and \inlinecode{weight}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{job\_t} stores \inlinecode{deadline} and \inlinecode{profit}. Deadlines are positive integer time slots.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{timed\_job\_t} stores \inlinecode{duration} and \inlinecode{deadline}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{online\_statistics()} constructs an empty summary.
