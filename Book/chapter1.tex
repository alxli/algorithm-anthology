...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ auxiliary for \inlinecode{compress1()} and \inlinecode{compress2()}.
...
\begin{compactitem}
  ...
  \item $O(k)$ auxiliary for \inlinecode{longest\_at\_most\_k\_distinct(a, k)}.
  \item $O(n)$ auxiliary for the result of \inlinecode{sliding\_window\_extrema()}, plus $O(k)$ for the deque.
\end{compactitem}
...
\textbf{Space Complexity}: $O(w)$ auxiliary for a sliding window of width $w$.
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ auxiliary for the stack and result.
...
\begin{compactitem}
  ...
  \item $O(m)$ auxiliary for \inlinecode{max\_submatrix\_sum()}, where $m$ is the number of rows in the matrix.
\end{compactitem}
...
\textbf{Space Complexity}: $O(n)$ auxiliary for both approaches.
...
\begin{compactitem}
  ...
  \item \inlinecode{min\_coin\_change(coins, target)} returns a pair $({\inlinecodemath{count}}, {\inlinecodemath{used}})$, where \inlinecode{count} is the minimum number of coins needed to make sum \inlinecode{target}, and \inlinecode{used[i]} is the number of copies of \inlinecode{coins[i]} in one optimal multiset. If \inlinecode{target} cannot be formed, \inlinecode{count} is $-1$ and \inlinecode{used} is empty.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(c \cdot t)$ for \inlinecode{min\_coin\_change(coins, target)}.
\end{compactitem}

\Needspace*{3\baselineskip}
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(W)$ auxiliary for \inlinecode{unbounded\_knapsack()}.
  \item $O(t)$ auxiliary for \inlinecode{count\_coin\_change(coins, target)}.
  \item $O(t + c)$ auxiliary for \inlinecode{min\_coin\_change(coins, target)}.
\end{compactitem}
...
\end{lstlisting}
\subsection{Grid Dynamic Programming}
Solves basic dynamic programming problems on a rectangular grid. In grid DP, each cell represents a state and transitions usually come from already-computed neighboring cells, most commonly the cell above and the cell to the left. Processing rows from top to bottom and columns from left to right therefore makes every predecessor available when a state is computed.

For path-counting, \inlinecode{dp[r][c]} stores the number of ways to reach cell $({\inlinecodemath{r}}, {\inlinecodemath{c}})$. For minimum-cost paths, \inlinecode{dp[r][c]} stores the cheapest cost to reach that cell, and a predecessor direction reconstructs one optimal path. The helpers below assume movement only down or right.

\begin{compactitem}
  \item \inlinecode{count\_grid\_paths(blocked)} returns the number of paths from the upper-left cell to the lower-right cell, moving only down or right and avoiding blocked cells marked nonzero.
  \item \inlinecode{min\_grid\_path\_cost(cost)} returns a pair $({\inlinecodemath{sum}}, {\inlinecodemath{parent}})$, where \inlinecode{sum} is the minimum cost of a down/right path from the upper-left cell to the lower-right cell and \inlinecode{parent[r][c]} is \inlinecode{\textquotesingle{}U\textquotesingle{}} or \inlinecode{\textquotesingle{}L\textquotesingle{}}, indicating the predecessor direction for one optimal path. If the grid is empty, \inlinecode{sum} is $0$ and \inlinecode{parent} is empty.
\end{compactitem}

\textbf{Time Complexity}: $O(R \cdot C)$ per call to \inlinecode{count\_grid\_paths(blocked)} and \inlinecode{min\_grid\_path\_cost(cost)}, where $R$ and $C$ are the grid dimensions.

\textbf{Space Complexity}: $O(R \cdot C)$ auxiliary for \inlinecode{count\_grid\_paths(blocked)} and \inlinecode{min\_grid\_path\_cost(cost)}.
...
\begin{compactitem}
  ...
  \item $O(k)$ auxiliary for \inlinecode{ReservoirSampleK}.
\end{compactitem}
...
Bitmask dynamic programming uses a bitmask to represent a small set or frontier, then treats that mask as the DP state. This is useful when one dimension is small enough for $2^n$ states: assignment problems can store which jobs are already used, set cover can store which elements are covered, submask DP can split a set into groups, and grid tiling can store which cells on the current frontier are already occupied.

For assignment, \inlinecode{dp[mask]} is the minimum cost after assigning the first \inlinecode{popcount(mask)} workers to the selected jobs. The next worker tries every unused job. For set cover, \inlinecode{dp[mask]} is the minimum number of chosen sets needed to cover exactly the elements in \inlinecode{mask}; adding one set moves to the union mask. For set partitioning, iterating all submasks gives the recurrence \inlinecode{dp[mask] = min(dp[mask \textasciicircum{} sub] + cost[sub])} over all masks this has $O(3^{n})$ transitions. For domino tiling, the mask is a profile of the current row; this technique is often called DP on a broken profile or plug DP. A domino covers exactly two edge-adjacent cells, so each piece is either horizontal ($1 \times 2$) or vertical ($2 \times 1$).

\begin{compactitem}
  \item \inlinecode{assignment\_min\_cost(cost)} returns a pair \inlinecode{(sum, job)}, where \inlinecode{sum} is the minimum cost of assigning each worker to a distinct job, and \inlinecode{job[i]} is the job assigned to worker \inlinecode{i}.
  \item \inlinecode{minimum\_set\_cover(sets, universe\_size)} returns a pair \inlinecode{(count, chosen)}, where \inlinecode{count} is the minimum number of sets needed to cover all elements from $0$ to \inlinecode{universe\_size - 1}, and \inlinecode{chosen} contains one optimal list of set indices. If no cover exists, \inlinecode{count} is $-1$ and \inlinecode{chosen} is empty. Each input set is represented as a bitmask.
  \item \inlinecode{partition\_min\_cost(group\_cost)} returns the minimum total cost to partition all elements into disjoint nonempty groups, where \inlinecode{group\_cost[mask]} is the cost of taking \inlinecode{mask} as one group.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n^{2} \cdot 2^{n})$ per call to \inlinecode{assignment\_min\_cost()}, where $n$ is the number of workers and jobs.
  \item $O(s \cdot 2^{u})$ per call to \inlinecode{minimum\_set\_cover()}, where $s$ is the number of sets and $u$ is \inlinecode{universe\_size}.
  \item $O(3^{n})$ per call to \inlinecode{partition\_min\_cost()}, where $n$ is the number of elements.
  \item $O(r \cdot c \cdot 2^{c})$ per call to \inlinecode{count\_domino\_tilings()}, where $r$ and $c$ are the number of rows and columns, respectively.
\end{compactitem}

\Needspace*{3\baselineskip}
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(2^{n} + n)$ auxiliary for \inlinecode{assignment\_min\_cost()}.
  \item $O(2^{u} + s)$ auxiliary for \inlinecode{minimum\_set\_cover()}.
  \item $O(2^{n})$ auxiliary for \inlinecode{partition\_min\_cost()}.
  \item $O(2^{c})$ auxiliary for \inlinecode{count\_domino\_tilings()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary for each convolution.
\end{compactitem}
