...
\begin{compactitem}
  \item \inlinecode{previous\_less(a)} returns, for each \inlinecode{i}, the largest index \inlinecode{j \textless{} i} with \inlinecode{a[j] \textless{} a[i]}, or \inlinecode{-1}.
  \item \inlinecode{next\_less(a)} returns, for each \inlinecode{i}, the smallest index \inlinecode{j \textgreater{} i} with \inlinecode{a[j] \textless{} a[i]}, or \inlinecode{n}.
  \item \inlinecode{previous\_greater(a)} returns, for each \inlinecode{i}, the largest index \inlinecode{j \textless{} i} with \inlinecode{a[j] \textgreater{} a[i]}, or \inlinecode{-1}.
  \item \inlinecode{next\_greater(a)} returns, for each \inlinecode{i}, the smallest index \inlinecode{j \textgreater{} i} with \inlinecode{a[j] \textgreater{} a[i]}, or \inlinecode{n}.
...
\begin{compactitem}
  ...
  \item \inlinecode{clz(x)} returns the number of leading 0-bits (undefined for 0), analgoous to \inlinecode{\_\_builtin\_clz()}.
