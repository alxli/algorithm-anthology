...
\subsection{Point}
A 2D point class template supporting exact equality and lexicographic ordering. The coordinate type \inlinecode{T} defaults to \inlinecode{double}. Integer types (e.g. \inlinecode{int}) fully support all exact operations. Metric operations (\inlinecode{norm}, \inlinecode{arg}, \inlinecode{normalize}, \inlinecode{rotateCW/CCW}, \inlinecode{reflect} across a line) return coordinates of type \inlinecode{fp\_t}, which is \inlinecode{T} when \inlinecode{T} is floating-point and \inlinecode{double} otherwise.
...
\end{lstlisting}
\subsection{Circle}
A circle in two dimensions with exact stored-value equality and epsilon-aware geometric predicates. The circle centered at $(h, k)$ is represented by the relation $(x - h)^2 + (y - k)^2 = r^2$, where the radius $r$ is normalized to a nonnegative number.
