...
\begin{compactitem}
  ...
  \item \inlinecode{seg\_intersection(a, b, c, d, \&p, \&q, TOUCH\_IS\_INTERSECT)} intersects segments \inlinecode{a}-\inlinecode{b} and \inlinecode{c}-\inlinecode{d}, returning $-1$ (none), 0 (one point), or 1 (overlapping segment). The \inlinecode{TOUCH\_IS\_INTERSECT} flag (default \inlinecode{true}) controls whether segments that meet only at an endpoint count as intersecting. If the intersection is a point (and \inlinecode{p} is not \inlinecode{nullptr}), it will be stored into \inlinecode{p}. If the intersection is an overlapping segment (and \inlinecode{p} and \inlinecode{q} are not \inlinecode{nullptr}) then their endpoints will be stored into \inlinecode{p} and \inlinecode{q}. The output types \inlinecode{p} and \inlinecode{q} must be floating-point points.
  \item \inlinecode{seg\_intersection(a, b, c, d, TOUCH\_IS\_INTERSECT)} is a simplified, detection-only version of the above. Both versions are exact for detection if the input \inlinecode{Pt} type is integral.
  \item \inlinecode{closest\_point(a, b, c, p)} returns the closest point on line $ax + by + c = 0$ to point \inlinecode{p}.
\end{compactitem}

Overflow warning: the exact integer paths multiply coordinate differences (cross products and squared lengths), which grow like the squared coordinate magnitude. With 32-bit \inlinecode{int} coordinates these overflow once coordinates exceed \textasciitilde{}46000, so for larger integer inputs use a point type with 64-bit (\inlinecode{int64\_t}) coordinates.
...
\begin{compactitem}
  ...
  \item \inlinecode{polygon\_centroid(lo, hi)} returns the centroid (center of mass) of a non-degenerate simple polygon with vertices in boundary order as \inlinecode{\{x, y\}}. It uses signed shoelace weights, so clockwise and counter-clockwise inputs both work.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{point\_in\_polygon(p, lo, hi, EDGE\_IS\_INSIDE)} returns whether \inlinecode{p} lies within the polygon with vertices specified by the range \inlinecode{[lo, hi)} of points in either clockwise or counter-clockwise order. The \inlinecode{EDGE\_IS\_INSIDE} flag (default \inlinecode{true}) controls whether points exactly on an edge or vertex count as inside.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{convex\_polygon\_tangents(poly, p)} returns the two tangent vertex indices from an outside point \inlinecode{p} as (left, right). The left tangent has all polygon vertices on or to the left of the directed line \inlinecode{p} $\to$ \inlinecode{poly[left]}; the right tangent is analogous for the right side.
\end{compactitem}
...
\end{lstlisting}
\subsection{Max Collinear Points}
...
\begin{compactitem}
  \item \inlinecode{convex\_cut(lo, hi, p, q)} returns the portion of the polygon lying on or to the left of the directed line \inlinecode{p} $\to$ \inlinecode{q}. The input range \inlinecode{[lo, hi)} must contain the vertices of a convex polygon in boundary order, either clockwise or counterclockwise. The returned polygon preserves that boundary order. If \inlinecode{p} equals \inlinecode{q}, the cutting line is invalid and an error is thrown.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{half\_plane\_intersection(planes)} returns the polygon cut out by the half-planes in counter-clockwise order. Each half-plane is represented by \inlinecode{HalfPlane(p, q)}, meaning the closed region to the left of the directed line \inlinecode{p} $\to$ \inlinecode{q}.
  \item If the intersection is empty, the function returns an empty vector.
  \item If the true intersection is unbounded, the returned polygon is not meaningful. Add explicit bounding-box half-planes when a bounded polygon is required.
\end{compactitem}
...
Given two convex polygons, compute their Minkowski sum: the set of all points \inlinecode{a + b} where \inlinecode{a} comes from the first polygon and \inlinecode{b} comes from the second. After rotating both polygons to start at their lowest-leftmost vertex, the boundary edges of the sum are obtained by merging the two cyclic edge-angle lists. Minkowski sums are useful for collision/translation problems, convex polygon distance checks, and diameter computations via $P + (-P)$.
...
This implementation uses the Guibas-Stolfi divide-and-conquer algorithm with a quad-edge data structure. The point set is split in half, each half is triangulated recursively, and the halves are stitched together from the bottom up by a sequence of cross edges, with circumcircle tests deciding each connecting edge and deleting invalidated ones. Input points are copied to \inlinecode{Point} with \inlinecode{double} coordinates, sorted lexicographically, and duplicates are removed. If four or more points are cocircular, the Delaunay triangulation is not unique; this implementation returns one valid triangulation.
...
\setcounter{subsection}{0}
\subsection{3D Point}
A 3D point class template supporting vector arithmetic, dot and cross products, distances, angles, unit vectors, normals, and rotation about an axis. Integer coordinates are suitable for exact algebraic predicates (\inlinecode{dot}, \inlinecode{cross}, \inlinecode{sqnorm}) as long as intermediate products do not overflow; metric operations return floating-point values.

Type aliases:

\begin{compactitem}
  \item \inlinecode{Point3I = Point3D\textless{}int\textgreater{}}
  \item \inlinecode{Point3L = Point3D\textless{}int64\_t\textgreater{}}
  \item \inlinecode{Point3 = Point3D\textless{}double\textgreater{}}
\end{compactitem}
...
Given an oriented triangular mesh of a closed polyhedron, compute its signed volume. Each triangle contributes the signed volume of the tetrahedron formed by the triangle and the origin: \inlinecode{dot(cross(a, b), c) / 6}. If faces are oriented outward, the result is positive for the orientation used by \inlinecode{convex\_hull\_3d()} in 7.5.2; take \inlinecode{abs()} if only the magnitude is needed.

\begin{compactitem}
  \item \inlinecode{signed\_polyhedron\_volume(points, faces)} returns the signed volume. Each face is an index triple with fields \inlinecode{.a}, \inlinecode{.b}, \inlinecode{.c}.
\end{compactitem}
