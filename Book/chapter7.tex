...
\begin{compactitem}
  ...
  \item \inlinecode{same\_side(p1, p2, a, b, edge\_is\_inside = true)} returns whether points \inlinecode{p1} and \inlinecode{p2} lie on the same side of the line containing points \inlinecode{a} and \inlinecode{b}. If one or both points lie exactly on the line, then the result will depend on \inlinecode{edge\_is\_inside}.
  \item \inlinecode{point\_in\_triangle(p, a, b, c, edge\_is\_inside = true)} returns whether point \inlinecode{p} lies within the triangle with vertices \inlinecode{a}, \inlinecode{b}, and \inlinecode{c}. If the point lies on or close to an edge (by roughly \inlinecode{EPS}), then the result will depend on \inlinecode{edge\_is\_inside}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{point\_in\_rectangle(p, v, w, h, edge\_is\_inside = true)} returns whether point \inlinecode{p} lies inside the axis-aligned rectangle with corner \inlinecode{v}, width \inlinecode{w}, and height \inlinecode{h}. Negative widths and heights are supported. The \inlinecode{edge\_is\_inside} flag controls whether boundary points count.
  \item \inlinecode{point\_in\_rectangle(p, a, b, edge\_is\_inside = true)} returns whether point \inlinecode{p} lies inside the axis-aligned rectangle with opposite corners \inlinecode{a} and \inlinecode{b}.
  \item \inlinecode{rectangle\_intersection(a1, b1, a2, b2, \&p, \&q, edge\_is\_inside = true)} computes the intersection of two axis-aligned rectangles, where \inlinecode{a1}/\inlinecode{b1} and \inlinecode{a2}/\inlinecode{b2} are opposite-corner pairs. Returns $-1$ if the rectangles are disjoint, $0$ if they partially intersect, $1$ if the first rectangle is completely inside the second, and $2$ if the second rectangle is completely inside the first. If an intersection exists, its lower-left and upper-right corners are stored in \inlinecode{p} and \inlinecode{q} when those pointers are non-null. The \inlinecode{edge\_is\_inside} flag controls whether boundary-only contact counts as intersecting.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{seg\_intersection(a, b, c, d, \&p, \&q, touch\_is\_intersect = true)} intersects segments \inlinecode{a}-\inlinecode{b} and \inlinecode{c}-\inlinecode{d}, returning $-1$ (none), $0$ (one point), or $1$ (overlapping segment). The \inlinecode{touch\_is\_intersect} flag controls whether segments that meet only at an endpoint count as intersecting. If the intersection is a point (and \inlinecode{p} is not \inlinecode{nullptr}), it will be stored into \inlinecode{p}. If the intersection is an overlapping segment (and \inlinecode{p} and \inlinecode{q} are not \inlinecode{nullptr}) then their endpoints will be stored into pointers \inlinecode{p} and \inlinecode{q}, which must reference a floating-point point type.
  \item \inlinecode{seg\_intersection(a, b, c, d, touch\_is\_intersect = true)} is a simplified, detection-only version of the above. Both versions are exact for detection if the input point type is integral.
...
\begin{compactitem}
  \item \inlinecode{circle\_tangents(c1, r1, c2, r2, internal = false)} returns $0$, $1$, or $2$ tangent point pairs. Each pair \inlinecode{\{p, q\}} means the tangent line touches the first circle at \inlinecode{p} and the second circle at \inlinecode{q}. Set \inlinecode{internal = true} to get internal tangents; otherwise external tangents are returned.
...
\begin{compactitem}
  \item \inlinecode{point\_in\_polygon(p, lo, hi, edge\_is\_inside = true)} returns whether \inlinecode{p} lies within the polygon with vertices specified by the range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ of points in either clockwise or counter-clockwise order. The \inlinecode{edge\_is\_inside} flag controls whether points exactly on an edge or vertex count as inside.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{point\_in\_convex\_polygon(poly, p, edge\_is\_inside = true)} returns whether point \inlinecode{p} lies inside the convex polygon \inlinecode{poly}. The polygon must be in counter-clockwise order, with no repeated final vertex. Points on the boundary count as inside unless \inlinecode{edge\_is\_inside} is set to \inlinecode{false}.
