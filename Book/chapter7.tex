...
\begin{compactitem}
  ...
  \item \inlinecode{Circle(a, b, c)} constructs the circumcircle through non-collinear points \inlinecode{a}, \inlinecode{b}, and \inlinecode{c}.
  \item \inlinecode{Circle(a, b, r)} constructs one circle of radius \inlinecode{abs(r)} passing through points \inlinecode{a} and \inlinecode{b}, throwing if the inputs admit no finite circle.
  ...
  \item \inlinecode{incircle(a, b, c)} returns the circle inscribed inside triangle \inlinecode{abc}.
...
When exact integer reasoning about a circumcircle is needed, use \inlinecode{in\_circumcircle(a, b, c, d)}, which tests whether \inlinecode{d} lies inside the circle through \inlinecode{a}, \inlinecode{b}, \inlinecode{c} using a determinant (no square root) and is therefore exact for integer coordinates without ever building a \inlinecode{Circle}. The determinant has degree four in the coordinate magnitude, so use a wider coordinate type for large integer inputs.
...
\begin{compactitem}
  ...
  \item \inlinecode{rectangle\_intersection(a1, b1, a2, b2, \&p, \&q)} determines the intersection region of the rectangle with opposing vertices \inlinecode{a1} and \inlinecode{b1} and the rectangle with opposing vertices \inlinecode{a2} and \inlinecode{b2}. Returns $-1$ if the rectangles are completely disjoint, 0 if the rectangles partially intersect, 1 if the first rectangle is completely inside the second, and 2 if the second rectangle is completely inside the first. If there is an intersection, the lower-left and upper-right vertices of the normalized intersection rectangle will be stored into pointers \inlinecode{p} and \inlinecode{q} if they are not \inlinecode{nullptr}. If the intersection is a single point or line segment, then the result will depend on the setting of \inlinecode{EDGE\_IS\_INSIDE} within \inlinecode{point\_in\_rectangle()}.
\end{compactitem}
...
\section{Angles, Distances, and Intersections}
\setcounter{section}{2}
\setcounter{subsection}{0}
\subsection{Angles}
Angle calculations in two dimensions. All operations are inherently floating-point (\inlinecode{atan2}, \inlinecode{acos}, trigonometry), so these functions use the local double-coordinate \inlinecode{PointD} type. Convert integer points to \inlinecode{PointD} when asking for angles. The constants \inlinecode{DEG} and \inlinecode{RAD} may be used as multipliers to convert between degrees and radians. For example, if $t$ is a value in degrees, then $t \cdot$ \inlinecode{DEG} is the equivalent angle in radians; if $t$ is in radians, then $t \cdot$ \inlinecode{RAD} is the equivalent angle in degrees.
...
\begin{compactitem}
  ...
  \item \inlinecode{closest\_point(a, b, c, p)} returns the closest point on line $ax + by + c = 0$ to $p$.
\end{compactitem}
...
\section{Polygons and Planar Searches}
...
\begin{compactitem}
  \item \inlinecode{convex\_hull(lo, hi)} returns the convex hull in clockwise order. The input range is sorted lexicographically after the call. To instead return the hull points counter-clockwise order, replace every \inlinecode{\textgreater{}= 0} with \inlinecode{\textless{}= 0}.
...
Given a list of line segments in two dimensions, determine whether any pair of segments intersect using a sweep line algorithm. The input type \inlinecode{Segment\textless{}Pt\textgreater{}} is templated on the point type, so endpoints may be integer (\inlinecode{PointI}) or floating-point (\inlinecode{Point}/\inlinecode{PointD}). The cross-product sign tests in \inlinecode{seg\_intersection} are exact for integer endpoints, so intersection detection is exact.
...
\section{Advanced Planar Geometry}
...
The triangulation may not exist (e.g. for a set of collinear points), or may not be unique if it does exist. This implementation produces one valid result using a simple algorithm which encases each triangle in a circle and rejects the triangle if another point in the tessellation is within the generalized circle.
