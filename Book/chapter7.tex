...
Overflow warning: each individual cross product is still on the order of the squared coordinate magnitude, so for integer point types use a 64-bit coordinate type (e.g. \inlinecode{PointL} from 7.1.1) once coordinates reach a few tens of thousands.
...
\begin{compactitem}
  \item \inlinecode{point\_in\_rectangle(p, v, w, h, edge\_is\_inside = true)} returns whether point \inlinecode{p} lies inside the axis-aligned rectangle whose opposite corners are \inlinecode{v} and $({\inlinecodemath{v.x + w}}, {\inlinecodemath{v.y + h}})$. Negative widths and heights are supported. The \inlinecode{edge\_is\_inside} flag controls whether boundary points count.
...
\begin{compactitem}
  ...
  \item \inlinecode{angle\_between(a, b)} returns the directed counter-clockwise angle in $[0, 2\pi)$ from vector \inlinecode{a} to vector \inlinecode{b}, treating both points as vectors from the origin.
...
The points passed to \inlinecode{angle()} must differ from its vertex \inlinecode{o}, vectors passed to \inlinecode{angle\_between()} must be nonzero, and coefficient pairs passed to its line overload must represent valid lines.
...
Coefficient triples passed to \inlinecode{line\_dist()} must represent valid lines. Use a floating-point point type with \inlinecode{closest\_point()} when the closest point may have fractional coordinates.
...
Coefficient triples must represent valid lines, and each point pair used to define an infinite line must contain two distinct points.
...
Circle radii are normalized to be nonnegative. \inlinecode{tangent()} requires a positive radius, and lines passed to \inlinecode{intersection()} must be valid.
...
Given a list of points in two dimensions, computes the convex hull using the monotone chain algorithm and the diametral pair using rotating calipers. Monotone chain sorts the points lexicographically and builds the lower and upper hulls in one pass each, popping any point that would fail to create a counter-clockwise turn. Rotating calipers then walks two antipodal pointers around the hull, advancing whichever increases the separation, visiting every candidate diametral pair in linear time. The functions are templated on the iterator type; the value type of the iterator is used as the point type. Both functions accept either floating-point or integral coordinates, and use exact comparisons for integral points.

\begin{compactitem}
  \item \inlinecode{convex\_hull(lo, hi)} returns the convex hull in counter-clockwise order for a range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ of points. Duplicate input points and collinear points along hull edges are omitted. To instead return the hull points in clockwise order, replace the cross product comparisons \inlinecode{\textless{}= 0} with \inlinecode{\textgreater{}= 0}.
...
\begin{compactitem}
  \item \inlinecode{manhattan\_mst\_edges(p)} returns $O(n)$ candidate edges $({\inlinecodemath{w}}, {\inlinecodemath{u}}, {\inlinecodemath{v}})$.
  \item \inlinecode{manhattan\_mst\_weight(p)} is a small Kruskal wrapper that returns the MST weight.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{half\_plane\_intersection(planes)} returns the polygon cut out by the half-planes in counter-clockwise order. Each half-plane is represented by \inlinecode{HalfPlane(p, q)}, meaning the closed region to the left of the directed line \inlinecode{p} $\to$ \inlinecode{q}. If the intersection is empty, the function returns an empty vector. If the true intersection is unbounded, the returned polygon is not meaningful; add explicit bounding-box half-planes when a bounded polygon is required. The two points defining each half-plane must differ.
\end{compactitem}
...
\textbf{Time Complexity}: $O(n \cdot m \cdot (n + m) \cdot \log\left(n + m\right))$ per call to \inlinecode{intersection\_area(lo1, hi1, lo2, hi2)} and \inlinecode{union\_area(lo1, hi1, lo2, hi2)}, where $n$ is the number of vertices in the first polygon and $m$ is the number of vertices in the second polygon (or $O(N^{3} \log N)$ for $N = n + m$).

\textbf{Space Complexity}: $O(n \cdot m)$ auxiliary for \inlinecode{intersection\_area(lo1, hi1, lo2, hi2)} and \inlinecode{union\_area(lo1, hi1, lo2, hi2)}, where $n$ and $m$ are the respective polygon sizes.
...
\setcounter{subsection}{0}
\subsection{3D Point}
A 3D point class template supporting vector arithmetic, dot and cross products, distances, angles, unit vectors, normals, and rotation about an axis. Point equality and ordering are exact, while \inlinecode{EQ(p, q)} provides epsilon equality for floating-point coordinates. Integer coordinates are suitable for the exact algebraic operations \inlinecode{dot()}, \inlinecode{cross()}, and \inlinecode{sqnorm()} as long as intermediate products do not overflow; metric operations preserve floating-point coordinates and otherwise convert them to \inlinecode{double}.

\inlinecode{unit()} requires a nonzero point, \inlinecode{normal(p)} requires nonparallel vectors, and \inlinecode{rotate(angle, axis)} requires a nonzero axis.
...
Given an oriented triangular mesh of a closed polyhedron, compute its signed volume. Each triangle contributes the signed volume of the tetrahedron formed by the triangle $abc$ and the origin $((a \times b) \cdot c) / 6$. If faces are oriented outward, the result is positive for the orientation used by \inlinecode{convex\_hull\_3d()} in 7.5.3; take \inlinecode{abs()} if only the magnitude is needed.

\begin{compactitem}
  \item \inlinecode{signed\_polyhedron\_volume(p, faces)} returns the signed volume. Each face \inlinecode{(a, b, c)} represents the triangle with vertices \inlinecode{p[a]}, \inlinecode{p[b]}, and \inlinecode{p[c]}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{convex\_hull\_3d(p)} returns triangular faces as \inlinecode{Face\{a, b, c\}} triples, where each face has vertices \inlinecode{p[a]}, \inlinecode{p[b]}, and \inlinecode{p[c]}.
\end{compactitem}
