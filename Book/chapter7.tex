...
\begin{compactitem}
  \item \inlinecode{point\_in\_rectangle(p, v, w, h, edge\_is\_inside = true)} returns whether point \inlinecode{p} lies inside the axis-aligned rectangle whose opposite corners are \inlinecode{v} and (\inlinecode{v.x + w}, \inlinecode{v.y + h}). Negative widths and heights are supported. The \inlinecode{edge\_is\_inside} flag controls whether boundary points count.
...
Overflow warning: the edge orientation test forms a cross product that grows like the squared coordinate magnitude. For integer point types use a 64-bit coordinate type (e.g. \inlinecode{PointL} from 7.1.1) once coordinates exceed roughly a few tens of thousands.
...
\begin{compactitem}
  \item \inlinecode{manhattan\_mst\_edges(p)} returns $O(n)$ candidate edges as tuples (\inlinecode{weight}, \inlinecode{u}, \inlinecode{v}).
...
\begin{compactitem}
  \item \inlinecode{signed\_polyhedron\_volume(p, faces)} returns the signed volume. Each face triplet (\inlinecode{a}, \inlinecode{b}, \inlinecode{c}) represents the triangle with vertices \inlinecode{p[a]}, \inlinecode{p[b]}, and \inlinecode{p[c]}.
\end{compactitem}
