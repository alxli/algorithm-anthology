...
\begin{compactitem}
  ...
  \item \inlinecode{PointLD = TPoint\textless{}long double\textgreater{}}: extra precision (where supported)
...
\begin{compactitem}
  ...
  \item \inlinecode{LineLD = TLine\textless{}long double\textgreater{}}: extra precision (where supported)
...
When a line crosses two sides, their returned order is determined using a floating-point line parameter.
...
\begin{compactitem}
  \item \inlinecode{find\_intersection(lo, hi, \&res1, \&res2)} returns whether any pair of segments intersect given a range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ of segments, where \inlinecode{lo} and \inlinecode{hi} are random-access iterators. If an intersection is found, then one such pair of segments is stored in \inlinecode{res1} and \inlinecode{res2}. Constructing a \inlinecode{Segment} places its lexicographically smaller endpoint first, as required by the sweep. If no intersection is found, the output segments are unchanged.
\end{compactitem}
...
The function is templated on the input point type. Side classification is done with cross products. Edge-line intersection points are computed in floating point, and the returned polygon uses \inlinecode{Point} with \inlinecode{double} coordinates.
