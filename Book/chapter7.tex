...
\begin{compactitem}
  \item \inlinecode{PointI = Point\textless{}int\textgreater{}}: exact integer geometry
...
\begin{compactitem}
  \item \inlinecode{LineI = line\textless{}int\textgreater{}}: exact integer-coefficient lines
...
\end{lstlisting}
\subsection{Triangle}
Common triangle calculations in two dimensions. The functions are templated on the point type \inlinecode{Pt}, which should work with \inlinecode{Point}/ \inlinecode{PointD}/\inlinecode{PointI} from 7.1.1, std::pair, or any struct with numeric \inlinecode{.x} and \inlinecode{.y} fields. \inlinecode{triangle\_area()} returns \inlinecode{double} regardless of input type (the area is fractional via the \inlinecode{/2}). \inlinecode{same\_side()} and \inlinecode{point\_in\_triangle()} do all arithmetic in the point's own coordinate type and are exact for integer points: they reduce each cross product to a sign before combining, so no precision is lost and no overflow occurs from multiplying cross products.

Note: with an integer point type, coordinates must be integers - fractional literals like \inlinecode{-2.44} cannot be represented (and brace-initialization \inlinecode{PointI\{-2.44, ...\}} is a narrowing error). Use a floating-point point type for fractional coordinates.
...
The functions are templated on the point type \inlinecode{Pt}, which should work with \inlinecode{Point}/ \inlinecode{PointD}/ \inlinecode{PointI} from 7.1.1, or any struct with numeric \inlinecode{.x} and \inlinecode{.y} fields. All operations use comparisons only and are exact for integer-coordinate points.
...
\begin{compactitem}
  ...
  \item \inlinecode{rectangle\_intersection(a1, b1, a2, b2, \&p, \&q)} determines the intersection region of the rectangle with opposing vertices \inlinecode{a\_1} and \inlinecode{b\_1} and the rectangle with opposing vertices \inlinecode{a\_2} and \inlinecode{b\_2}. Returns \inlinecode{-1} if the rectangles are completely disjoint, 0 if the rectangles partially intersect, 1 if the first rectangle is completely inside the second, and 2 if the second rectangle is completely inside the first. If there is an intersection, the lower-left and upper-right vertices of the normalized intersection rectangle will be stored into pointers \inlinecode{p} and \inlinecode{q} if they are not \inlinecode{nullptr}. If the intersection is a single point or line segment, then the result will depend on the setting of \inlinecode{EDGE\_IS\_INSIDE} within \inlinecode{point\_in\_rectangle()}.
\end{compactitem}
...
The functions below are templated on the point type \inlinecode{Pt}. The local \inlinecode{Point} struct (double coordinates) is the default; replace it with \inlinecode{PointD}/ \inlinecode{PointI} from 7.1.1 or any struct with numeric \inlinecode{.x} and \inlinecode{.y} fields. \inlinecode{sqnorm}, \inlinecode{dot}, \inlinecode{cross}, and \inlinecode{sqdist} preserve the coordinate type and are exact for integer points. Functions that return actual distances or projected points use \inlinecode{double} only for the final division/square root.
...
The functions below are templated on the point type \inlinecode{Pt}. The local \inlinecode{Point} struct (double coordinates) is the default; replace it with \inlinecode{PointD}/ \inlinecode{PointI} from 7.1.1 or any struct with numeric \inlinecode{.x} and \inlinecode{.y} fields.

For \inlinecode{seg\_intersection()} detection-only use (\inlinecode{p=nullptr}/\inlinecode{q=nullptr}), calculations are exact when \inlinecode{Pt} has integer coordinates. Segment intersection point output may use a different floating-point point type, e.g. integer endpoints with \inlinecode{Point*} outputs.
...
\begin{compactitem}
  \item \inlinecode{tangent(c, p, \&l1, \&l2)} determines the line(s) tangent to circle \inlinecode{c} that pass through point \inlinecode{p}, returning \inlinecode{-1} if there is no tangent line because \inlinecode{p} is strictly inside \inlinecode{c}, 0 if there is exactly one tangent line because \inlinecode{p} is on the boundary of \inlinecode{c} (in which case the line will be stored into pointer \inlinecode{l1} if it's not \inlinecode{nullptr}), or 1 if there are two tangent lines because \inlinecode{p} is strictly outside of \inlinecode{c} (in which case the lines will be stored into pointers \inlinecode{l1} and \inlinecode{l2} if they are not \inlinecode{nullptr}).
...
The functions below are templated on the point type. The local \inlinecode{Point} struct (\inlinecode{double} coordinates) is the default; replace it with \inlinecode{Point}/\inlinecode{PointD}/ \inlinecode{PointI} from 7.1.1 or any struct with numeric \inlinecode{.x} and \inlinecode{.y} fields and \inlinecode{\textless{}} / \inlinecode{==} operators.

\inlinecode{polygon\_area\_2x} returns twice the area as the coordinate type - exact for integer points (avoids floating-point entirely). \inlinecode{polygon\_area} always returns \inlinecode{double}.
...
The function is templated on the point type \inlinecode{Pt}. The local \inlinecode{Point} struct (\inlinecode{double} coordinates) is the default; replace it with \inlinecode{Point}/\inlinecode{PointD}/ \inlinecode{PointI} from 7.1.1 or any struct with numeric \inlinecode{.x} and \inlinecode{.y} fields. With integer-coordinate points the cross-product comparisons are exact.

\begin{compactitem}
  \item \inlinecode{point\_in\_polygon(p, lo, hi)} returns whether \inlinecode{p} lies within the polygon given by range \inlinecode{[lo, hi)} in clockwise or counter-clockwise order. If \inlinecode{p} lies on an edge (within \inlinecode{EPS}), the result depends on \inlinecode{EDGE\_IS\_INSIDE}.
\end{compactitem}
...
Given a list of points in two dimensions, computes the convex hull using the monotone chain algorithm and the diametral pair using rotating calipers.

The functions are templated on the iterator type; the value type of the iterator is used as the point type. The local \inlinecode{Point} struct (\inlinecode{double} coordinates) is the default; replace it with \inlinecode{Point}/ \inlinecode{PointD}/\inlinecode{PointI} from 7.1.1 or any struct with numeric \inlinecode{.x} and \inlinecode{.y} fields, \inlinecode{\textless{}}, \inlinecode{==}, and \inlinecode{!=} operators. \inlinecode{convex\_hull()} uses only cross product comparisons and is exact for integer-coordinate points. \inlinecode{diametral\_pair()} uses squared-distance comparisons and is also exact for integer points.

\begin{compactitem}
  \item \inlinecode{convex\_hull(lo, hi)} returns the convex hull in clockwise order. The input range is sorted lexicographically after the call. To produce CCW order, replace every \inlinecode{GE} with \inlinecode{LE}.
...
\begin{compactitem}
  \item \inlinecode{convex\_cut(lo, hi, p, q)} returns the left-side polygon in clockwise order after cutting with the line through \inlinecode{p} and \inlinecode{q}. The function is templated on the iterator type. The local \inlinecode{Point} struct (\inlinecode{double} coordinates) is the default; replace it with \inlinecode{Point}/\inlinecode{PointD}/\inlinecode{PointI} from 7.1.1 or any struct with numeric \inlinecode{.x} and \inlinecode{.y} fields. The \inlinecode{turn} classification uses cross products and is exact for integer-coordinate points; the edge-cut intersection point uses floating-point arithmetic (the output \inlinecode{Point} always has \inlinecode{double} coordinates).
\end{compactitem}
...
This combines the next few subsections in this chapter in a cross-dependent manner. Point, Line, Circle, and all algorithms here operate on floating-point (double) coordinates on a 2D Cartesian plane. For more generic point\textless{}T\textgreater{} and line\textless{}T\textgreater{} supporting integer-coordinate exact arithmetic, see the first few subsections of this chapter.
