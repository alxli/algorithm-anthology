...
\begin{compactitem}
  \item \inlinecode{point\_in\_polygon(p, lo, hi, EDGE\_IS\_INSIDE)} returns whether \inlinecode{p} lies within the polygon with vertices specified by the range [\inlinecode{lo}, \inlinecode{hi}) of points in either clockwise or counter-clockwise order. The \inlinecode{EDGE\_IS\_INSIDE} flag (default \inlinecode{true}) controls whether points exactly on an edge or vertex count as inside.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{minimum\_enclosing\_circle(lo, hi)} returns the minimum enclosing circle for the range [\inlinecode{lo}, \inlinecode{hi}) of points, where \inlinecode{lo} and \inlinecode{hi} must be random-access iterators. The input range is shuffled in place. The point type may be any type exposing numeric \inlinecode{.x} and \inlinecode{.y} members. The returned circle always uses \inlinecode{double} coordinates, so integer-coordinate inputs are accepted.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{convex\_cut(lo, hi, p, q)} returns the portion of the polygon lying on or to the left of the directed line \inlinecode{p} $\to$ \inlinecode{q}. The input range [\inlinecode{lo}, \inlinecode{hi}) must contain the vertices of a convex polygon in boundary order, either clockwise or counterclockwise. The returned polygon preserves that boundary order. The points \inlinecode{p} and \inlinecode{q} must differ.
\end{compactitem}
...
Angles are sorted exactly to satisfy \inlinecode{sort}'s ordering requirements. EPS is then used outside the comparator to group parallel directions and retain only the tightest half-plane in each group.
...
\begin{compactitem}
  \item \inlinecode{intersection\_area(lo1, hi1, lo2, hi2)} returns the intersection area of two polygons respectively specified by two ranges [\inlinecode{lo1}, \inlinecode{hi1}) and [\inlinecode{lo2}, \inlinecode{hi2}) of vertices in clockwise order, where \inlinecode{lo1}, \inlinecode{hi1}, \inlinecode{lo2}, and \inlinecode{hi2} must be random-access iterators.
  \item \inlinecode{union\_area(lo1, hi1, lo2, hi2)} returns the union area of two polygons respectively specified by two ranges [\inlinecode{lo1}, \inlinecode{hi1}) and [\inlinecode{lo2}, \inlinecode{hi2}) of vertices in clockwise order, where \inlinecode{lo1}, \inlinecode{hi1}, \inlinecode{lo2}, and \inlinecode{hi2} must be random-access iterators.
\end{compactitem}
...
// returning $-1$ for no intersection, $0$ for one intersection point, or $1$ for positive length
// overlap.
template<typename Pt>
...
\setcounter{subsection}{0}
\subsection{3D Point}
A 3D point class template supporting vector arithmetic, dot and cross products, distances, angles, unit vectors, normals, and rotation about an axis. Point equality and ordering are exact, while \inlinecode{EQ(p, q)} provides epsilon equality for floating-point coordinates. Integer coordinates are suitable for exact algebraic predicates (\inlinecode{dot}, \inlinecode{cross}, \inlinecode{sqnorm}) as long as intermediate products do not overflow; metric operations preserve floating-point coordinates and otherwise convert them to \inlinecode{double}. For \inlinecode{Modular}, this uses the stored representative and is not finite-field geometry.
