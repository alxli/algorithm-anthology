...
\begin{compactitem}
  \item \inlinecode{closest\_pair(lo, hi, res = nullptr)} returns the minimum squared distance between any two points in the range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$, where \inlinecode{lo} and \inlinecode{hi} must be random-access iterators. The input range is reordered during the computation. If \inlinecode{res} is non-null, one closest pair is stored there in lexicographic order. With fewer than two points, the maximum value of the squared-distance type is returned and \inlinecode{res} is unchanged. The function is templated on the point type and works with any type exposing numeric \inlinecode{.x} and \inlinecode{.y} members and a lexicographic \inlinecode{operator\textless{}}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{find\_intersection(lo, hi, \&res1, \&res2)} returns whether any pair of segments intersect given a range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ of segments, where \inlinecode{lo} and \inlinecode{hi} are random-access iterators. If an intersection is found, then one such pair of segments will be stored into pointers \inlinecode{res1} and \inlinecode{res2}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{intersection\_area(lo1, hi1, lo2, hi2)} returns the intersection area of two polygons respectively specified by two ranges $[{\inlinecodemath{lo1}}, {\inlinecodemath{hi1}})$ and $[{\inlinecodemath{lo2}}, {\inlinecodemath{hi2}})$ of vertices in boundary order, where \inlinecode{lo1}, \inlinecode{hi1}, \inlinecode{lo2}, and \inlinecode{hi2} must be random-access iterators.
  \item \inlinecode{union\_area(lo1, hi1, lo2, hi2)} returns the union area of two polygons respectively specified by two ranges $[{\inlinecodemath{lo1}}, {\inlinecodemath{hi1}})$ and $[{\inlinecodemath{lo2}}, {\inlinecodemath{hi2}})$ of vertices in boundary order, where \inlinecode{lo1}, \inlinecode{hi1}, \inlinecode{lo2}, and \inlinecode{hi2} must be random-access iterators.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{delaunay\_triangulation(p)} sorts \inlinecode{p} lexicographically and removes duplicate points in place (so the input vector is modified), then returns the triangles of one Delaunay triangulation as a vector of counterclockwise-oriented \inlinecode{Point\textless{}T\textgreater{}} triples (\inlinecode{std::tuple}). If fewer than three non-collinear unique points remain, the result is empty.
\end{compactitem}
