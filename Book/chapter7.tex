...
\begin{compactitem}
  \item \inlinecode{delaunay\_triangulation(lo, hi)} returns a Delaunay triangulation for a range [\inlinecode{lo}, \inlinecode{hi}) of distinct points, where \inlinecode{lo} and \inlinecode{hi} must be random-access iterators, or an empty vector if a triangulation does not exist.
\end{compactitem}
