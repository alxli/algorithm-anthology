...
Exact operations (return \inlinecode{TPoint\textless{}T\textgreater{}} or \inlinecode{T}, no precision lost for integers):
...
Floating-point-only operations (return \inlinecode{TPoint\textless{}fp\_t\textgreater{}} or \inlinecode{fp\_t}):
...
\end{lstlisting}
\subsection{Line}
A straight line in two dimensions, templated on the coordinate type \inlinecode{T} (default \inlinecode{double}). The line $ax + by + c = 0$ is stored unnormalized, so a line through integer points $(p_x, p_y)$ and $(q_x, q_y)$ keeps the integer coefficients: $a = q_y - p_y$, $b = p_x - q_x$, $c = -(a \cdot p_x + b \cdot p_y)$. Storage and the exact predicates \inlinecode{contains()}, \inlinecode{is\_parallel()}, and \inlinecode{is\_perpendicular()} therefore stay exact for integral type \inlinecode{T}. The inherently-fractional operations \inlinecode{slope()}, \inlinecode{x()}, and \inlinecode{y()} return \inlinecode{fp\_t}, which is \inlinecode{double} when \inlinecode{T} is integral and \inlinecode{T} itself otherwise.
...
All arithmetic uses the point's own coordinate type, so integer inputs yield an exact triangulation. The lifted-paraboloid test scales as \textasciitilde{}96*C\textasciicircum{}4 where \inlinecode{C} is the coordinate magnitude, so a 64-bit integer coordinate type is safe up to |C| $\leq$ \textasciitilde{}17600. For floating-point coordinates the test is subject to the usual rounding error.

\textbf{Time Complexity}: $O(n^{6})$ per call to \inlinecode{delaunay\_triangulation(p)}, where $n$ is the number of points. The empty-circumcircle test contributes $O(n^{4})$; the remaining factor comes from checking candidate triangles against previously accepted triangles.
...
All arithmetic uses the point's own coordinate type, so integer inputs yield an exact triangulation. The in-circle test is a degree-4 polynomial in the coordinates and dominates the overflow budget: a 64-bit integer coordinate type stays exact up to |C| \textasciitilde{}= 1.4e4, where \inlinecode{C} is the coordinate magnitude. For floating-point coordinates the predicates are subject to the usual rounding error.

\textbf{Time Complexity}: $O(n \log n)$ per call to \inlinecode{delaunay\_triangulation(pts)}, where $n$ is the number of input points.
