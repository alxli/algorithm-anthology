...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \cdot \log\left(R\right) \cdot \log\left(C\right))$ for storage after $n$ point updates.
...
\begin{compactitem}
  \item \inlinecode{RangeTree\textless{}T\textgreater{}(lo, hi)} constructs a set of \inlinecode{std::pair} points from the half-open forward-iterator range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
...
\end{lstlisting}
\subsection{K-d Tree (2D Range Reporting)}
\label{sec:2.6.4}
...
\begin{compactitem}
  \item \inlinecode{RangeKDTree\textless{}T\textgreater{}(lo, hi)} constructs a set of \inlinecode{std::pair} points from the half-open forward-iterator range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
...
\begin{compactitem}
  \item $O(n \log n)$ per call to the constructor, where $n$ is the number of points.
  \item $O(\sqrt{n} + m)$ per call to \inlinecode{query()}, where $m$ is the number of points that are reported by the query.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{NearestKDTree\textless{}T\textgreater{}(lo, hi)} constructs a set of \inlinecode{std::pair} points from the half-open forward-iterator range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
  \item \inlinecode{nearest(x, y, can\_equal = true)} returns a point in the set that is closest to (\inlinecode{x}, \inlinecode{y}) by Euclidean distance. This may be equal to (\inlinecode{x}, \inlinecode{y}) only if \inlinecode{can\_equal} is \inlinecode{true}; at least one eligible point must exist.
\end{compactitem}
