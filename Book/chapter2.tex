...
The dynamic programming state \inlinecode{dp[i][j]} holds the index of the minimum value in the sub-array starting at $i$ and having length $2^j$. Each \inlinecode{dp[i][j]} will always be equal to either \inlinecode{dp[i][j - 1]} or \inlinecode{dp[i + (1 \textless{}\textless{} (j - 1))][j - 1]}, whichever of the indices corresponds to the smaller value in the array.
...
\begin{compactitem}
  \item \inlinecode{initialize(n)} resets the data structure.
