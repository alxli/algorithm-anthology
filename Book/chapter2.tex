...
\begin{compactitem}
  ...
  \item \inlinecode{add\_node(i, value)} adds a new single-node tree to the forest, labeled with the integer \inlinecode{i} and with value initialized to \inlinecode{value}.
...
\begin{compactitem}
  \item \inlinecode{TopTree(n, value)} constructs a forest on vertices $[0, {\inlinecodemath{n}})$ with every vertex value initialized to \inlinecode{value}.
  ...
  \item \inlinecode{link(u, v, value)} adds an edge with value \inlinecode{value} between different trees, returning its edge id or $-1$ if the edge would create a cycle.
