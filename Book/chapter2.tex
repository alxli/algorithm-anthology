...
class SparseSegTree2D {
  ...
  T query_nodes(const Node *n, int qlo, int qhi, int64_t span, const Get &get) const {
    if (n == nullptr) {
      return repeat_value(init, span * length(qlo, qhi));
    }
    int lo = n->lo, hi = n->hi, mid = lo + (hi - lo) / 2;
    std::optional<T> res;
    if (qlo < lo) {
      append_result(res, repeat_value(init, span * length(qlo, std::min(qhi, lo - 1))));
    }
    ...
    if (hi < qhi) {
      append_result(res, repeat_value(init, span * length(std::max(qlo, hi + 1), qhi)));
    }
    return *res;
  }
  ...
  void update_inner(InnerNode *n, int c, int64_t rows, const Apply &apply) {
    ...
    T lval = (n->left != nullptr) ? n->left->value : repeat_value(init, rows * length(lo, mid));
    T rval =
  ...
  void update(OuterNode *n, int r, int c, const T &d) {
    ...
    if (r <= mid) {
      if (n->left == nullptr) {
        n->left = new OuterNode(lo, mid, repeat_value(init, length(lo, mid) * C));
      }
      update(n->left, r, c, d);
    } else {
      if (n->right == nullptr) {
        n->right = new OuterNode(mid + 1, hi, repeat_value(init, length(mid + 1, hi) * C));
      }
 ...
 public:
  explicit SparseSegTree2D(const T &v = T())
      : root(new OuterNode(0, R - 1, repeat_value(v, int64_t{R} * C))), init(v) {}
