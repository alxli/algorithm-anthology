...
\begin{compactitem}
  ...
  \item \inlinecode{splice\_range\_after(pos, before\_first, last)} moves the half-open range (\inlinecode{before\_first}, \inlinecode{last}) so it appears after \inlinecode{pos}.
\end{compactitem}

The splicing helpers use the same ``after'' convention as \inlinecode{std::forward\_list::splice\_after()}. They work naturally with a dummy head node, which makes insertions and removals at the beginning of a list match all other positions. For \inlinecode{splice\_range\_after()}, \inlinecode{pos} must not lie inside the moved range. For doubly linked list splicing, use the next section.
...
\begin{compactitem}
  ...
  \item \inlinecode{query(lo, hi)} returns \inlinecode{combine()} applied to all indices in $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices in $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$.
...
\begin{compactitem}
  ...
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices in $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$. If \inlinecode{lo == hi}, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ satisfies the monotonic predicate \inlinecode{pred()}. It returns \inlinecode{size()} if \inlinecode{pred()} remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ satisfies the monotonic predicate \inlinecode{pred()}. It returns $0$ if \inlinecode{pred()} remains true to the beginning.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{update(lo, hi, d)} modifies the value at each array index in $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$ by applying the delta \inlinecode{d} to each value.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ satisfies the monotonic predicate \inlinecode{pred()}. It returns $N + 1$ if \inlinecode{pred()} remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ satisfies the monotonic predicate \inlinecode{pred()}. It returns $0$ if \inlinecode{pred()} remains true to the beginning.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{query(version, lo, hi)} returns the result of \inlinecode{combine()} applied to all indices in $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$ in the specified version. If \inlinecode{lo == hi}, then the single specified value is returned.
...
\begin{compactitem}
  ...
  \item \inlinecode{root()} returns the root index, or $-1$ if the array is empty.
  \item \inlinecode{parent(i)}, \inlinecode{left(i)}, and \inlinecode{right(i)} return neighboring node indices, or $-1$ if absent.
...
\begin{compactitem}
  ...
  \item $O(1)$ per call to \inlinecode{size()}, \inlinecode{empty()}, \inlinecode{root()}, \inlinecode{parent()}, \inlinecode{left()}, and \inlinecode{right()}.
...
\begin{compactitem}
  \item \inlinecode{SegTree2D\textless{}T\textgreater{}(rows, cols, value = T())} constructs a \inlinecode{rows} by \inlinecode{cols} array with all entries initialized to \inlinecode{value}.
...
\begin{compactitem}
  \item \inlinecode{SparseSegTree2D\textless{}T, R, C\textgreater{}(v = T())} constructs a two-dimensional array with rows $[0, R]$ and columns $[0, C]$. All array values are implicitly initialized to \inlinecode{v}. Nodes are allocated lazily as indices are touched.
...
\begin{compactitem}
  ...
  \item \inlinecode{query(x1, y1, x2, y2, f)} calls the function \inlinecode{f(i, p)} on each point whose $x$-coordinate is in $[{\inlinecodemath{x1}}, {\inlinecodemath{x2}}]$ and whose $y$-coordinate is in $[{\inlinecodemath{y1}}, {\inlinecodemath{y2}}]$. The first argument to \inlinecode{f} is the 0-based index of the point in the original range given to the constructor. The second argument is the point itself as an \inlinecode{std::pair}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{query(x1, y1, x2, y2, f)} calls the function \inlinecode{f(p)} on each point whose $x$-coordinate is in $[{\inlinecodemath{x1}}, {\inlinecodemath{x2}}]$ and whose $y$-coordinate is in $[{\inlinecodemath{y1}}, {\inlinecodemath{y2}}]$. The argument \inlinecode{p} is the point as an \inlinecode{std::pair}.
\end{compactitem}
...
This implementation uses \inlinecode{std::pair} to represent points, requiring \inlinecode{operator\textless{}} and \inlinecode{long double} casting to be defined on the numeric template type.
...
\begin{compactitem}
  ...
  \item \inlinecode{max\_prefix(c)} returns the largest boundary \inlinecode{hi} such that \inlinecode{sum(0, hi - 1)} $\leq$ \inlinecode{c}, assuming prefix sums are nondecreasing and \inlinecode{c} is nonnegative. It may return any boundary in $[0, n]$, where $n$ is \inlinecode{size()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{sum(hi)} returns the sum of all values at indices $[0, {\inlinecodemath{hi}}]$.
  \item \inlinecode{sum(lo, hi)} returns the sum of all values at indices $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$.
  \item \inlinecode{max\_prefix(c)} returns the largest boundary \inlinecode{hi} such that \inlinecode{sum(0, hi - 1)} $\leq$ \inlinecode{c}, assuming prefix sums are nondecreasing and \inlinecode{c} is nonnegative. It may return any boundary in $[0, N]$.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{SparseFenwick2D\textless{}T, R, C\textgreater{}()} constructs a 2D array over rows $[0, R)$ and columns $[0, C)$. All values are implicitly initialized to $0$; nodes are allocated lazily as indices are touched.
  \item \inlinecode{add(r, c, x)} adds \inlinecode{x} to the value at index (\inlinecode{r}, \inlinecode{c}).
...
\begin{compactitem}
  \item \inlinecode{SparseTableLCA(adj)} builds the structure over a forest represented by the bidirectional adjacency list \inlinecode{adj}, whose indices represent the nodes.
  \item \inlinecode{go\_up(u, k)} returns the \inlinecode{k}-th ancestor of node \inlinecode{u}, stopping at that tree's root if \inlinecode{k} is larger than \inlinecode{u}'s depth.
...
\begin{compactitem}
  \item \inlinecode{SegTreeLCA(adj)} builds the structure over a forest represented by the bidirectional adjacency list \inlinecode{adj}, whose indices represent the nodes.
  \item \inlinecode{lca(u, v)} returns the lowest common ancestor of nodes \inlinecode{u} and \inlinecode{v}, or $-1$ if they are in different trees.
  \item \inlinecode{dist(u, v)} returns the number of edges on the path between nodes \inlinecode{u} and \inlinecode{v}, or $-1$ if they are in different trees.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{HeavyLight\textless{}T, VALUES\_ON\_EDGES = true\textgreater{}(adj, value)} constructs a new heavy light decomposition on a forest defined by the adjacency list \inlinecode{adj}, with all values initialized to \inlinecode{value}. Its entries must be valid indices into \inlinecode{adj}, and no duplicate edges should exist. Set \inlinecode{VALUES\_ON\_EDGES} to false to store values on nodes instead.
...
The forest is unrooted: a tree's root is whichever node was most recently established as such. \inlinecode{reroot(u)} sets it explicitly, while \inlinecode{link}, \inlinecode{query}, and \inlinecode{update} reroot implicitly (the latter two at their first argument). \inlinecode{find\_root(u)} and \inlinecode{lca(u, v)} are therefore relative to the current root, so call \inlinecode{reroot(r)} first whenever a specific root \inlinecode{r} is intended.
...
\end{lstlisting}
\subsection{Top Tree}
Maintain a dynamic forest while supporting both path aggregates and rooted-subtree aggregates. Top trees represent each tree as a hierarchy of clusters. A path cluster summarizes a contiguous path, while non-path clusters are raked onto nodes so that changing the preferred path only requires a logarithmic number of local rotations and recomputations.

This is an advanced alternative to link/cut trees. Link/cut trees are usually simpler for dynamic path queries; top trees are useful when the same dynamic forest also needs rooted-subtree queries.

The aggregate operation is defined by an identity element \inlinecode{identity()} and an associative \inlinecode{combine(a, b)}. The default code below computes sums over node and edge values. For minimum queries, use \inlinecode{std::numeric\_limits\textless{}T\textgreater{}::max()} as the identity and \inlinecode{std::min(a, b)} as the combine function. For non-commutative aggregates, store enough information in \inlinecode{T} to support reversing a path, and update \inlinecode{flip\_path()} accordingly.
