...
\begin{compactitem}
  \item $O(n)$ per call to both constructors, where $n$ is the size of the array.
  \item $O(1)$ per call to \inlinecode{size()}.
  \item $O(\log n)$ per call to \inlinecode{at()}, \inlinecode{update()}, \inlinecode{query()}, \inlinecode{find\_first()}, and \inlinecode{find\_last()}.
\end{compactitem}
...
When the stored values are nonnegative frequencies, \inlinecode{kth(k)} answers order-statistic queries by binary lifting: it walks the implicit power-of-two block structure to find the first index whose prefix count reaches \inlinecode{k}. This lets the same structure double as a dynamic multiset over a bounded integer domain, useful for coordinate-compressed kth-element lookups and online rank queries.
...
\begin{compactitem}
  ...
  \item \inlinecode{kth(k)} returns the smallest index \inlinecode{i} such that \inlinecode{sum(i)} $\geq$ \inlinecode{k}, assuming all values are nonnegative. The rank \inlinecode{k} must satisfy \inlinecode{1 \textless{}= k \textless{}= sum(size() - 1)}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(1)$ per call to \inlinecode{size()}.
  \item $O(\log n)$ per call to \inlinecode{add()}, both \inlinecode{sum()} functions, \inlinecode{set()}, \inlinecode{at()}, and \inlinecode{kth()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{SparseTableLCA(adj)} builds the structure over a forest represented by a bidirectional adjacency list of nodes numbered from 0 to \inlinecode{adj.size() - 1}.
...
\begin{compactitem}
  ...
  \item $O(\log n)$ per call to \inlinecode{go\_up()} or \inlinecode{lca()}.
...
\begin{compactitem}
  ...
  \item $O(1)$ auxiliary for \inlinecode{go\_up()}, \inlinecode{lca()}, and \inlinecode{is\_ancestor()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{SegTreeLCA(adj)} builds the structure over a forest represented by a bidirectional adjacency list of nodes numbered from 0 to \inlinecode{adj.size() - 1}.
  \item \inlinecode{lca(u, v)} returns the lowest common ancestor of nodes \inlinecode{u} and \inlinecode{v}, or $-1$ if they are in different trees.
\end{compactitem}
