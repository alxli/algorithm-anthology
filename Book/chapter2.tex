...
Maintain a min-priority queue, that is, a collection of elements with support for querying and extraction of the minimum. This implementation requires an ordering on the set of possible elements defined by the < operator. A binary min-heap implements a priority queue by inserting and deleting nodes into a binary tree such that the parent of any node is always less than its children.

\begin{compactitem}
  \item binary\_heap() constructs an empty priority queue.
  \item binary\_heap(lo, hi) constructs a priority queue from two ForwardIterators, consisting of elements in the range [lo, hi).
  \item size() returns the size of the priority queue.
  \item empty() returns whether the priority queue is empty.
  \item push(v) inserts the value v into the priority queue.
  \item pop() removes the minimum element from the priority queue.
  \item top() returns the minimum element in the priority queue.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the first constructor, size(), empty(), and top().
  \item $O(\log n)$ per call to push() and pop(), where n is the number of elements in the priority queue.
  \item $O(n)$ per call to the second constructor on the distance between lo and hi.
\end{compactitem}
...
Maintain a mergeable min-priority queue, that is, a collection of elements with support for querying and extraction of the minimum as well as efficient merging with other instances. This implementation requires an ordering on the set of possible elements defined by the < operator. A randomized mergeable heap uses a coin-toss to recursively combine nodes of two binary trees.

\begin{compactitem}
  \item randomized\_heap() constructs an empty priority queue.
  \item randomized\_heap(lo, hi) constructs a priority queue from two ForwardIterators, consisting of elements in the range [lo, hi).
  \item size() returns the size of the priority queue.
  \item empty() returns whether the priority queue is empty.
  \item push(v) inserts the value v into the priority queue.
  \item pop() removes the minimum element from the priority queue.
  \item top() returns the minimum element in the priority queue.
  \item absorb(h) inserts every value from h and sets h to the empty priority queue.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the first constructor, size(), empty(), and top().
  \item $O(\log n)$ expected worst case per call to push(), pop(), and absorb(), where n is the number of elements in the priority queue.
  \item $O(n)$ per call to the second constructor on the distance between lo and hi.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ for storage of the priority queue elements.
  \item $O(\log n)$ auxiliary stack space for push(), pop(), and absorb().
...
Maintain a mergeable min-priority queue, that is, a collection of elements with support for querying and extraction of the minimum as well as efficient merging with other instances. This implementation requires an ordering on the set of possible elements defined by the < operator. A skew heap attempts to maintain balance by unconditionally swapping all nodes in the merge path when merging.

\begin{compactitem}
  \item skew\_heap() constructs an empty priority queue.
  \item skew\_heap(lo, hi) constructs a priority queue from two ForwardIterators, consisting of elements in the range [lo, hi).
  \item size() returns the size of the priority queue.
  \item empty() returns whether the priority queue is empty.
  \item push(v) inserts the value v into the priority queue.
  \item pop() removes the minimum element from the priority queue.
...
\begin{compactitem}
  \item $O(1)$ per call to the first constructor, size(), empty(), and top().
  \item $O(\log n)$ amortized auxiliary per call to push(), pop(), and absorb(), where n is the number of elements in the priority queue.
  \item $O(n)$ per call to the second constructor, where n is the distance between lo and hi.
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the priority queue elements.
  \item $O(\log n)$ amortized auxiliary stack space for push(), pop(), and absorb().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
Maintain a mergeable min-priority queue, that is, a collection of elements with support for querying and extraction of the minimum as well as efficient merging with other instances. This implementation requires an ordering on the set of possible elements defined by the < operator. A pairing heap is a heap-ordered multi-way tree, using a two-pass merge to self-adjust during each deletion.

\begin{compactitem}
  \item pairing\_heap() constructs an empty priority queue.
  \item pairing\_heap(lo, hi) constructs a priority queue from two ForwardIterators, consisting of elements in the range [lo, hi).
  \item size() returns the size of the priority queue.
  \item empty() returns whether the priority queue is empty.
  \item push(v) inserts the value v into the priority queue.
  \item pop() removes the minimum element from the priority queue.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the first constructor, size(), empty(), top(), push(), and absorb().
  \item $O(\log n)$ amortized per call to pop().
  \item $O(n)$ per call to the second constructor on the distance between lo and hi.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ for storage of the priority queue elements.
  \item $O(\log n)$ amortized auxiliary stack space for pop().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
Maintain a map, that is, a collection of key-value pairs such that each possible key appears at most once in the collection. This implementations requires an ordering on the set of possible keys defined by the < operator on the key type. A binary search tree implements this map by inserting and deleting keys into a binary tree such that every node's left child has a lesser key and every node's right child has a greater key.

\begin{compactitem}
  \item binary\_search\_tree() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(k, v) adds an entry with key k and value v to the map, returning true if an new entry was added or false if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(k) removes the entry with key k from the map, returning true if the removal was successful or false if the key to be removed was not found.
  \item find(k) returns a pointer to a const value associated with key k, or NULL if the key was not found.
  \item walk(f) calls the function f(k, v) on each entry of the map, in ascending order of keys.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the constructor, size(), and empty().
  \item $O(n)$ per call to insert(), erase(), find(), and walk(), where n is the number of nodes currently in the map.
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the map elements.
  \item $O(n)$ auxiliary stack space for insert(), erase(), and walk().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\begin{compactitem}
  \item treap() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(k, v) adds an entry with key k and value v to the map, returning true if an new entry was added or false if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(k) removes the entry with key k from the map, returning true if the removal was successful or false if the key to be removed was not found.
...
\begin{compactitem}
  \item $O(1)$ per call to the constructor, size(), and empty().
  \item $O(\log n)$ on average per call to insert(), erase(), and find(), where n is the number of entries currently in the map.
  \item $O(n)$ per call to walk().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the map elements.
  \item $O(\log n)$ auxiliary stack space on average for insert(), erase(), and walk().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\begin{compactitem}
  \item avl\_tree() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(k, v) adds an entry with key k and value v to the map, returning true if an new entry was added or false if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(k) removes the entry with key k from the map, returning true if the removal was successful or false if the key to be removed was not found.
...
\begin{compactitem}
  \item $O(1)$ per call to the constructor, size(), and empty().
  \item $O(\log n)$ per call to insert(), erase(), and find(), where n is the number of entries currently in the map.
  \item $O(n)$ per call to walk().
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ for storage of the map elements.
  \item $O(\log n)$ auxiliary stack space for insert(), erase(), and walk().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\begin{compactitem}
  \item red\_black\_tree() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(k, v) adds an entry with key k and value v to the map, returning true if an new entry was added or false if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(k) removes the entry with key k from the map, returning true if the removal was successful or false if the key to be removed was not found.
...
\begin{compactitem}
  \item $O(n)$ for storage of the map elements.
  \item $O(\log n)$ auxiliary stack space for walk().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\begin{compactitem}
  \item splay\_tree() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(k, v) adds an entry with key k and value v to the map, returning true if an new entry was added or false if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(k) removes the entry with key k from the map, returning true if the removal was successful or false if the key to be removed was not found.
...
\begin{compactitem}
  \item size\_balanced\_tree() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(k, v) adds an entry with key k and value v to the map, returning true if an new entry was added or false if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(k) removes the entry with key k from the map, returning true if the removal was successful or false if the key to be removed was not found.
  \item find(k) returns a pointer to a const value associated with key k, or NULL if the key was not found.
  \item select(r) returns a key-value pair of the node with a key of zero-based rank r in the map, throwing an exception if the rank is not between 0 and size() - 1.
  \item rank(k) returns the zero-based rank of key k in the map, throwing an exception if the key was not found in the map.
  \item walk(f) calls the function f(k, v) on each entry of the map, in ascending order of keys.
\end{compactitem}
...
\begin{compactitem}
  \item $O(1)$ per call to the constructor, size(), and empty().
  \item $O(\log n)$ per call to insert(), erase(), find(), select(), and rank(), where n is the number of entries currently in the map.
  \item $O(n)$ per call to walk().
\end{compactitem}
...
\begin{compactitem}
  \item interval\_treap() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(lo, hi, v) adds an entry with key [lo, hi] and value v to the map, returning true if a new interval was added or false if the interval already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(lo, hi) removes the entry with key [lo, hi] from the map, returning true if the removal was successful or false if the interval was not found.
  \item find\_key(lo, hi) returns a pointer to a const std::pair representing the key of some interval in the map which intersects with [lo, hi], or NULL if no such entry was found.
  \item find\_value(lo, hi) returns a pointer to a const value of some entry in the map with a key that intersects with [lo, hi], or NULL if no such entry was found.
  \item find\_all(lo, hi, f) calls the function f(lo, hi, v) on each entry in the map that overlaps with [lo, hi], in lexicographically ascending order of intervals.
  \item walk(f) calls the function f(lo, hi, v) on each interval in the map, in lexicographically ascending order of intervals.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the constructor, size(), and empty().
  \item $O(\log n)$ on average per call to insert(), erase(), and find\_any(), where n is the number of intervals currently in the set.
  \item $O(\log n + m)$ on average per call to find\_all(), where m is the number of intersecting intervals that are reported.
  \item $O(n)$ per call to walk().
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ for storage of the map elements.
  \item $O(1)$ auxiliary for size() and empty().
...
\begin{compactitem}
  \item hash\_map() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(k, v) adds an entry with key k and value v to the map, returning true if an new entry was added or false if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(k) removes the entry with key k from the map, returning true if the removal was successful or false if the key to be removed was not found.
  \item find(k) returns a pointer to a const value associated with key k, or NULL if the key was not found.
  \item operator[k] returns a reference to key k's associated value (which may be modified), or if necessary, inserts and returns a new entry with the default constructed value if key k was not originally found.
  \item walk(f) calls the function f(k, v) on each entry of the map, in no guaranteed order.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the constructor, size(), and empty().
  \item $O(1)$ amortized per call to insert(), erase(), find(), and operator[].
  \item $O(n)$ per call to walk(), where n is the number of entries in the map.
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the map elements.
  \item $O(n)$ auxiliary heap space for insert().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
Maintain a map, that is, a collection of key-value pairs such that each possible key appears at most once in the collection. This implementation requires both the < and the == operators to be defined on the key type. A skip list maintains a linked hierarchy of sorted subsequences with each successive subsequence skipping over fewer elements than the previous one.

\begin{compactitem}
  \item skip\_list() constructs an empty map.
  \item size() returns the size of the map.
  \item empty() returns whether the map is empty.
  \item insert(k, v) adds an entry with key k and value v to the map, returning true if an new entry was added or false if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item erase(k) removes the entry with key k from the map, returning true if the removal was successful or false if the key to be removed was not found.
...
\begin{compactitem}
  \item $O(1)$ per call to the constructor, size(), and empty().
  \item $O(\log n)$ on average per call to insert(), erase(), find(), and operator[], where n is the number of entries currently in the map.
  \item $O(n)$ per call to walk().
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary heap space for insert() and erase().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
Given a static array with indices from 0 to n - 1, precompute a table that may later be used perform range minimum queries on the array in constant time. This version is simplified to only work on integer arrays.

The dynamic programming state dp[i][j] holds the index of the minimum value in the sub-array starting at i and having length 2\textasciicircum{}j. Each dp[i][j] will always be equal to either dp[i][j - 1] or dp[i + 2\textasciicircum{}(j - 1) - 1)][j - 1], whichever of the indices corresponds to the smaller value in the array.

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n \log n)$ per call to build(), where n is the size of the array.
  \item $O(1)$ per call to query\_min().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \log n)$ for storage of the sparse table, where n is the size of the array.
  \item $O(1)$ auxiliary for query().
\end{compactitem}
...
The query operation is defined by an associative join\_values() function which satisfies join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z) for all values x, y, and z in the array. The default definition below assumes a numerical array type, supporting queries for the "min" of the target range. Another possible query operation is "sum", in which the join\_values() function should be defined to return "a + b".

The update operation is defined by the join\_value\_with\_delta() function which determines the change made to array values. The default definition below supports updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which join\_value\_with\_delta(v, d) should be defined to return "v + d".
...
\begin{compactitem}
  ...
  \item $O(1)$ per call to size().
  \item $O(\sqrt{n})$ per call to at(), update(), and query().
\end{compactitem}
...
The query operation is defined by an associative join\_values() function which satisfies join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z) for all values x, y, and z in the array. The default code below assumes a numerical array type, defining queries for the "min" of the target range. Another possible query operation is "sum", in which case the join\_values() function should be defined to return "a + b".

The update operation is defined by the join\_value\_with\_delta() function, which determines the change made to array values. The default definition below supports updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which join\_value\_with\_delta(v, d) should be defined to return "v + d".

\begin{compactitem}
  \item segment\_tree(n, v) constructs an array of size n with indices from 0 to n - 1, inclusive, and all values initialized to v.
  \item segment\_tree(lo, hi) constructs an array from two random-access iterators as a range [lo, hi), initialized to the elements of the range in the same order.
  \item size() returns the size of the array.
  \item at(i) returns the value at index i.
  \item query(lo, hi) returns the result of join\_values() applied to all indices from lo to hi, inclusive. If the distance between lo and hi is 1, then the single specified value is returned.
  \item update(i, d) assigns the value v at index i to join\_value\_with\_delta(v, d).
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ per call to both constructors, where n is the size of the array.
  \item $O(1)$ per call to size().
  \item $O(\log n)$ per call to at(), update(), and query().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the array elements.
  \item $O(\log n)$ auxiliary stack space for update() and query().
  \item $O(1)$ auxiliary for size().
\end{compactitem}
...
The update operation is defined by the join\_value\_with\_delta() and join\_deltas() functions, which determines the change made to array values. These must satisfy:

\begin{compactitem}
  \item join\_deltas(d1, join\_deltas(d2, d3)) = join\_deltas(join\_deltas(d1, d2), d3).
  \item join\_value\_with\_delta(join\_values(v, ...(m times)..., v), d, m)) should be equal to join\_values(join\_value\_with\_delta(v, d, 1), ...(m times)).
  \item if a sequence d\_1, ..., d\_m of deltas is used to update a value v, then join\_value\_with\_delta(v, join\_deltas(d\_1, ..., d\_m), 1) should be equivalent to m sequential calls to join\_value\_with\_delta(v, d\_i, 1) for i = 1..m. The default code below defines updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which case join\_value\_with\_delta(v, d, len) should be defined to return "v + d*len" and join\_deltas(d1, d2) should be defined to return "d1 + d2".
\end{compactitem}
...
\begin{compactitem}
  ...
  \item segment\_tree(lo, hi) constructs an array from two random-access iterators as a range [lo, hi), initialized to the elements of the range in the same order.
  \item size() returns the size of the array.
  \item at(i) returns the value at index i, where i is between 0 and size() - 1.
  \item query(lo, hi) returns the result of join\_values() applied to all indices from lo to hi, inclusive. If the distance between lo and hi is 1, then the single specified value is returned.
  \item update(i, d) assigns the value v at index i to join\_value\_with\_delta(v, d).
  \item update(lo, hi, d) modifies the value at each array index from lo to hi, inclusive, by respectively joining them with d using join\_value\_with\_delta().
\end{compactitem}
...
This data structure shares every operation of one-dimensional segment trees in this section, with the additional operations empty(), insert(), erase(), push\_back(), and pop\_back() analogous to those of std::vector (here, insert() and erase() both take an index instead of an iterator).

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to both constructors, where n is the size of the array.
  \item $O(1)$ per call to size() and empty().
...
The query operation is defined by the join\_values() and join\_region() functions where join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z) for all values x, y, and z in the array. The join\_region(v, area) function must be defined in conjunction to efficiently return the result of join\_values() applied to a rectangular sub-array of area elements. The default code below assumes a numerical array type, defining queries for the "min" of the target range. Another possible query operation is "sum", in which case join\_values(a, b) should return "a + b" and join\_region(v, area) should return "v*area".

The update operation is defined by the join\_value\_with\_delta() function, which determines the change made to array values. The default code below defines updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which join\_value\_with\_delta(v, d) should be defined to return "v + d".

\begin{compactitem}
  \item quadtree(v) constructs a two-dimensional array with rows from 0 to MAXR and columns from 0 to MAXC, inclusive. All values are implicitly initialized to v.
  \item at(r, c) returns the value at row r, column c.
  \item query(r1, c1, r2, c2) returns the result of join\_values() applied to every value in the rectangular region consisting of rows from r1 to r2, inclusive, and columns from c1 to c2, inclusive.
  \item update(r, c, d) assigns the value v at (r, c) to join\_value\_with\_delta(v, d).
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the constructor.
  \item $O(\max\left(\text{MAXR}, \text{MAXC}\right))$ per call to at(), update(), and query().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the array elements, where n is the number of updated entries in the array.
  \item $O(\sqrt{\max\left(\text{MAXR}, \text{MAXC}\right)})$ auxiliary stack space for update(), query(), and at().
\end{compactitem}
...
\begin{compactitem}
  \item join\_deltas(d1, join\_deltas(d2, d3)) = join\_deltas(join\_deltas(d1, d2), d3).
  \item join\_value\_with\_delta(join\_values(v, ...(m times)..., v), d, m)) should be equal to join\_values(join\_value\_with\_delta(v, d, 1), ...(m times)).
  \item if a sequence d\_1, ..., d\_m of deltas is used to update a value v, then join\_value\_with\_delta(v, join\_deltas(d\_1, ..., d\_m), 1) should be equivalent to m sequential calls to join\_value\_with\_delta(v, d\_i, 1) for i = 1..m. The default code below defines updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which case join\_value\_with\_delta(v, d, area) should be defined to return "v + d*area" and join\_deltas(d1, d2) should be defined to return "d1 + d2".
\end{compactitem}

\begin{compactitem}
  \item quadtree(v) constructs a two-dimensional array with rows from 0 to MAXR and columns from 0 to MAXC, inclusive. All values are implicitly initialized to v.
  \item at(r, c) returns the value at row r, column c.
  \item query(r1, c1, r2, c2) returns the result of join\_values() applied to every value in the rectangular region consisting of rows from r1 to r2 and columns from c1 to c2, inclusive.
  \item update(r, c, d) assigns the value v at (r, c) to join\_value\_with\_delta(v, d).
  \item update(r1, c1, r2, c2) modifies the value at each index of the rectangular region consisting of rows from r1 to r2 and columns from c1 to c2, inclusive, by respectively joining them with d using join\_value\_with\_delta().
\end{compactitem}
...
\begin{compactitem}
  \item segment\_tree\_2d(v) constructs a two-dimensional array with rows from 0 to MAXR and columns from 0 to MAXC, inclusive. All values are implicitly initialized to v.
  \item at(r, c) returns the value at row r, column c.
  \item query(r1, c1, r2, c2) returns the result of join\_values() applied to every value in the rectangular region consisting of rows from r1 to r2, inclusive, and columns from c1 to c2, inclusive.
  \item update(r, c, d) assigns the value v at (r, c) to join\_value\_with\_delta(v, d).
\end{compactitem}
...
\begin{compactitem}
  \item $O(1)$ per call to the constructor.
  \item $O(\log\left(\text{MAXR}\right) \cdot \log\left(\text{MAXC}\right))$ per call to at(), update(), and query().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the array elements, where n is the number of updated entries in the array.
  \item $O(\log\left(\text{MAXR}\right) + \log\left(\text{MAXC}\right))$ auxiliary stack space for update(), query(), and at().
\end{compactitem}
...
\begin{compactitem}
  \item range\_tree(lo, hi) constructs a set from two random-access iterators to std::pair as a range [lo, hi) of points.
  \item query(x1, y1, x2, y2, f) calls the function f(i, p) on each point in the set that falls into the rectangular region consisting of rows from x1 to x2, inclusive, and columns from y1 to y2, inclusive. The first argument to f is the zero-based index of the point in the original range given to the constructor. The second argument is the point itself as an std::pair.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n \log n)$ per call to the constructor, where n is the number of points.
  \item $O(\log^{2}(n) + m)$ per call to query(), where m is the number of points that are reported by the query.
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \log n)$ for storage of the points.
  \item $O(\log^{2}(n))$ auxiliary stack space for query().
\end{compactitem}
...
\begin{compactitem}
  \item kd\_tree(lo, hi) constructs a set from two random-access iterators to std::pair as a range [lo, hi) of points.
  \item query(x1, y1, x2, y2, f) calls the function f(i, p) on each point in the set that falls into the rectangular region consisting of rows from x1 to x2, inclusive, and columns from y1 to y2, inclusive. The first argument to f is the zero-based index of the point in the original range given to the constructor. The second argument is the point itself as an std::pair.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n \log n)$ per call to the constructor, where n is the number of points.
  \item $O(\log\left(n\right) + m)$ on average per call to query(), where m is the number of points that are reported by the query.
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the points.
  \item $O(\log n)$ auxiliary stack space for query().
\end{compactitem}
...
\begin{compactitem}
  \item kd\_tree(lo, hi) constructs a set from two random-access iterators to std::pair as a range [lo, hi) of points.
  \item nearest(x, y, can\_equal) returns a point in the set that is closest to (x, y) by Euclidean distance. This may be equal to (x, y) only if can\_equal is true.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n \log n)$ per call to the constructor, where n is the number of points.
  \item $O(\log n)$ on average per call to nearest().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the points.
  \item $O(\log n)$ auxiliary stack space for nearest().
\end{compactitem}
...
\begin{compactitem}
  \item r\_tree(lo, hi) constructs a set from two random-access iterators as a range [lo, hi) of segments.
  \item nearest(x, y) returns a segment in the set that contains some point which is as close or closer to (x, y) by Euclidean distance than any point on any other segment in the set.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n \log n)$ per call to the constructor, where n is the number of segments.
  \item $O(\log n)$ on average per call to nearest().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the segments.
  \item $O(\log n)$ auxiliary stack space for nearest().
\end{compactitem}
...
\begin{compactitem}
  \item initialize() resets the data structure.
  \item a[i] stores the value at index i.
  \item add(i, x) adds x to the value at index i.
  \item set(i, x) assigns the value at index i to x.
  \item sum(hi) returns the sum of all values at indices from 1 to hi, inclusive.
  \item sum(lo, hi) returns the sum of all values at indices from lo to hi, inclusive.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to initialize(), where n is the size of the array.
...
\begin{compactitem}
  ...
  \item at(i) returns the value at index i.
  \item add(i, x) adds x to the value at index i.
  \item add(lo, hi, x) adds x to the values at all indices from lo to hi, inclusive.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(1)$ per call to size().
  \item $O(\log n)$ per call to at() and both add() functions.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item add(i, x) adds x to the value at index i.
  \item set(i, x) assigns the value at index i to x.
  \item sum(hi) returns the sum of all values at indices from 0 to hi, inclusive.
  \item sum(lo, hi) returns the sum of all values at indices from lo to hi, inclusive.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ per call to the constructor, where n is the size of the array.
  \item $O(1)$ per call to size() and at().
  \item $O(\log n)$ per call to add(), set(), and both sum() functions.
\end{compactitem}
...
\begin{compactitem}
  \item size() returns the size of the array.
  \item at(i) returns the value at index i.
  \item add(i, x) increments the value at index i by x.
  \item add(lo, hi, x) adds x to the values at all indices from lo to hi, inclusive.
  \item set(i, x) assigns the value at index i to x.
  \item sum(hi) returns the sum of all values at indices from 0 to hi, inclusive.
  \item sum(lo, hi) returns the sum of all values at indices from lo to hi, inclusive.
...
\textbf{Time Complexity}: $O(\log^{2} MAXN)$ per call to all member functions. If std::map is replaced with std::unordered\_map, then the amortized running time will become $O(\log MAXN)$.

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \log MAXN)$ for storage of the array elements, where n is the number of distinct indices that have been accessed across all of the operations so far.
  \item $O(1)$ auxiliary for all operations.
\end{compactitem}
...
\begin{compactitem}
  \item initialize() resets the data structure.
  \item a[r][c] stores the value at index (r, c).
  \item add(r, c, x) adds x to the value at index (r, c).
  \item set(r, c, x) assigns x to the value at index (r, c).
  \item sum(r, c) returns the sum of the rectangle with upper-left corner (1, 1) and lower-right corner (r, c).
  \item sum(r1, c1, r2, c2) returns the sum of the rectangle with upper-left corner (r1, c1) and lower-right corner (r2, c2).
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n \cdot m)$ per call to initialize(), where n is the number of rows and m is the number of columns.
  \item $O(\log\left(n\right) \cdot \log\left(m\right))$ per call to all other operations.
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \cdot m)$ for storage of the array elements.
  \item $O(1)$ auxiliary for all operations.
\end{compactitem}
...
\begin{compactitem}
  \item add(r, c, x) adds x to the value at index (r, c).
  \item add(r1, c1, r2, c2, x) adds x to all indices in the rectangle with upper-left corner (r1, c1) and lower-right corner (r2, c2).
  \item set(r, c, x) assigns x to the value at index (r, c).
  \item sum(r, c) returns the sum of the rectangle with upper-left corner (0, 0) and lower-right corner (r, c).
  \item sum(r1, c1, r2, c2) returns the sum of the rectangle with upper-left corner (r1, c1) and lower-right corner (r2, c2).
  \item at(r, c) returns the value at index (r, c).
\end{compactitem}

\textbf{Time Complexity}: $O(\log^{2}(\text{MAXR}) \cdot \log^{2}(\text{MAXC}))$ per call to all member functions. If std::map is replaced with std::unordered\_map, then the amortized running time will become $O(\log\left(\text{MAXR}\right) \cdot \log\left(\text{MAXC}\right))$.

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \cdot \log\left(\text{MAXR}\right) \cdot \log\left(\text{MAXC}\right))$ for storage of the array elements, where n is the number of distinct indices that have been accessed across all of the operations so far.
  \item $O(1)$ auxiliary for all operations.
\end{compactitem}
...
\begin{compactitem}
  \item initialize() resets the data structure.
  \item make\_set(u) creates a new partition consisting of the single element u, which must not have been previously added to the data structure.
  \item find\_root(u) returns the unique representative of the partition containing u.
  \item is\_united(u, v) returns whether elements u and v belong to the same partition.
  \item unite(u, v) replaces the partitions containing u and v with a single new partition consisting of the union of elements in the original partitions.
\end{compactitem}

A precondition to the last three operations is that make\_set() must have been previously called on their arguments.

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to initialize() and make\_set().
  \item $O(a(n))$ per call to find\_root(), is\_united(), and unite(), where n is the number of elements that has been added via make\_set() so far, and a(n) is the extremely slow growing inverse of the Ackermann function (effectively a very small constant for all practical values of n).
\end{compactitem}
...
\begin{compactitem}
  ...
  \item is\_united(u, v) returns whether elements u and v belong to the same partition.
  \item unite(u, v) replaces the partitions containing u and v with a single new partition consisting of the union of elements in the original partitions.
  \item get\_all\_sets() returns all current partitions as a vector of vectors.
\end{compactitem}
...
\begin{compactitem}
  \item $O(1)$ per call to the constructor.
  \item $O(\log n)$ per call to make\_set(), where n is the number of elements that have been added via make\_set() so far.
  \item $O(a(n) \log n)$ per call to is\_united() and unite(), where n is the number of elements that have been added via make\_set() so far, and a(n) is the extremely slow growing inverse of the Ackermann function (effectively a very small constant for all practical values of n).
  \item $O(n)$ per call to get\_all\_sets().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the disjoint set forest elements.
  \item $O(n)$ auxiliary heap space for get\_all\_sets().
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n \log n)$ per call to build(), where n is the number of nodes.
  \item $O(\log n)$ per call to lca().
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \log n)$ to store the sparse table, where n is the number of nodes.
  \item $O(n)$ auxiliary stack space for build().
  \item $O(1)$ auxiliary for lca().
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(n \log n)$ auxiliary stack space for build().
  \item $O(\log n)$ auxiliary stack space for lca().
\end{compactitem}
...
The query operation is defined by an associative join\_values() function which satisfies join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z) for all values x, y, and z in the tree. The default code below assumes a numerical tree type, defining queries for the "min" of the target range. Another possible query operation is "sum", in which case the join\_values() function should be defined to return "a + b".

The update operation is defined by the join\_value\_with\_delta() and join\_deltas() functions, which determines the change made to values. These must satisfy:

\begin{compactitem}
  \item join\_deltas(d1, join\_deltas(d2, d3)) = join\_deltas(join\_deltas(d1, d2), d3).
  \item join\_value\_with\_delta(join\_values(v, ...(m times)..., v), d, m)) should be equal to join\_values(join\_value\_with\_delta(v, d, 1), ...(m times)).
  \item if a sequence d\_1, ..., d\_m of deltas is used to update a value v, then join\_value\_with\_delta(v, join\_deltas(d\_1, ..., d\_m), 1) should be equivalent to m sequential calls to join\_value\_with\_delta(v, d\_i, 1) for i = 1..m. The default code below defines updates that "set" a path's edges or nodes to a new value. Another possible update operation is "increment", in which case join\_value\_with\_delta(v, d, len) should be defined to return "v + d*len" and join\_deltas(d1, d2) should be defined to return "d1 + d2".
\end{compactitem}

\begin{compactitem}
  \item heavy\_light(n, adj[], v) constructs a new heavy light decomposition on a tree with n nodes defined by the adjacency list adj[], with all values initialized to v. The adjacency list must be a size n array of vectors consisting of only the integers from 0 to n - 1, inclusive. No duplicate edges should exist, and the graph must be connected.
  \item query(u, v) returns the result of join\_values() applied to all values on the path from node u to node v.
  \item update(u, v, d) modifies all values on the path from node u to node v by respectively joining them with d using join\_value\_with\_delta().
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to the constructor, where n is the number of nodes.
  \item $O(\log n)$ per call to query() and update();
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(1)$ auxiliary for query() and update().
\end{compactitem}
...
The query operation is defined by an associative join\_values() function which satisfies join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z) for all values x, y, and z in the forest. The default code below assumes a numerical forest type, defining queries for the "min" of the target range. Another possible query operation is "sum", in which case the join\_values() function should be defined to return "a + b".
...
\begin{compactitem}
  \item join\_deltas(d1, join\_deltas(d2, d3)) = join\_deltas(join\_deltas(d1, d2), d3).
  \item join\_value\_with\_delta(join\_values(v, ...(m times)..., v), d, m)) should be equal to join\_values(join\_value\_with\_delta(v, d, 1), ...(m times)).
  \item if a sequence d\_1, ..., d\_m of deltas is used to update a value v, then join\_value\_with\_delta(v, join\_deltas(d\_1, ..., d\_m), 1) should be equivalent to m sequential calls to join\_value\_with\_delta(v, d\_i, 1) for i = 1..m. The default code below defines updates that "set" a path's nodes to a new value. Another possible update operation is "increment", in which case join\_value\_with\_delta(v, d, len) should be defined to return "v + d*len" and join\_deltas(d1, d2) should be defined to return "d1 + d2".
\end{compactitem}

\begin{compactitem}
  \item link\_cut\_forest() constructs an empty forest with no trees.
  \item size() returns the number of nodes in the forest.
  \item trees() returns the number of trees in the forest.
  \item make\_root(i, v) creates a new tree in the forest consisting of a single node labeled with the integer i and value initialized to v.
  \item is\_connected(a, b) returns whether nodes a and b are connected.
  \item link(a, b) adds an edge between the nodes a and b, both of which must exist and not be connected.
  \item cut(a, b) removes the edge between the nodes a and b, both of which must exist and be connected.
  \item query(a, b) returns the result of join\_values() applied to all values on the path from the node a to node b.
  \item update(a, b, d) modifies all the values on the path from node a to node b by respectively joining them with d using join\_value\_with\_delta().
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the constructor, size(), and trees().
