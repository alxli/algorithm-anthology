...
Since \inlinecode{morris\_inorder()} mutates the tree as it runs, it is unsafe on a tree shared between threads and leaves threads behind if an exception escapes. Computing over a tree rather than listing it is dynamic programming on a tree; see sections \secref{4.1.4} through \secref{4.1.7} for general rooted trees.

\textbf{Time Complexity}: $O(n)$ per call to every traversal and to \inlinecode{flatten()}, where $n$ is the number of nodes.

\Needspace*{3\baselineskip}
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for each returned vector.
  \item $O(h)$ auxiliary for the stack-based and queue-based traversals, where $h$ is the height of the tree, which is $O(n)$ in the worst case.
  \item $O(1)$ auxiliary for \inlinecode{morris\_inorder()} and \inlinecode{flatten()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{serialize(root)} returns the tree as a whitespace-separated preorder string, writing \inlinecode{\#} for each null child. The value type must support stream output.
  \item \inlinecode{deserialize(s)} rebuilds the tree that \inlinecode{serialize()} produced. The value type must support stream input. Its returned nodes are allocated with \inlinecode{new} and have the same ownership convention as the construction routines above.
\end{compactitem}
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ expected per call to both construction routines, where n is the number of nodes, from the hash lookups of inorder positions.
  \item $O(n)$ per call to \inlinecode{serialize()} and \inlinecode{deserialize()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{lca(root, a, b)} returns the lowest node having both \inlinecode{a} and \inlinecode{b} in its subtree, counting a node as its own ancestor. Both nodes must belong to the tree; if only one does, that one is returned.
  \item \inlinecode{distance(root, a, b)} returns the number of edges on the path between the two nodes, which is the depth of each below their lowest common ancestor.
\end{compactitem}
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to every operation except \inlinecode{count\_paths\_with\_sum()}, where $n$ is the number of nodes.
