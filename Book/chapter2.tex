...
\begin{compactitem}
  ...
  \item \inlinecode{pop()} removes the minimum element from the priority queue.
  \item \inlinecode{top()} returns the minimum element in the priority queue.
  \item \inlinecode{absorb(h)} inserts every value from the distinct heap \inlinecode{h} and sets \inlinecode{h} to the empty priority queue.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{top()} returns the minimum element in the priority queue.
  \item \inlinecode{absorb(h)} inserts every value from the distinct heap \inlinecode{h} and sets \inlinecode{h} to the empty priority queue.
  \item \inlinecode{decrease\_key(n, v)} lowers the value of the element at handle \inlinecode{n} to \inlinecode{v}, which must not be greater than its current value.
...
This implementation requires an ordering on the key type \inlinecode{K} defined by \inlinecode{operator\textless{}}. The sentinel node also requires \inlinecode{K} and \inlinecode{V} to be default constructible.
...
\begin{compactitem}
  \item \inlinecode{ChainingHashMap\textless{}K, V, Hash\textgreater{}(buckets = 128)} constructs an empty map with the positive number of buckets given by \inlinecode{buckets}.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if a new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  \item \inlinecode{ProbingHashMap\textless{}K, V, Hash\textgreater{}(buckets = 128)} constructs an empty map with the positive number of slots given by \inlinecode{buckets}.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if a new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  ...
  \item $O(\sqrt{n})$ per call to \inlinecode{update()} and \inlinecode{query()}.
\end{compactitem}
...
Build the min Cartesian tree of an array in linear time. A Cartesian tree is a binary tree whose inorder traversal is the original array order and whose heap property is determined by array values. For a min Cartesian tree, each parent has value less than or equal to its children. Each subtree therefore represents a contiguous array interval whose minimum is at the subtree root. Equal values are broken by position, so the earlier minimum becomes an ancestor of the later one.
...
\begin{compactitem}
  ...
  \item \inlinecode{range\_min\_index(lo, hi)} returns the index of the minimum value in $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{size\_rows()} and \inlinecode{size\_cols()} return the dimensions of the array.
...
\begin{compactitem}
  ...
  \item $O(1)$ per call to \inlinecode{size\_rows()}, \inlinecode{size\_cols()}, and \inlinecode{at()}.
\end{compactitem}
...
This implementation uses \inlinecode{std::pair} to represent points, requiring operator \inlinecode{\textless{}} to be defined on the numeric template type.
...
This implementation uses \inlinecode{std::pair} to represent points, requiring operator \inlinecode{\textless{}} and \inlinecode{long double} casting to be defined on the numeric template type.
...
\begin{compactitem}
  \item \inlinecode{HeavyLight\textless{}T, VALUES\_ON\_EDGES = true\textgreater{}(adj, value)} constructs a new heavy light decomposition on a forest defined by the adjacency list \inlinecode{adj}, with all values initialized to \inlinecode{value}. The adjacency list must consist of only the integers $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. No duplicate edges should exist. Set \inlinecode{VALUES\_ON\_EDGES} to false to store values on nodes instead.
