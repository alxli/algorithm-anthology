...
\begin{compactitem}
  ...
  \item \inlinecode{splice\_range(pos, first, last)} moves the half-open range [\inlinecode{first}, \inlinecode{last}) so it appears immediately before \inlinecode{pos}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{BinaryHeap\textless{}T\textgreater{}(lo, hi)} constructs a priority queue from two ForwardIterators, consisting of elements in the range [\inlinecode{lo}, \inlinecode{hi}).
...
\begin{compactitem}
  ...
  \item \inlinecode{LeftistHeap\textless{}T\textgreater{}(lo, hi)} constructs a priority queue from two ForwardIterators, consisting of elements in the range [\inlinecode{lo}, \inlinecode{hi}).
  \item \inlinecode{size()} returns the size of the priority queue.
  \item \inlinecode{empty()} returns whether the priority queue is empty.
  \item \inlinecode{push(v)} inserts the value \inlinecode{v} into the priority queue.
  \item \inlinecode{pop()} removes the minimum element from the priority queue.
...
\begin{compactitem}
  ...
  \item \inlinecode{SkewHeap\textless{}T\textgreater{}(lo, hi)} constructs a priority queue from two ForwardIterators, consisting of elements in the range [\inlinecode{lo}, \inlinecode{hi}).
  \item \inlinecode{size()} returns the size of the priority queue.
  \item \inlinecode{empty()} returns whether the priority queue is empty.
  \item \inlinecode{push(v)} inserts the value \inlinecode{v} into the priority queue.
  \item \inlinecode{pop()} removes the minimum element from the priority queue.
...
\begin{compactitem}
  ...
  \item \inlinecode{PairingHeap\textless{}T\textgreater{}(lo, hi)} constructs a priority queue from two ForwardIterators, consisting of elements in the range [\inlinecode{lo}, \inlinecode{hi}).
  \item \inlinecode{size()} returns the size of the priority queue.
  \item \inlinecode{empty()} returns whether the priority queue is empty.
...
\begin{compactitem}
  ...
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
  \item \inlinecode{find(k)} returns a pointer to a const value associated with key \inlinecode{k}, or \inlinecode{nullptr} if the key was not found.
  \item \inlinecode{find\_by\_order(k)} returns a key-value pair of the node with a key of 0-based rank \inlinecode{k}, throwing an exception if the rank is not in the range [0, \inlinecode{size()}).
...
\begin{compactitem}
  ...
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(lo, hi, v)} adds an entry with key [\inlinecode{lo}, \inlinecode{hi}] and value \inlinecode{v} to the map, returning \inlinecode{true} if a new interval was added or \inlinecode{false} if the interval already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(lo, hi)} removes the entry with key [\inlinecode{lo}, \inlinecode{hi}] from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the interval was not found.
  \item \inlinecode{find\_key(lo, hi)} returns a pointer to a const \inlinecode{std::pair} representing the key of some interval in the map which intersects with [\inlinecode{lo}, \inlinecode{hi}], or \inlinecode{nullptr} if no such entry was found.
  \item \inlinecode{find\_value(lo, hi)} returns a pointer to a const value of some entry in the map with a key that intersects with [\inlinecode{lo}, \inlinecode{hi}], or \inlinecode{nullptr} if no such entry was found.
  \item \inlinecode{find\_all(lo, hi, f)} calls the function \inlinecode{f(lo, hi, v)} on each entry in the map that overlaps with [\inlinecode{lo}, \inlinecode{hi}], in lexicographically ascending order of intervals.
...
\begin{compactitem}
  \item \inlinecode{SqrtDecomposition\textless{}T\textgreater{}(n, v)} constructs an array of size \inlinecode{n} with indices [0, \inlinecode{n}), and all values initialized to \inlinecode{v}.
...
\begin{compactitem}
  \item \inlinecode{SegTree\textless{}T\textgreater{}(n, v)} constructs an array of size \inlinecode{n} with indices [0, \inlinecode{n}), and all values initialized to \inlinecode{v}.
  \item \inlinecode{SegTree\textless{}T\textgreater{}(lo, hi)} constructs an array from two random-access iterators as a range [\inlinecode{lo}, \inlinecode{hi}), initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  ...
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}. It returns \inlinecode{size()} if \inlinecode{pred} remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}. It returns 0 if \inlinecode{pred} remains true to the beginning.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{LazySegTree\textless{}T\textgreater{}(n, v)} constructs an array of size \inlinecode{n} with indices [0, \inlinecode{n}), and all values initialized to \inlinecode{v}.
  \item \inlinecode{LazySegTree\textless{}T\textgreater{}(lo, hi)} constructs an array from two random-access iterators as a range [\inlinecode{lo}, \inlinecode{hi}), initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If \inlinecode{lo == hi}, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
...
\begin{compactitem}
  \item \inlinecode{IterativeSegTree\textless{}T\textgreater{}(n, v)} constructs an array of size \inlinecode{n} with indices [0, \inlinecode{n}), and all values initialized to \inlinecode{v}.
  \item \inlinecode{IterativeSegTree\textless{}T\textgreater{}(lo, hi)} constructs an array from two random-access iterators as a range [\inlinecode{lo}, \inlinecode{hi}), initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If \inlinecode{lo == hi}, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
...
\begin{compactitem}
  \item \inlinecode{IterativeLazySegTree\textless{}T\textgreater{}(n, v)} constructs an array of size \inlinecode{n} with indices [0, \inlinecode{n}), and all values initialized to \inlinecode{v}.
  \item \inlinecode{IterativeLazySegTree\textless{}T\textgreater{}(lo, hi)} constructs an array from two random-access iterators as a range [\inlinecode{lo}, \inlinecode{hi}), initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If \inlinecode{lo == hi}, then the single specified value is returned.
...
\begin{compactitem}
  \item If \inlinecode{t} $\geq$ \inlinecode{max1}, the clamp changes nothing and the recursion stops (the prune case).
  \item If \inlinecode{max2} \textless{} \inlinecode{t} \textless{} \inlinecode{max1}, every entry equal to \inlinecode{max1} drops to \inlinecode{t} and all others are untouched, so the node updates in $O(1)$: the sum falls by $({\inlinecodemath{max1}} - {\inlinecodemath{t}}) * {\inlinecodemath{count}}$ and \inlinecode{max1} becomes \inlinecode{t} (the break, or tag, case). This lazy tag pushes down to a child only when the child's maximum is larger.
  \item Otherwise (\inlinecode{t} $\leq$ \inlinecode{max2}) the node is too coarse to update directly, so the clamp recurses into both children and re-pulls.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{SegTreeBeats\textless{}T\textgreater{}(n, v)} constructs an array of size \inlinecode{n}, indices [0, \inlinecode{n}), all equal to \inlinecode{v}.
  \item \inlinecode{SegTreeBeats\textless{}T\textgreater{}(lo, hi)} constructs an array from two random-access iterators as a range [\inlinecode{lo}, \inlinecode{hi}), initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{chmin(lo, hi, t)} replaces \inlinecode{a[i]} with \inlinecode{min(a[i], t)} for every \inlinecode{i} in [\inlinecode{lo}, \inlinecode{hi}], inclusive.
  \item \inlinecode{query\_sum(lo, hi)} returns the sum of \inlinecode{a[i]} over \inlinecode{i} in [\inlinecode{lo}, \inlinecode{hi}], inclusive.
  \item \inlinecode{query\_max(lo, hi)} returns the maximum of \inlinecode{a[i]} over \inlinecode{i} in [\inlinecode{lo}, \inlinecode{hi}], inclusive.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{SparseSegTree\textless{}T, N\textgreater{}(v)} constructs an array over indices [0, \inlinecode{N}], with every value implicitly initialized to \inlinecode{v}. Nodes are allocated lazily as indices are touched.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}, where \inlinecode{i} is between 0 and \inlinecode{N}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If \inlinecode{lo == hi}, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{update(lo, hi, d)} modifies the value at each array index from \inlinecode{lo} to \inlinecode{hi}, inclusive, by applying the delta \inlinecode{d} to each value.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}. It returns $N + 1$ if \inlinecode{pred} remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}. It returns 0 if \inlinecode{pred} remains true to the beginning.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{PersistentSegTree\textless{}T\textgreater{}(n, v)} constructs version 0 of an array of size \inlinecode{n} with indices [0, \inlinecode{n}), and all values initialized to \inlinecode{v}.
  \item \inlinecode{PersistentSegTree\textless{}T\textgreater{}(lo, hi)} constructs version 0 from two random-access iterators as a range [\inlinecode{lo}, \inlinecode{hi}), initialized to the elements of the range in the same order.
...
The structure is built once and not modified afterward. All index ranges are inclusive [\inlinecode{lo}, \inlinecode{hi}] with 0-based indices, and values may be of any comparable type.

\begin{compactitem}
  \item \inlinecode{MergeSortTree\textless{}T\textgreater{}(a)} builds the tree over the array \inlinecode{a}.
  \item \inlinecode{count\_leq(lo, hi, x)} returns the number of indices \inlinecode{i} in [\inlinecode{lo}, \inlinecode{hi}] with \inlinecode{a[i]} $\leq$ \inlinecode{x}.
  \item \inlinecode{count\_in(lo, hi, x, y)} returns the number of indices \inlinecode{i} in [\inlinecode{lo}, \inlinecode{hi}] with \inlinecode{x} $\leq$ \inlinecode{a[i]} $\leq$ \inlinecode{y}.
\end{compactitem}
...
All position ranges are inclusive [\inlinecode{lo}, \inlinecode{hi}] with 0-based indices. If the values are large or sparse, compress them to a small contiguous range first.
...
\begin{compactitem}
  ...
  \item \inlinecode{kth\_smallest(lo, hi, k)} returns the \inlinecode{k}-th smallest value among positions [\inlinecode{lo}, \inlinecode{hi}], where \inlinecode{k} is 1-based (so \inlinecode{k == 1} returns the minimum).
  \item \inlinecode{count\_leq(lo, hi, x)} returns the number of positions \inlinecode{i} in [\inlinecode{lo}, \inlinecode{hi}] with \inlinecode{a[i]} $\leq$ \inlinecode{x}.
  \item \inlinecode{count\_in(lo, hi, x, y)} returns the number of positions \inlinecode{i} in [\inlinecode{lo}, \inlinecode{hi}] with \inlinecode{x} $\leq$ \inlinecode{a[i]} $\leq$ \inlinecode{y}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{ImplicitTreap\textless{}T\textgreater{}(n, v)} constructs an array of size \inlinecode{n}, with indices [0, \inlinecode{n}), with all values initialized to \inlinecode{v}.
  \item \inlinecode{ImplicitTreap\textless{}T\textgreater{}(lo, hi)} constructs an array from two iterators as a range [\inlinecode{lo}, \inlinecode{hi}), initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
...
\begin{compactitem}
  \item \inlinecode{RangeTree\textless{}T\textgreater{}(lo, hi)} constructs a set from two random-access iterators to \inlinecode{std::pair} as a range [\inlinecode{lo}, \inlinecode{hi}) of points.
...
\begin{compactitem}
  \item \inlinecode{RangeKDTree\textless{}T\textgreater{}(lo, hi)} constructs a set from two random-access iterators to \inlinecode{std::pair} as a range [\inlinecode{lo}, \inlinecode{hi}) of points.
  \item \inlinecode{query(x1, y1, x2, y2, f)} calls the function \inlinecode{f(i, p)} on each point in the set that falls into the rectangular region consisting of rows from \inlinecode{x1} to \inlinecode{x2}, inclusive, and columns from \inlinecode{y1} to \inlinecode{y2}, inclusive. The first argument to \inlinecode{f} is the 0-based index of the point in the original range given to the constructor. The second argument is the point itself as an \inlinecode{std::pair}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{NearestKDTree\textless{}T\textgreater{}(lo, hi)} constructs a set from two random-access iterators to \inlinecode{std::pair} as a range [\inlinecode{lo}, \inlinecode{hi}) of points.
...
\begin{compactitem}
  \item \inlinecode{Fenwick\textless{}T\textgreater{}(n)} constructs an array with 0-based indices [0, \inlinecode{n}), with all values initialized to 0.
  \item \inlinecode{size()} returns the size of the array.
...
Maintain a numerical array while supporting range increments and point queries. This is done by storing the difference array in a Fenwick tree: adding \inlinecode{x} to [\inlinecode{lo}, \inlinecode{hi}] adds \inlinecode{+x} at \inlinecode{lo} and \inlinecode{-x} after \inlinecode{hi}, and the value at index \inlinecode{i} is the prefix sum of those differences through \inlinecode{i}.

\begin{compactitem}
  \item \inlinecode{FenwickRUPQ\textless{}T\textgreater{}(n)} constructs an array with 0-based indices [0, \inlinecode{n}), with values set to 0.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{add(i, x)} adds \inlinecode{x} to the value at index \inlinecode{i}.
...
\begin{compactitem}
  \item \inlinecode{FenwickRURQ\textless{}T\textgreater{}(n)} constructs an array with 0-based indices [0, \inlinecode{n}), with values set to 0.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
...
\begin{compactitem}
  \item \inlinecode{SparseFenwick\textless{}T, N\textgreater{}()} constructs an array with 0-based indices [0, \inlinecode{N}), implicitly initialized to 0.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{add(i, x)} adds \inlinecode{x} to the value at index \inlinecode{i}.
  \item \inlinecode{add(lo, hi, x)} adds \inlinecode{x} to the values at all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive.
  \item \inlinecode{set(i, x)} assigns the value at index \inlinecode{i} to \inlinecode{x}.
  \item \inlinecode{sum(hi)} returns the sum of all values at indices from 0 to \inlinecode{hi}, inclusive.
  \item \inlinecode{sum(lo, hi)} returns the sum of all values at indices from \inlinecode{lo} to \inlinecode{hi}, inclusive.
  \item \inlinecode{max\_prefix(c)} returns the largest boundary \inlinecode{hi} such that \inlinecode{sum(0, hi - 1) \textless{}= c}, assuming prefix sums are nondecreasing. It may return any value from 0 to \inlinecode{N}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{sum(r, c)} returns the sum of the rectangle with corners (\inlinecode{0}, \inlinecode{0}) and (\inlinecode{r}, \inlinecode{c}), inclusive.
  \item \inlinecode{sum(r1, c1, r2, c2)} returns the sum of the rectangle with upper-left corner (\inlinecode{r1}, \inlinecode{c1}) and lower-right corner (\inlinecode{r2}, \inlinecode{c2}), inclusive.
\end{compactitem}
...
This version is simplified to work only on the fixed set of integer elements [0, \inlinecode{n}), chosen at construction. For arbitrary element types, see Sparse Disjoint Set Union.

\begin{compactitem}
  \item \inlinecode{DSU(n)} constructs \inlinecode{n} singleton partitions over elements [0, \inlinecode{n}).
...
\begin{compactitem}
  \item \inlinecode{RollbackDSU(n)} constructs \inlinecode{n} singleton sets over elements [0, \inlinecode{n}).
...
\begin{compactitem}
  \item \inlinecode{SparseTableLCA(adj)} builds the structure over a forest represented by a bidirectional adjacency list \inlinecode{adj} of nodes numbered [0, \inlinecode{n}), where \inlinecode{n} is \inlinecode{adj.size()}.
...
\begin{compactitem}
  \item \inlinecode{SegTreeLCA(adj)} builds the structure over a forest represented by a bidirectional adjacency list \inlinecode{adj} of nodes numbered [0, \inlinecode{n}), where \inlinecode{n} is \inlinecode{adj.size()}.
  \item \inlinecode{lca(u, v)} returns the lowest common ancestor of nodes \inlinecode{u} and \inlinecode{v}, or $-1$ if they are in different trees.
  \item \inlinecode{dist(u, v)} returns the number of edges on the path between nodes \inlinecode{u} and \inlinecode{v}, or $-1$ if they are in different trees.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{VirtualTree(adj, root)} builds the structure over a tree rooted at \inlinecode{root}, given a bidirectional adjacency list \inlinecode{adj} of nodes numbered [0, \inlinecode{n}), where \inlinecode{n} is \inlinecode{adj.size()}.
  \item \inlinecode{lca(u, v)} returns the lowest common ancestor of nodes \inlinecode{u} and \inlinecode{v}.
  \item \inlinecode{depth\_of(u)} returns the depth of \inlinecode{u} in the original tree (the root has depth 0). The original path length of a virtual-tree edge (\inlinecode{p}, \inlinecode{c}) is \inlinecode{depth\_of(c) - depth\_of(p)}.
  \item \inlinecode{build(nodes)} returns the virtual tree induced by \inlinecode{nodes}. The result has \inlinecode{root} set to the topmost node (the common ancestor of \inlinecode{nodes}), \inlinecode{vertices} listing every original node ID in the virtual tree sorted by entry time, and \inlinecode{edges} listing each (\inlinecode{p}, \inlinecode{c}) pair in original IDs. Both the vertex set and the edges use the node numbering of the original tree. An empty input yields an empty result with \inlinecode{root} = $-1$.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{EulerTourTree(n)} constructs a forest on vertices numbered [0, \inlinecode{n}).
