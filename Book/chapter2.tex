...
\begin{compactitem}
  ...
  \item \inlinecode{reserve(r, c)} declares that the cell (\inlinecode{r}, \inlinecode{c}) may later be updated. Call before \inlinecode{build()}.
  \item \inlinecode{build()} finalizes the coordinate compression. Call exactly once, after all \inlinecode{reserve()} calls.
  \item \inlinecode{add(r, c, x)} adds \inlinecode{x} to the value at index (\inlinecode{r}, \inlinecode{c}), which must have been reserved.
  \item \inlinecode{set(r, c, x)} assigns \inlinecode{x} to the value at index (\inlinecode{r}, \inlinecode{c}), which must have been reserved.
  \item \inlinecode{at(r, c)} returns the value at index (\inlinecode{r}, \inlinecode{c}).
  \item \inlinecode{sum(r, c)} returns the sum of the rectangle with rows $[0, {\inlinecodemath{r}}]$ and columns $[0, {\inlinecodemath{c}}]$.
  \item \inlinecode{sum(r1, c1, r2, c2)} returns the sum of the rectangle with rows $[{\inlinecodemath{r1}}, {\inlinecodemath{r2}}]$ and columns $[{\inlinecodemath{c1}}, {\inlinecodemath{c2}}]$.
\end{compactitem}
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n \log^{2} n)$ per call to \inlinecode{build()}, where $n$ is the number of reserved cells.
...
\begin{compactitem}
  ...
  \item \inlinecode{add\_node(i, value = T())} adds a new single-node tree to the forest, labeled with the integer \inlinecode{i} and with value initialized to \inlinecode{value}.
  ...
  \item \inlinecode{update(a, b, d)} modifies all the values on the path from node \inlinecode{a} to node \inlinecode{b} by applying the delta \inlinecode{d}.
