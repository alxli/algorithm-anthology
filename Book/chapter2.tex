...
\begin{compactitem}
  \item $O(1)$ per call to the first constructor, \inlinecode{size()}, \inlinecode{empty()}, and \inlinecode{top()}.
  \item $O(\log n)$ amortized per call to \inlinecode{push()}, \inlinecode{pop()}, and \inlinecode{join()}, where $n$ is the number of elements in the priority queue.
...
\begin{compactitem}
  \item \inlinecode{ChainingHashMap\textless{}K, V, Hash\textgreater{}(size = 128)} constructs an empty map with \inlinecode{size} buckets.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if a new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  \item $O(n)$ for storage of the map elements.
  \item $O(n)$ auxiliary for \inlinecode{insert()}.
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ for storage of the map elements.
  \item $O(1)$ auxiliary for \inlinecode{insert()} and \inlinecode{operator[]} amortized over a sequence of operations (excluding occasional table rehashes).
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary for \inlinecode{insert()} and \inlinecode{erase()}.
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(k \log N)$ for storage after $k$ index updates.
  \item $O(\log N)$ auxiliary stack space for \inlinecode{update()}, \inlinecode{query()}, \inlinecode{max\_right()}, and \inlinecode{min\_left()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{Quadtree\textless{}T, R, C\textgreater{}(v = T())} constructs a two-dimensional array with rows $[0, R]$ and columns $[0, C]$. All array values are initialized to \inlinecode{v}.
...
\begin{compactitem}
  \item \inlinecode{LazyQuadtree\textless{}T, R, C\textgreater{}(v = T())} constructs a 2D array with rows $[0, R]$ and columns $[0, C]$. All array values are implicitly initialized to \inlinecode{v}. Nodes are allocated lazily as indices are touched.
  \item \inlinecode{at(r, c)} returns the value at row \inlinecode{r}, column \inlinecode{c}.
  \item \inlinecode{query(r1, c1, r2, c2)} returns the result of \inlinecode{combine()} applied to every value in the rectangular region consisting of rows $[{\inlinecodemath{r1}}, {\inlinecodemath{r2}}]$ and columns $[{\inlinecodemath{c1}}, {\inlinecodemath{c2}}]$.
  \item \inlinecode{update(r, c, d)} assigns the value \inlinecode{v} at $({\inlinecodemath{r}}, {\inlinecodemath{c}})$ to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{update(r1, c1, r2, c2)} modifies the value at each index of the rectangular region consisting of rows $[{\inlinecodemath{r1}}, {\inlinecodemath{r2}}]$ and columns $[{\inlinecodemath{c1}}, {\inlinecodemath{c2}}]$ by applying the delta \inlinecode{d} to each value.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{SegTree2D\textless{}T, R, C\textgreater{}(v = T())} constructs a two-dimensional array with rows $[0, R]$ and columns $[0, C]$. All array values are implicitly initialized to \inlinecode{v}. Nodes are allocated lazily as indices are touched.
  \item \inlinecode{at(r, c)} returns the value at row \inlinecode{r}, column \inlinecode{c}.
  \item \inlinecode{query(r1, c1, r2, c2)} returns the result of \inlinecode{combine()} applied to every value in the rectangular region consisting of rows $[{\inlinecodemath{r1}}, {\inlinecodemath{r2}}]$ and columns $[{\inlinecodemath{c1}}, {\inlinecodemath{c2}}]$.
  \item \inlinecode{update(r, c, d)} assigns the value \inlinecode{v} at $({\inlinecodemath{r}}, {\inlinecodemath{c}})$ to \inlinecode{apply\_delta(v, d)}.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n \log n)$ per call to the constructor, where $n$ is the number of points.
  \item $O(\log n + m)$ on average per call to \inlinecode{query()}, where $m$ is the number of points that are reported by the query.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{nearest(x, y, can\_equal = true)} returns a point in the set that is closest to $({\inlinecodemath{x}}, {\inlinecodemath{y}})$ by Euclidean distance. This may be equal to $({\inlinecodemath{x}}, {\inlinecodemath{y}})$ only if \inlinecode{can\_equal} is \inlinecode{true}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{for\_each\_path(u, v, include\_lca, f)} decomposes the path from node \inlinecode{u} to node \inlinecode{v} into heavy path ranges and calls \inlinecode{f(path, lo, hi, up)} on each range, where \inlinecode{up} says the path segment is traversed upward from \inlinecode{u} toward the LCA. If \inlinecode{VALUES\_ON\_EDGES = true}, pass \inlinecode{include\_lca = false} to skip the LCA's vertex slot. Returns false if \inlinecode{u} and \inlinecode{v} are in different trees.
\end{compactitem}
...
\end{lstlisting}
\subsection{Top Tree}
Maintain a dynamic forest while supporting both path aggregates and rooted-subtree aggregates. Top trees represent each tree as a hierarchy of clusters. A path cluster summarizes a contiguous path, while non-path clusters are raked onto vertices so that changing the preferred path only requires a logarithmic number of local rotations and recomputations.
...
\begin{compactitem}
  ...
  \item \inlinecode{set\_vertex(u, value)} changes vertex \inlinecode{u}'s value to \inlinecode{value}.
  ...
  \item \inlinecode{path\_query(u, v)} returns the aggregate over all vertex and edge values on the path from \inlinecode{u} to \inlinecode{v}. The vertices must be connected.
  ...
  \item \inlinecode{reroot(u)} makes vertex \inlinecode{u} the root of its represented tree.
  \item \inlinecode{lca(u, v)} returns the lowest common ancestor of \inlinecode{u} and \inlinecode{v} relative to the current root. The vertices must be connected.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the constructor, \inlinecode{size()}, and \inlinecode{edges()}.
  \item $O(\log n)$ amortized per call to all other operations, where $n$ is the number of vertices and edges in the represented tree.
\end{compactitem}

\Needspace*{3\baselineskip}
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n + m)$ for the vertex and edge nodes, where $m$ is the number of edges ever added.
  \item $O(1)$ auxiliary for all operations.
\end{compactitem}
