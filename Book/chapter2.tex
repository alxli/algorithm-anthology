...
\begin{compactitem}
  \item \inlinecode{WaveletTree(a, min\_val, max\_val)} builds the tree over the array \inlinecode{a}, whose values must all lie in [\inlinecode{min\_val}, \inlinecode{max\_val}].
  \item \inlinecode{size()} returns the size of the array.
...
\begin{compactitem}
  ...
  \item \inlinecode{query(x1, y1, x2, y2, f)} calls the function \inlinecode{f(i, p)} on each point in the set that falls into the rectangular region consisting of rows $[{\inlinecodemath{x1}}, {\inlinecodemath{x2}}]$ and columns $[{\inlinecodemath{y1}}, {\inlinecodemath{y2}}]$. The first argument to \inlinecode{f} is the 0-based index of the point in the original range given to the constructor. The second argument is the point itself as an \inlinecode{std::pair}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{query(x1, y1, x2, y2, f)} calls the function \inlinecode{f(p)} on each point in the set that falls into the rectangular region consisting of rows $[{\inlinecodemath{x1}}, {\inlinecodemath{x2}}]$ and columns $[{\inlinecodemath{y1}}, {\inlinecodemath{y2}}]$. The argument \inlinecode{p} is the point as an \inlinecode{std::pair}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{max\_prefix(c)} returns the largest boundary \inlinecode{hi} such that \inlinecode{sum(0, hi - 1)} $\leq$ \inlinecode{c}, assuming prefix sums are nondecreasing. It may return any boundary in $[0, n]$, where $n$ is \inlinecode{size()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{sum(hi)} returns the sum of all values at indices $[0, {\inlinecodemath{hi}}]$.
  \item \inlinecode{sum(lo, hi)} returns the sum of all values at indices $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$.
  \item \inlinecode{max\_prefix(c)} returns the largest boundary \inlinecode{hi} such that \inlinecode{sum(0, hi - 1)} $\leq$ \inlinecode{c}, assuming prefix sums are nondecreasing. It may return any boundary in $[0, N]$.
\end{compactitem}
