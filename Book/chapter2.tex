...
\begin{compactitem}
  ...
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{find\_first(lo, hi, pred)} returns the smallest index in [\inlinecode{lo}, \inlinecode{hi}] matching the search, or $-1$ if none, in $O(\log n)$. \inlinecode{pred(v)} takes a node aggregate and must be monotone: if it is false, no element under that node qualifies (e.g. for the default min tree, \inlinecode{pred(v) = (v \textless{}= x)} finds the leftmost element \inlinecode{\textless{}= x}).
  \item \inlinecode{find\_last(lo, hi, pred)} is the analogous query returning the largest such index.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}, or \inlinecode{size()} if the predicate remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}, or 0 if the predicate remains true to the beginning.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ per call to both constructors, where $n$ is the size of the array.
  \item $O(1)$ per call to \inlinecode{size()}.
  \item $O(\log n)$ per call to \inlinecode{at()}, \inlinecode{update()}, \inlinecode{query()}, \inlinecode{find\_first()}, \inlinecode{find\_last()}, \inlinecode{max\_right()}, and \inlinecode{min\_left()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If \inlinecode{lo == hi}, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{find\_first(lo, hi, pred)} returns the smallest index in [\inlinecode{lo}, \inlinecode{hi}] matching the search, or $-1$ if none. \inlinecode{pred(v)} takes a node aggregate and must be monotone: if it is false, no element under that node qualifies.
  \item \inlinecode{find\_last(lo, hi, pred)} is the analogous query returning the largest such index.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}, or \inlinecode{size()} if the predicate remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}, or 0 if the predicate remains true to the beginning.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(\log n)$ per call to \inlinecode{update()}, \inlinecode{query()}, \inlinecode{find\_first()}, \inlinecode{find\_last()}, \inlinecode{max\_right()}, and \inlinecode{min\_left()}.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n)$ for storage of the array elements.
  \item $O(1)$ auxiliary space for \inlinecode{query()} and \inlinecode{update()}, and $O(\log n)$ for boundary searches.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{update(lo, hi, d)} modifies the value at each array index from \inlinecode{lo} to \inlinecode{hi}, inclusive, by applying the delta \inlinecode{d} to each value.
  \item \inlinecode{find\_first(lo, hi, pred)} returns the smallest index in [\inlinecode{lo}, \inlinecode{hi}] matching the search, or $-1$ if none, in $O(\log N)$. \inlinecode{pred(v)} takes a node aggregate and must be monotone: if it is false, no element under that node qualifies (untouched ranges use the implicit \inlinecode{repeat\_value(init, len)}). For the default min tree, \inlinecode{pred(v) = (v \textless{}= x)} finds the leftmost element \inlinecode{\textless{}= x}.
  \item \inlinecode{find\_last(lo, hi, pred)} is the analogous query returning the largest such index.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}, or $N + 1$ if the predicate remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range [\inlinecode{lo}, \inlinecode{hi}) satisfies \inlinecode{pred}, or 0 if the predicate remains true to the beginning.
\end{compactitem}
...
Maintains immutable versions of a segment tree under point updates. Each update creates a new root by copying only the nodes on the path from the root to the updated leaf, while all unchanged subtrees are shared with previous versions. Persistent segment trees are useful for offline order-statistics queries, rollback-like histories, and functional dynamic programming states.

\begin{compactitem}
  \item \inlinecode{PersistentSegTree(a)} constructs version 0 of an array with indices [0, \inlinecode{n}) from an array \inlinecode{a} of length \inlinecode{n}.
  ...
  \item \inlinecode{query(version, lo, hi)} returns the sum of the inclusive range [\inlinecode{lo}, \inlinecode{hi}] in the specified version.
...
\begin{compactitem}
  ...
  \item \inlinecode{query(u, v)} returns the result of \inlinecode{combine()} applied to all values on the path from node \inlinecode{u} to node \inlinecode{v}, or throws if \inlinecode{u} and \inlinecode{v} are in different trees.
  \item \inlinecode{update(u, v, d)} modifies all values on the path from node \inlinecode{u} to node \inlinecode{v} by respectively applying the delta \inlinecode{d}, or throws if \inlinecode{u} and \inlinecode{v} are in different trees.
