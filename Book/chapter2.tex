...
\textbf{Time Complexity}: $O(1)$ expected amortized per call to \inlinecode{get(key, \&value)} and \inlinecode{put(key, value)}, and $O(n)$ in the collision-heavy worst case for the hash table.
...
\begin{compactitem}
  ...
  \item \inlinecode{update(r, c, d)} applies delta \inlinecode{d} to the entry at index $({\inlinecodemath{r}}, {\inlinecodemath{c}})$.
  \item \inlinecode{at(r, c)} returns the value at index $({\inlinecodemath{r}}, {\inlinecodemath{c}})$.
  \item \inlinecode{query(r1, c1, r2, c2)} returns the aggregate over the rectangle with rows $[{\inlinecodemath{r1}}, {\inlinecodemath{r2}}]$ and columns $[{\inlinecodemath{c1}}, {\inlinecodemath{c2}}]$.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{query(r1, c1, r2, c2)} returns the result of \inlinecode{combine()} applied to every value in the rectangular region consisting of rows $[{\inlinecodemath{r1}}, {\inlinecodemath{r2}}]$ and columns $[{\inlinecodemath{c1}}, {\inlinecodemath{c2}}]$.
...
\begin{compactitem}
  ...
  \item \inlinecode{nearest(x, y, can\_equal = true)} returns a point in the set that is closest to $({\inlinecodemath{x}}, {\inlinecodemath{y}})$ by Euclidean distance. This may be equal to $({\inlinecodemath{x}}, {\inlinecodemath{y}})$ only if \inlinecode{can\_equal} is \inlinecode{true}; at least one eligible point must exist.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(1)$ per undone union during \inlinecode{rollback(snapshot)}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{depth\_of(u)} returns the depth of \inlinecode{u} in the original tree (the root has depth $0$). The original path length of a virtual-tree edge $({\inlinecodemath{p}}, {\inlinecodemath{c}})$ is \inlinecode{depth\_of(c)} $-$ \inlinecode{depth\_of(p)}.
  \item \inlinecode{build(nodes)} returns the virtual tree induced by \inlinecode{nodes}. The result has \inlinecode{root} set to the topmost node (the common ancestor of \inlinecode{nodes}), \inlinecode{nodes} listing every original node ID in the virtual tree sorted by entry time, and \inlinecode{edges} listing each $({\inlinecodemath{p}}, {\inlinecodemath{c}})$ pair in original IDs. Both the node set and the edges use the node numbering of the original tree. An empty input yields an empty result with \inlinecode{root} $= -1$.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n \log n)$ to store the binary ancestor table.
  \item $O(k)$ auxiliary plus result size per call to \inlinecode{build()}.
\end{compactitem}
