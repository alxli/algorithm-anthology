...
\begin{compactitem}
  ...
  \item \inlinecode{min()}, \inlinecode{max()}, \inlinecode{lower\_bound(k)}, \inlinecode{upper\_bound(k)}, \inlinecode{prev(k)}, and \inlinecode{next(k)} return the matching key-value entry, or \inlinecode{std::nullopt} if no such entry exists.
  \item \inlinecode{entries()} returns all key-value entries in ascending order of keys.
\end{compactitem}

The explicit navigation routines above depend only on the BST property and can be copied unchanged to the other BST variants later in this chapter.

\Needspace*{3\baselineskip}
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the constructor, \inlinecode{size()}, and \inlinecode{empty()}.
  \item $O(h)$ per call to \inlinecode{insert()}, \inlinecode{erase()}, \inlinecode{find()}, \inlinecode{min()}, \inlinecode{max()}, \inlinecode{lower\_bound()}, \inlinecode{upper\_bound()}, \inlinecode{prev()}, and \inlinecode{next()}, where $h$ is the height of the tree. This is $O(\log n)$ when the tree is balanced, but $O(n)$ in the worst case for this unbalanced implementation.
  \item $O(n)$ per call to \inlinecode{entries()}, where $n$ is the number of nodes currently in the map.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary stack space for \inlinecode{insert()}, \inlinecode{erase()}, and \inlinecode{entries()}.
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
\end{exampleoutput}
\subsection{AVL Tree}
Maintain an ordered map, that is, an ordered collection of key-value pairs such that each possible key appears at most once in the collection. This implementation requires an ordering on the set of possible keys defined by \inlinecode{operator\textless{}} on the key type. An AVL tree is a binary search tree balanced by height, guaranteeing $O(\log n)$ worst-case running time in insertions and deletions by making sure that the heights of the left and right subtrees at every node differ by at most 1. Whenever an insertion or deletion breaks this invariant, it is repaired with one or two rotations at each affected node along the search path.
...
Maintain an ordered map, that is, an ordered collection of key-value pairs such that each possible key appears at most once in the collection. This implementation requires an ordering on the set of possible keys defined by \inlinecode{operator\textless{}} on the key type. A red-black tree is a binary search tree balanced by coloring its nodes red or black, then constraining node colors on any simple path from the root to a leaf. Specifically, a red node may never have a red child, and every path from the root to a null leaf must pass through the same number of black nodes, which together bound the tree's height to $O(\log n)$. Insertions and deletions repair these invariants by recoloring nodes and performing rotations.
...
Maintain an ordered map, that is, an ordered collection of key-value pairs such that each possible key appears at most once in the collection. In addition, support queries for keys given their ranks as well as queries for the ranks of given keys. This implementation requires an ordering on the set of possible keys defined by \inlinecode{operator\textless{}} on the key type. A size balanced tree augments each node with the size of its subtree, using it to maintain balance and compute order statistics. After each update, rotations restore the invariant that every subtree is at least as large as each of its sibling's child subtrees, keeping the height logarithmic.
...
Compared with the open-addressing version (2.3.9), chaining keeps each entry at a stable address: a rehash never moves existing nodes, so pointers from \inlinecode{find()} and references from \inlinecode{operator[]} stay valid, and load factors above 1 are tolerated gracefully. The costs are a separate allocation per entry and cache-unfriendly pointer chasing during traversal.

\begin{compactitem}
  \item \inlinecode{ChainingHashMap()} constructs an empty map.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if a new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  ...
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{find\_first(lo, hi, pred)} returns the smallest index in \inlinecode{[lo, hi]} matching the search, or $-1$ if none, in $O(\log n)$. \inlinecode{pred(v)} takes a node aggregate and must be monotone: if it is false, no element under that node qualifies (e.g. for the default min tree, \inlinecode{pred(v) = (v \textless{}= x)} finds the leftmost element \inlinecode{\textless{}= x}).
  \item \inlinecode{find\_last(lo, hi, pred)} is the analogous query returning the largest such index.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range \inlinecode{[lo, hi)} satisfies \inlinecode{pred}, or \inlinecode{size()} if the predicate remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range \inlinecode{[lo, hi)} satisfies \inlinecode{pred}, or 0 if the predicate remains true to the beginning.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{IterativeSegTree(n, v)} constructs an array of size \inlinecode{n} with indices from 0 to \inlinecode{n - 1}, inclusive, and all values initialized to \inlinecode{v}.
  \item \inlinecode{IterativeSegTree(lo, hi)} constructs an array from two random-access iterators as a range \inlinecode{[lo, hi)}, initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If \inlinecode{lo == hi}, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{find\_first(lo, hi, pred)} returns the smallest index in \inlinecode{[lo, hi]} matching the search, or $-1$ if none. \inlinecode{pred(v)} takes a node aggregate and must be monotone: if it is false, no element under that node qualifies.
  \item \inlinecode{find\_last(lo, hi, pred)} is the analogous query returning the largest such index.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range \inlinecode{[lo, hi)} satisfies \inlinecode{pred}, or \inlinecode{size()} if the predicate remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range \inlinecode{[lo, hi)} satisfies \inlinecode{pred}, or 0 if the predicate remains true to the beginning.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{IterativeLazySegTree(n, v)} constructs an array of size \inlinecode{n} with indices from 0 to \inlinecode{n - 1}, inclusive, and all values initialized to \inlinecode{v}.
  \item \inlinecode{IterativeLazySegTree(lo, hi)} constructs an array from two random-access iterators as a range \inlinecode{[lo, hi)}, initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}, where \inlinecode{i} is between 0 and \inlinecode{size() - 1}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If \inlinecode{lo == hi}, then the single specified value is returned.
...
\begin{compactitem}
  \item If \inlinecode{t \textgreater{}= max1}, the clamp changes nothing and the recursion stops (the prune case).
  \item If \inlinecode{max2 \textless{} t \textless{} max1}, every entry equal to \inlinecode{max1} drops to \inlinecode{t} and all others are untouched, so the node updates in $O(1)$: the sum falls by \inlinecode{(max1 - t) * count} and \inlinecode{max1} becomes \inlinecode{t} (the break, or tag, case). This lazy tag pushes down to a child only when the child's maximum is larger.
  \item Otherwise (\inlinecode{t \textless{}= max2}) the node is too coarse to update directly, so the clamp recurses into both children and re-pulls.
\end{compactitem}
...
Use a 64-bit element type \inlinecode{T} when sums can be large.

\begin{compactitem}
  \item \inlinecode{SegTreeBeats(n, v)} constructs an array of size \inlinecode{n}, indices 0 to \inlinecode{n - 1}, all equal to \inlinecode{v}.
  \item \inlinecode{SegTreeBeats(lo, hi)} constructs the array from the range \inlinecode{[lo, hi)} of random-access iterators.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{chmin(lo, hi, t)} replaces \inlinecode{a[i]} with \inlinecode{min(a[i], t)} for every \inlinecode{i} in \inlinecode{[lo, hi]}, inclusive.
  \item \inlinecode{query\_sum(lo, hi)} returns the sum of \inlinecode{a[i]} over \inlinecode{i} in \inlinecode{[lo, hi]}, inclusive.
  \item \inlinecode{query\_max(lo, hi)} returns the maximum of \inlinecode{a[i]} over \inlinecode{i} in \inlinecode{[lo, hi]}, inclusive.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{SparseSegTree\textless{}T, N\textgreater{}(v)} constructs an array over indices 0 to $N$ (inclusive), with every value implicitly initialized to \inlinecode{v}. Nodes are allocated lazily as indices are touched.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}, where \inlinecode{i} is between 0 and \inlinecode{N}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If \inlinecode{lo == hi}, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{apply\_delta(v, d)}.
  \item \inlinecode{update(lo, hi, d)} modifies the value at each array index from \inlinecode{lo} to \inlinecode{hi}, inclusive, by applying the delta \inlinecode{d} to each value.
  \item \inlinecode{find\_first(lo, hi, pred)} returns the smallest index in \inlinecode{[lo, hi]} matching the search, or $-1$ if none, in $O(\log N)$. \inlinecode{pred(v)} takes a node aggregate and must be monotone: if it is false, no element under that node qualifies (untouched ranges use the implicit \inlinecode{repeat\_value(init, len)}). For the default min tree, \inlinecode{pred(v) = (v \textless{}= x)} finds the leftmost element \inlinecode{\textless{}= x}.
  \item \inlinecode{find\_last(lo, hi, pred)} is the analogous query returning the largest such index.
  \item \inlinecode{max\_right(lo, pred)} returns the largest boundary \inlinecode{hi} such that the aggregate over the half-open range \inlinecode{[lo, hi)} satisfies \inlinecode{pred}, or $N + 1$ if the predicate remains true to the end.
  \item \inlinecode{min\_left(hi, pred)} returns the smallest boundary \inlinecode{lo} such that the aggregate over the half-open range \inlinecode{[lo, hi)} satisfies \inlinecode{pred}, or 0 if the predicate remains true to the beginning.
\end{compactitem}
...
When prefix sums are nondecreasing, \inlinecode{max\_prefix(c)} answers prefix-boundary queries by binary lifting: it walks the implicit power-of-two block structure to find the longest prefix with sum at most \inlinecode{c}. This lets the same structure double as a dynamic multiset over a bounded integer domain, useful for coordinate-compressed kth-element lookups and online rank queries.

\begin{compactitem}
  \item \inlinecode{FenwickTree(n)} constructs an array with 0-based indices from 0 to \inlinecode{n - 1}, inclusive, with all values initialized to 0.
  \item \inlinecode{size()} returns the size of the array.
...
\begin{compactitem}
  \item \inlinecode{SparseFenwickRURQ\textless{}T, N\textgreater{}()} constructs an array with 0-based indices 0 to \inlinecode{N - 1} (inclusive), implicitly initialized to 0.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{add(i, x)} adds \inlinecode{x} to the value at index \inlinecode{i}.
  \item \inlinecode{add(lo, hi, x)} adds \inlinecode{x} to the values at all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive.
  \item \inlinecode{set(i, x)} assigns the value at index \inlinecode{i} to \inlinecode{x}.
...
\begin{compactitem}
  \item \inlinecode{FenwickTree2D(rows, cols)} constructs a \inlinecode{rows} by \inlinecode{cols} array with 0-based indices, with all values initialized to 0.
  ...
  \item \inlinecode{sum(r, c)} returns the sum of the rectangle with corners \inlinecode{(0, 0)} and \inlinecode{(r, c)}, inclusive.
  \item \inlinecode{sum(r1, c1, r2, c2)} returns the sum of the rectangle with upper-left corner \inlinecode{(r1, c1)} and lower-right corner \inlinecode{(r2, c2)}, inclusive.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{OfflineFenwick2D()} constructs an empty structure in the declaration phase.
  \item \inlinecode{reserve(r, c)} declares that the cell \inlinecode{(r, c)} may later be updated. Call before \inlinecode{build()}.
  \item \inlinecode{build()} finalizes the coordinate compression. Call exactly once, after all \inlinecode{reserve()} calls.
  \item \inlinecode{add(r, c, x)} adds \inlinecode{x} to the value at index \inlinecode{(r, c)}, which must have been reserved.
  \item \inlinecode{set(r, c, x)} assigns \inlinecode{x} to the value at index \inlinecode{(r, c)}, which must have been reserved.
  \item \inlinecode{sum(r, c)} returns the sum of the rectangle with corners \inlinecode{(0, 0)} and \inlinecode{(r, c)}, inclusive.
  \item \inlinecode{sum(r1, c1, r2, c2)} returns the sum of the rectangle with upper-left corner \inlinecode{(r1, c1)} and lower-right corner \inlinecode{(r2, c2)}, inclusive.
  \item \inlinecode{at(r, c)} returns the value at index \inlinecode{(r, c)}.
\end{compactitem}
...
This version is simplified to work only on the fixed set of integer elements 0 through \inlinecode{n - 1}, chosen at construction. For arbitrary element types, see Sparse Disjoint Set Union.

\begin{compactitem}
  \item \inlinecode{DSU(n)} constructs \inlinecode{n} singleton partitions over elements 0 through \inlinecode{n - 1}.
...
\begin{compactitem}
  \item \inlinecode{SparseDSU()} constructs an empty set.
...
\begin{compactitem}
  \item \inlinecode{RollbackDSU(n)} constructs \inlinecode{n} singleton sets over elements 0 through \inlinecode{n - 1}.
...
\begin{compactitem}
  \item \inlinecode{SparseTableLCA(adj)} builds the structure over a forest represented by a bidirectional adjacency list \inlinecode{adj} of nodes numbered from 0 to \inlinecode{adj.size() - 1}.
...
\begin{compactitem}
  \item \inlinecode{SegTreeLCA(adj)} builds the structure over a forest represented by a bidirectional adjacency list \inlinecode{adj} of nodes numbered from 0 to \inlinecode{adj.size() - 1}.
  \item \inlinecode{lca(u, v)} returns the lowest common ancestor of nodes \inlinecode{u} and \inlinecode{v}, or $-1$ if they are in different trees.
  \item \inlinecode{dist(u, v)} returns the number of edges on the path between nodes \inlinecode{u} and \inlinecode{v}, or $-1$ if they are in different trees.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{WeightedTreePath(adj)} builds the structure over a forest given by a weighted, bidirectional adjacency list \inlinecode{adj}, where \inlinecode{adj[u]} holds \inlinecode{\{v, w\}} pairs for each edge \inlinecode{u}-\inlinecode{v} of weight \inlinecode{w}, over nodes numbered from 0 to \inlinecode{adj.size() - 1}.
...
Maintain an undirected forest dynamically using Euler Tour Trees. Each tree is represented by a sequence of directed edge occurrences in an implicit treap: an undirected edge \inlinecode{\{u, v\}} contributes two nodes, \inlinecode{(u -\textgreater{} v)} and \inlinecode{(v -\textgreater{} u)}. Rerooting a represented tree is a cyclic rotation of this sequence, while \inlinecode{link} concatenates two tours with the new edge occurrences and \inlinecode{cut} removes the two occurrences of an existing edge, splitting one tour into two.
...
\begin{compactitem}
  \item \inlinecode{EulerTourTree(n)} constructs a forest on vertices \inlinecode{0..n-1}.
