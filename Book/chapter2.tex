...
\begin{compactitem}
  \item \inlinecode{TopTree(n, v)} constructs a forest on vertices $[0, {\inlinecodemath{n}})$ with every vertex value initialized to \inlinecode{v}.
  ...
  \item \inlinecode{link(u, v, w)} adds an edge with value \inlinecode{w} between different trees, returning its edge id or $-1$ if the edge would create a cycle.
  \item \inlinecode{cut(e)} removes edge id \inlinecode{e}. The edge id must currently be present.
  \item \inlinecode{set\_vertex(u, v)} changes vertex \inlinecode{u}'s value to \inlinecode{v}.
  \item \inlinecode{set\_edge(e, v)} changes edge \inlinecode{e}'s value to \inlinecode{v}. The edge id must currently be present.
