...
For cycle detection, see the variants on iterated functions in 1.7.3, which can be directly adapted for linked lists.
...
The comparator-aware navigation routines \inlinecode{min()}, \inlinecode{max()}, \inlinecode{lower\_bound(k)}, \inlinecode{upper\_bound(k)}, \inlinecode{prev(k)}, and \inlinecode{next(k)} from the treap in 2.3.1 depend only on the BST property and may be adapted here as needed.
...
The comparator-aware navigation routines \inlinecode{min()}, \inlinecode{max()}, \inlinecode{lower\_bound(k)}, \inlinecode{upper\_bound(k)}, \inlinecode{prev(k)}, and \inlinecode{next(k)} from the treap in 2.3.1 depend only on the BST property and may be adapted here as needed. For contest use on GNU C++ judges, PBDS ordered trees provide the same order-statistic operations with much less code; see 8.6. This implementation is useful when PBDS is unavailable or when customizing tree internals.
...
\begin{compactitem}
  ...
  \item \inlinecode{query(version, lo, hi)} returns the aggregate of the values at indices in $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$ in the specified version. If \inlinecode{lo == hi}, then the single specified value is returned.
...
\begin{compactitem}
  ...
  \item $O(\log n)$ per call to \inlinecode{at()}, \inlinecode{query()}, and \inlinecode{update()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(\log n)$ auxiliary stack space for construction, \inlinecode{at()}, \inlinecode{query()}, and \inlinecode{update()}.
