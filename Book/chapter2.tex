...
\begin{compactitem}
  \item \inlinecode{SparseTable(lo, hi)} builds the table from the range \inlinecode{[lo, hi)} of a random-access iterator.
...
\begin{compactitem}
  \item \inlinecode{DisjointSparseTable(lo, hi)} builds the table from the range \inlinecode{[lo, hi)} of a random-access iterator.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{query(lo, hi)} returns \inlinecode{combine()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{SqrtDecomposition(lo, hi)} constructs an array from two random-access iterators as a range \inlinecode{[lo, hi)}, initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
...
\begin{compactitem}
  \item \inlinecode{SparseSegTree(v)} constructs an array over indices 0 to \inlinecode{N}, inclusive, with every value implicitly initialized to \inlinecode{v}. Nodes are allocated lazily as indices are touched.
...
\begin{compactitem}
  \item \inlinecode{ImplicitTreap(n, v)} constructs an array of size \inlinecode{n} with all values initialized to \inlinecode{v}.
...
\begin{compactitem}
  \item \inlinecode{LazyQuadtree(v)} constructs a two-dimensional array with rows from 0 to \inlinecode{R} and columns from 0 to \inlinecode{C}, inclusive. All values are implicitly initialized to \inlinecode{v}.
  \item \inlinecode{at(r, c)} returns the value at row \inlinecode{r}, column \inlinecode{c}.
...
This implementation uses 1-based array indices, so valid indices are from 1 to \inlinecode{n}, inclusive.

\begin{compactitem}
  \item \inlinecode{initialize(n)} initializes array indices \inlinecode{1} to \inlinecode{n} to 0.
...
This implementation uses 0-based array indices, so valid indices are from 0 to \inlinecode{size() - 1}, inclusive.

\begin{compactitem}
  \item \inlinecode{FenwickRUPQ(n)} constructs an array of size \inlinecode{n} with every value initialized to 0.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{add(i, x)} adds \inlinecode{x} to the value at index \inlinecode{i}.
...
This implementation uses 0-based array indices externally, so valid indices are from 0 to \inlinecode{size() - 1}, inclusive.

\begin{compactitem}
  \item \inlinecode{FenwickPURQ(n)} constructs an array of size \inlinecode{n} with every value initialized to 0.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{add(i, x)} adds \inlinecode{x} to the value at index \inlinecode{i}.
  \item \inlinecode{set(i, x)} assigns the value at index \inlinecode{i} to \inlinecode{x}.
...
\begin{compactitem}
  \item \inlinecode{FenwickRURQ(n)} constructs an array of size \inlinecode{n} with every value initialized to 0.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
...
This implementation uses 0-based array indices, so all indices from 0 to \inlinecode{N}, inclusive, are accessible.

\begin{compactitem}
  \item \inlinecode{SparseFenwickRURQ()} constructs an initially zero array over indices 0 to \inlinecode{N}.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{add(i, x)} adds \inlinecode{x} to the value at index \inlinecode{i}.
  \item \inlinecode{add(lo, hi, x)} adds \inlinecode{x} to the values at all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive.
  \item \inlinecode{set(i, x)} assigns the value at index \inlinecode{i} to \inlinecode{x}.
...
Maintain a 2D numerical array while supporting point increments and rectangle-sum queries. This is the two-dimensional form of the standard Fenwick tree: each internal entry stores a rectangular block sum, and prefix queries combine $O(\log R \cdot \log C)$ disjoint blocks.

This implementation uses 1-based indices, so rows are from 1 to \inlinecode{R}, inclusive, and columns are from 1 to \inlinecode{C}, inclusive.
...
This implementation uses 0-based indices, so rows are from 0 to \inlinecode{R}, inclusive, and columns are from 0 to \inlinecode{C}, inclusive.
...
\begin{compactitem}
  \item \inlinecode{SparseFenwick2D()} constructs an initially zero 2D array over rows 0 to \inlinecode{R} and columns 0 to \inlinecode{C}.
  \item \inlinecode{add(r, c, x)} adds \inlinecode{x} to the value at index \inlinecode{(r, c)}.
