...
\begin{compactitem}
  \item \inlinecode{binary\_heap()} constructs an empty priority queue.
  \item \inlinecode{binary\_heap(lo, hi)} constructs a priority queue from two ForwardIterators, consisting of elements in the range [\inlinecode{lo}, \inlinecode{hi}).
...
Maintain a mergeable min-priority queue, that is, a collection of elements with support for querying and extraction of the minimum as well as efficient merging with other instances. This implementation requires an ordering on the set of possible elements defined by \inlinecode{operator \textless{}}. A randomized mergeable heap uses a coin-toss to recursively combine nodes of two binary trees.

\begin{compactitem}
  \item \inlinecode{randomized\_heap()} constructs an empty priority queue.
  \item \inlinecode{randomized\_heap(lo, hi)} constructs a priority queue from two ForwardIterators, consisting of elements in the range [\inlinecode{lo}, \inlinecode{hi}).
  \item \inlinecode{size()} returns the size of the priority queue.
  \item \inlinecode{empty()} returns whether the priority queue is empty.
  \item \inlinecode{push(v)} inserts the value \inlinecode{v} into the priority queue.
  \item \inlinecode{pop()} removes the minimum element from the priority queue.
...
\begin{compactitem}
  \item $O(1)$ per call to the first constructor, \inlinecode{size()}, \inlinecode{empty()}, and \inlinecode{top()}.
  \item $O(\log n)$ expected worst case per call to \inlinecode{push()}, \inlinecode{pop()}, and \inlinecode{absorb()}, where $n$ is the number of elements in the priority queue.
  \item $O(n)$ per call to the second constructor on the distance between lo and hi.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{skew\_heap()} constructs an empty priority queue.
  \item \inlinecode{skew\_heap(lo, hi)} constructs a priority queue from two ForwardIterators, consisting of elements in the range [\inlinecode{lo}, \inlinecode{hi}).
  \item \inlinecode{size()} returns the size of the priority queue.
  \item \inlinecode{empty()} returns whether the priority queue is empty.
  \item \inlinecode{push(v)} inserts the value \inlinecode{v} into the priority queue.
  \item \inlinecode{pop()} removes the minimum element from the priority queue.
...
\begin{compactitem}
  \item \inlinecode{pairing\_heap()} constructs an empty priority queue.
  \item \inlinecode{pairing\_heap(lo, hi)} constructs a priority queue from two ForwardIterators, consisting of elements in the range [\inlinecode{lo}, \inlinecode{hi}).
  \item \inlinecode{size()} returns the size of the priority queue.
  \item \inlinecode{empty()} returns whether the priority queue is empty.
  \item \inlinecode{push(v)} inserts the value \inlinecode{v} into the priority queue.
  \item \inlinecode{pop()} removes the minimum element from the priority queue.
...
\begin{compactitem}
  \item \inlinecode{binary\_search\_tree()} constructs an empty map.
  ...
  \item \inlinecode{find(k)} returns a pointer to a const value associated with key \inlinecode{k}, or \inlinecode{NULL} if the key was not found.
...
\begin{compactitem}
  \item \inlinecode{avl\_tree()} constructs an empty map.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if an new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  \item \inlinecode{red\_black\_tree()} constructs an empty map.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if an new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  \item \inlinecode{splay\_tree()} constructs an empty map.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if an new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  \item \inlinecode{size\_balanced\_tree()} constructs an empty map.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if an new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  \item \inlinecode{interval\_treap()} constructs an empty map.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  ...
  \item \inlinecode{find\_key(lo, hi)} returns a pointer to a const \inlinecode{std::pair} representing the key of some interval in the map which intersects with [\inlinecode{lo}, \inlinecode{hi}], or \inlinecode{NULL} if no such entry was found.
  \item \inlinecode{find\_value(lo, hi)} returns a pointer to a const value of some entry in the map with a key that intersects with [\inlinecode{lo}, \inlinecode{hi}], or \inlinecode{NULL} if no such entry was found.
...
\begin{compactitem}
  \item \inlinecode{hash\_map()} constructs an empty map.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if an new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
\begin{compactitem}
  \item \inlinecode{skip\_list()} constructs an empty map.
  \item \inlinecode{size()} returns the size of the map.
  \item \inlinecode{empty()} returns whether the map is empty.
  \item \inlinecode{insert(k, v)} adds an entry with key \inlinecode{k} and value \inlinecode{v} to the map, returning \inlinecode{true} if an new entry was added or \inlinecode{false} if the key already exists (in which case the map is unchanged and the old value associated with the key is preserved).
  \item \inlinecode{erase(k)} removes the entry with key \inlinecode{k} from the map, returning \inlinecode{true} if the removal was successful or \inlinecode{false} if the key to be removed was not found.
...
The query operation is defined by an associative \inlinecode{join\_values()} function which satisfies \inlinecode{join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z)} for all values \inlinecode{x}, \inlinecode{y}, and \inlinecode{z} in the array. The default definition below assumes a numerical array type, supporting queries for the "min" of the target range. Another possible query operation is "sum", in which the \inlinecode{join\_values()} function should be defined to return $a + b$.

The update operation is defined by the \inlinecode{join\_value\_with\_delta()} function which determines the change made to array values. The default definition below supports updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which \inlinecode{join\_value\_with\_delta(v, d)} should be defined to return $v + d$.
...
The query operation is defined by an associative \inlinecode{join\_values()} function which satisfies \inlinecode{join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z)} for all values \inlinecode{x}, \inlinecode{y}, and \inlinecode{z} in the array. The default code below assumes a numerical array type, defining queries for the "min" of the target range. Another possible query operation is "sum", in which case the \inlinecode{join\_values()} function should be defined to return $a + b$.

The update operation is defined by the \inlinecode{join\_value\_with\_delta()} function, which determines the change made to array values. The default definition below supports updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which \inlinecode{join\_value\_with\_delta(v, d)} should be defined to return $v + d$.

\begin{compactitem}
  \item \inlinecode{segment\_tree(n, v)} constructs an array of size \inlinecode{n} with indices from 0 to \inlinecode{n - 1}, inclusive, and all values initialized to \inlinecode{v}.
  \item \inlinecode{segment\_tree(lo, hi)} constructs an array from two random-access iterators as a range [\inlinecode{lo}, \inlinecode{hi}), initialized to the elements of the range in the same order.
  \item \inlinecode{size()} returns the size of the array.
  \item \inlinecode{at(i)} returns the value at index \inlinecode{i}.
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{join\_values()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If the distance between \inlinecode{lo} and \inlinecode{hi} is 1, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{join\_value\_with\_delta(v, d)}.
\end{compactitem}
...
The update operation is defined by the \inlinecode{join\_value\_with\_delta()} and \inlinecode{join\_deltas()} functions, which determines the change made to array values. These must satisfy:

\begin{compactitem}
  \item \inlinecode{join\_deltas(d1, join\_deltas(d2, d3)) = join\_deltas(join\_deltas(d1, d2), d3)}.
  \item \inlinecode{join\_value\_with\_delta(join\_values(v, ..., v), d, m)} should be equal to \inlinecode{join\_values(join\_value\_with\_delta(v, d, 1), ...)}, with $m$ values on each side.
  \item if a sequence $d_1, ..., d_m$ of deltas is used to update a value $v$, then \inlinecode{join\_value\_with\_delta(v, join\_deltas(d\_1, ..., d\_m), 1)} should be equivalent to $m$ sequential calls to \inlinecode{join\_value\_with\_delta(v, d\_i, 1)} for $i = 1, ..., m$. The default code below defines updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which case \inlinecode{join\_value\_with\_delta(v, d, len)} should be defined to return $v + d \cdot len$ and \inlinecode{join\_deltas(d1, d2)} should be defined to return $d1 + d2$.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{query(lo, hi)} returns the result of \inlinecode{join\_values()} applied to all indices from \inlinecode{lo} to \inlinecode{hi}, inclusive. If the distance between \inlinecode{lo} and \inlinecode{hi} is 1, then the single specified value is returned.
  \item \inlinecode{update(i, d)} assigns the value \inlinecode{v} at index \inlinecode{i} to \inlinecode{join\_value\_with\_delta(v, d)}.
  \item \inlinecode{update(lo, hi, d)} modifies the value at each array index from \inlinecode{lo} to \inlinecode{hi}, inclusive, by respectively joining them with \inlinecode{d} using \inlinecode{join\_value\_with\_delta()}.
\end{compactitem}
...
The query operation is defined by the \inlinecode{join\_values()} and \inlinecode{join\_region()} functions where \inlinecode{join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z)} for all values \inlinecode{x}, \inlinecode{y}, and \inlinecode{z} in the array. The \inlinecode{join\_region(v, area)} function must be defined in conjunction to efficiently return the result of \inlinecode{join\_values()} applied to a rectangular sub-array of \inlinecode{area} elements. The default code below assumes a numerical array type, defining queries for the "min" of the target range. Another possible query operation is "sum", in which case \inlinecode{join\_values(a, b)} should return $a + b$ and \inlinecode{join\_region(v, area)} should return $v \cdot area$.

The update operation is defined by the \inlinecode{join\_value\_with\_delta()} function, which determines the change made to array values. The default code below defines updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which \inlinecode{join\_value\_with\_delta(v, d)} should be defined to return $v + d$.

\begin{compactitem}
  \item \inlinecode{quadtree(v)} constructs a two-dimensional array with rows from 0 to \inlinecode{MAXR} and columns from 0 to \inlinecode{MAXC}, inclusive. All values are implicitly initialized to \inlinecode{v}.
  \item \inlinecode{at(r, c)} returns the value at row \inlinecode{r}, column \inlinecode{c}.
  \item \inlinecode{query(r1, c1, r2, c2)} returns the result of \inlinecode{join\_values()} applied to every value in the rectangular region consisting of rows from \inlinecode{r1} to \inlinecode{r2}, inclusive, and columns from \inlinecode{c1} to \inlinecode{c2}, inclusive.
  \item \inlinecode{update(r, c, d)} assigns the value \inlinecode{v} at (\inlinecode{r}, \inlinecode{c}) to \inlinecode{join\_value\_with\_delta(v, d)}.
\end{compactitem}

\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ per call to the constructor.
  \item $O(\max\left(\text{MAXR}, \text{MAXC}\right))$ per call to \inlinecode{at()}, \inlinecode{update()}, and \inlinecode{query()}.
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the array elements, where $n$ is the number of updated entries in the array.
  \item $O(\sqrt{\max\left(\text{MAXR}, \text{MAXC}\right)})$ auxiliary stack space for \inlinecode{update()}, \inlinecode{query()}, and \inlinecode{at()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{join\_deltas(d1, join\_deltas(d2, d3)) = join\_deltas(join\_deltas(d1, d2), d3)}.
  \item \inlinecode{join\_value\_with\_delta(join\_values(v, ..., v), d, m)} should be equal to \inlinecode{join\_values(join\_value\_with\_delta(v, d, 1), ...)}, with $m$ values on each side.
  \item if a sequence $d_1, ..., d_m$ of deltas is used to update a value $v$, then \inlinecode{join\_value\_with\_delta(v, join\_deltas(d\_1, ..., d\_m), 1)} should be equivalent to $m$ sequential calls to \inlinecode{join\_value\_with\_delta(v, d\_i, 1)} for $i = 1, ..., m$. The default code below defines updates that "set" the chosen array index to a new value. Another possible update operation is "increment", in which case \inlinecode{join\_value\_with\_delta(v, d, area)} should be defined to return $v + d \cdot area$ and \inlinecode{join\_deltas(d1, d2)} should be defined to return $d1 + d2$.
\end{compactitem}

\begin{compactitem}
  \item \inlinecode{quadtree(v)} constructs a two-dimensional array with rows from 0 to \inlinecode{MAXR} and columns from 0 to \inlinecode{MAXC}, inclusive. All values are implicitly initialized to \inlinecode{v}.
  \item \inlinecode{at(r, c)} returns the value at row \inlinecode{r}, column \inlinecode{c}.
  \item \inlinecode{query(r1, c1, r2, c2)} returns the result of \inlinecode{join\_values()} applied to every value in the rectangular region consisting of rows from \inlinecode{r1} to \inlinecode{r2} and columns from \inlinecode{c1} to \inlinecode{c2}, inclusive.
  \item \inlinecode{update(r, c, d)} assigns the value \inlinecode{v} at (\inlinecode{r}, \inlinecode{c}) to \inlinecode{join\_value\_with\_delta(v, d)}.
  \item \inlinecode{update(r1, c1, r2, c2)} modifies the value at each index of the rectangular region consisting of rows from \inlinecode{r1} to \inlinecode{r2} and columns from \inlinecode{c1} to \inlinecode{c2}, inclusive, by respectively joining them with \inlinecode{d} using \inlinecode{join\_value\_with\_delta()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{segment\_tree\_2d(v)} constructs a two-dimensional array with rows from 0 to \inlinecode{MAXR} and columns from 0 to \inlinecode{MAXC}, inclusive. All values are implicitly initialized to \inlinecode{v}.
  \item \inlinecode{at(r, c)} returns the value at row \inlinecode{r}, column \inlinecode{c}.
  \item \inlinecode{query(r1, c1, r2, c2)} returns the result of \inlinecode{join\_values()} applied to every value in the rectangular region consisting of rows from \inlinecode{r1} to \inlinecode{r2}, inclusive, and columns from \inlinecode{c1} to \inlinecode{c2}, inclusive.
  \item \inlinecode{update(r, c, d)} assigns the value \inlinecode{v} at (\inlinecode{r}, \inlinecode{c}) to \inlinecode{join\_value\_with\_delta(v, d)}.
\end{compactitem}
...
\begin{compactitem}
  \item $O(1)$ per call to the constructor.
  \item $O(\log\left(\text{MAXR}\right) \cdot \log\left(\text{MAXC}\right))$ per call to \inlinecode{at()}, \inlinecode{update()}, and \inlinecode{query()}.
\end{compactitem}

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the array elements, where $n$ is the number of updated entries in the array.
  \item $O(\log\left(\text{MAXR}\right) + \log\left(\text{MAXC}\right))$ auxiliary stack space for \inlinecode{update()}, \inlinecode{query()}, and \inlinecode{at()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{range\_tree(lo, hi)} constructs a set from two random-access iterators to \inlinecode{std::pair} as a range [\inlinecode{lo}, \inlinecode{hi}) of points.
...
\begin{compactitem}
  \item \inlinecode{kd\_tree(lo, hi)} constructs a set from two random-access iterators to \inlinecode{std::pair} as a range [\inlinecode{lo}, \inlinecode{hi}) of points.
  \item \inlinecode{query(x1, y1, x2, y2, f)} calls the function \inlinecode{f(i, p)} on each point in the set that falls into the rectangular region consisting of rows from \inlinecode{x1} to \inlinecode{x2}, inclusive, and columns from \inlinecode{y1} to \inlinecode{y2}, inclusive. The first argument to \inlinecode{f} is the zero-based index of the point in the original range given to the constructor. The second argument is the point itself as an \inlinecode{std::pair}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{r\_tree(lo, hi)} constructs a set from two random-access iterators as a range [\inlinecode{lo}, \inlinecode{hi}) of segments.
  \item \inlinecode{nearest(x, y)} returns a segment in the set that contains some point which is as close or closer to (\inlinecode{x}, \inlinecode{y}) by Euclidean distance than any point on any other segment in the set.
\end{compactitem}
...
Maintain an array of numerical type, allowing for updates of individual indices (point update) and queries for the sum of contiguous sub-arrays (range queries). This implementation assumes that the array is 1-based (i.e. has valid indices from 1 to \inlinecode{MAXN - 1}, inclusive).

\begin{compactitem}
  \item \inlinecode{initialize()} resets the data structure.
  \item \inlinecode{a[i]} stores the value at index \inlinecode{i}.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to \inlinecode{initialize()}, where $n$ is the size of the array.
...
Maintain an array of numerical type, allowing for contiguous sub-arrays to be simultaneously incremented by arbitrary values (range update) and queries for the sum of contiguous sub-arrays (range query). This implementation uses \inlinecode{std::map} for coordinate compression, allowing for large indices to be accessed with efficient space complexity. That is, all array indices from 0 to \inlinecode{MAXN}, inclusive, are accessible.
...
\textbf{Time Complexity}: $O(\log^{2} \text{MAXN})$ per call to all member functions. If \inlinecode{std::map} is replaced with \inlinecode{std::unordered\_map}, then the amortized running time will become $O(\log \text{MAXN})$.

\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \log \text{MAXN})$ for storage of the array elements, where $n$ is the number of distinct indices that have been accessed across all of the operations so far.
  \item $O(1)$ auxiliary for all operations.
\end{compactitem}
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n \cdot m)$ per call to \inlinecode{initialize()}, where $n$ is the number of rows and $m$ is the number of columns.
...
Maintain a 2D array of numerical type, allowing for rectangular sub-matrices to be simultaneously incremented by arbitrary values (range update) and queries for the sum of rectangular sub-matrices (range query). This implementation uses \inlinecode{std::map} for coordinate compression, allowing for large indices to be accessed with efficient space complexity. That is, rows have valid indices from 0 to \inlinecode{MAXR}, inclusive, and columns have valid indices from 0 to \inlinecode{MAXC}, inclusive.
...
\textbf{Time Complexity}: $O(\log^{2}(\text{MAXR}) \cdot \log^{2}(\text{MAXC}))$ per call to all member functions. If \inlinecode{std::map} is replaced with \inlinecode{std::unordered\_map}, then the amortized running time will become $O(\log\left(\text{MAXR}\right) \cdot \log\left(\text{MAXC}\right))$.
...
Maintain a set of elements partitioned into non-overlapping subsets using a collection of trees. Each partition is assigned a unique representative known as the parent, or root. The following implements two well-known optimizations known as union-by-rank and path compression. This version uses an \inlinecode{std::map} for storage and coordinate compression (thus, element types must meet the requirements of key types for \inlinecode{std::map}).
...
The query operation is defined by an associative \inlinecode{join\_values()} function which satisfies \inlinecode{join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z)} for all values \inlinecode{x}, \inlinecode{y}, and \inlinecode{z} in the tree. The default code below assumes a numerical tree type, defining queries for the "min" of the target range. Another possible query operation is "sum", in which case the \inlinecode{join\_values()} function should be defined to return $a + b$.

The update operation is defined by the \inlinecode{join\_value\_with\_delta()} and \inlinecode{join\_deltas()} functions, which determines the change made to values. These must satisfy:

\begin{compactitem}
  \item \inlinecode{join\_deltas(d1, join\_deltas(d2, d3)) = join\_deltas(join\_deltas(d1, d2), d3)}.
  \item \inlinecode{join\_value\_with\_delta(join\_values(v, ..., v), d, m)} should be equal to \inlinecode{join\_values(join\_value\_with\_delta(v, d, 1), ...)}, with $m$ values on each side.
  \item if a sequence $d_1, ..., d_m$ of deltas is used to update a value $v$, then \inlinecode{join\_value\_with\_delta(v, join\_deltas(d\_1, ..., d\_m), 1)} should be equivalent to $m$ sequential calls to \inlinecode{join\_value\_with\_delta(v, d\_i, 1)} for $i = 1, ..., m$. The default code below defines updates that "set" a path's edges or nodes to a new value. Another possible update operation is "increment", in which case \inlinecode{join\_value\_with\_delta(v, d, len)} should be defined to return $v + d \cdot len$ and \inlinecode{join\_deltas(d1, d2)} should be defined to return $d1 + d2$.
\end{compactitem}

\begin{compactitem}
  \item \inlinecode{heavy\_light(n, adj[], v)} constructs a new heavy light decomposition on a tree with \inlinecode{n} nodes defined by the adjacency list \inlinecode{adj[]}, with all values initialized to \inlinecode{v}. The adjacency list must be a size \inlinecode{n} array of vectors consisting of only the integers from 0 to \inlinecode{n - 1}, inclusive. No duplicate edges should exist, and the graph must be connected.
  \item \inlinecode{query(u, v)} returns the result of \inlinecode{join\_values()} applied to all values on the path from node \inlinecode{u} to node \inlinecode{v}.
  \item \inlinecode{update(u, v, d)} modifies all values on the path from node \inlinecode{u} to node \inlinecode{v} by respectively joining them with \inlinecode{d} using \inlinecode{join\_value\_with\_delta()}.
\end{compactitem}
...
The query operation is defined by an associative \inlinecode{join\_values()} function which satisfies \inlinecode{join\_values(x, join\_values(y, z)) = join\_values(join\_values(x, y), z)} for all values \inlinecode{x}, \inlinecode{y}, and \inlinecode{z} in the forest. The default code below assumes a numerical forest type, defining queries for the "min" of the target range. Another possible query operation is "sum", in which case the \inlinecode{join\_values()} function should be defined to return $a + b$.
...
\begin{compactitem}
  \item \inlinecode{join\_deltas(d1, join\_deltas(d2, d3)) = join\_deltas(join\_deltas(d1, d2), d3)}.
  \item \inlinecode{join\_value\_with\_delta(join\_values(v, ..., v), d, m)} should be equal to \inlinecode{join\_values(join\_value\_with\_delta(v, d, 1), ...)}, with $m$ values on each side.
  \item if a sequence $d_1, ..., d_m$ of deltas is used to update a value $v$, then \inlinecode{join\_value\_with\_delta(v, join\_deltas(d\_1, ..., d\_m), 1)} should be equivalent to $m$ sequential calls to \inlinecode{join\_value\_with\_delta(v, d\_i, 1)} for $i = 1, ..., m$. The default code below defines updates that "set" a path's nodes to a new value. Another possible update operation is "increment", in which case \inlinecode{join\_value\_with\_delta(v, d, len)} should be defined to return $v + d \cdot len$ and \inlinecode{join\_deltas(d1, d2)} should be defined to return $d1 + d2$.
\end{compactitem}

\begin{compactitem}
  \item \inlinecode{link\_cut\_forest()} constructs an empty forest with no trees.
  ...
  \item \inlinecode{query(a, b)} returns the result of \inlinecode{join\_values()} applied to all values on the path from the node \inlinecode{a} to node \inlinecode{b}.
  \item \inlinecode{update(a, b, d)} modifies all the values on the path from node \inlinecode{a} to node \inlinecode{b} by respectively joining them with \inlinecode{d} using \inlinecode{join\_value\_with\_delta()}.
\end{compactitem}
