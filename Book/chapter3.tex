...
\begin{compactitem}
  ...
  \item \inlinecode{entropy(freq)} returns order-0 empirical entropy in bits per symbol for a frequency table.
  ...
  \item \inlinecode{expected\_code\_length(freq, length)} returns the average encoded bits per symbol for code lengths \inlinecode{length[c]}. The two input vectors must have the same size.
\end{compactitem}
