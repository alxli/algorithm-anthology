...
\begin{compactitem}
  ...
  \item \inlinecode{count(haystack, needle)} returns the number of non-overlapping occurrences of \inlinecode{needle} in \inlinecode{haystack}, like Python's \inlinecode{str.count}. This differs from \inlinecode{find\_all().size()}, which counts overlapping matches; e.g. \inlinecode{count("aaa", "aa")} returns \inlinecode{1}, but \inlinecode{find\_all("aaa", "aa")} finds two.
...
By default, each sequence value is cast to \inlinecode{uint64\_t} and mixed. For non-integer element types, pass a custom value hasher that maps each element to a stable nonzero value in [1, \inlinecode{HASH\_MOD}).
...
\begin{compactitem}
  ...
  \item \inlinecode{get(l, r)} returns the hash of the half-open subsequence [\inlinecode{l}, \inlinecode{r}).
...
\begin{compactitem}
  ...
  \item \inlinecode{hash(lo, hi)} returns the pair of hashes of the half-open subsequence [\inlinecode{lo}, \inlinecode{hi}). Two ranges are equal with high probability if and only if their \inlinecode{hash()} pairs are equal.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{distinct\_prefix\_hashes(lo, hi)} returns prefix hashes where each distinct key in the range [\inlinecode{lo}, \inlinecode{hi}) contributes only on its first occurrence.
\end{compactitem}
...
For odd palindromes, \inlinecode{odd[i]} is the radius centered at character \inlinecode{s[i]}, including the center character. Thus the palindrome spans [\inlinecode{i} - \inlinecode{odd[i]} + 1, \inlinecode{i} + \inlinecode{odd[i]}). For even palindromes, \inlinecode{even[i]} is the radius centered between \inlinecode{s[i - 1]} and \inlinecode{s[i]}. Thus the palindrome spans [\inlinecode{i} - \inlinecode{even[i]}, \inlinecode{i} + \inlinecode{even[i]}).

\begin{compactitem}
  \item \inlinecode{Manacher(s)} constructs the palindromic radii arrays for string \inlinecode{s}.
  \item \inlinecode{is\_palindrome(lo, hi)} returns whether substring [\inlinecode{lo}, \inlinecode{hi}) is a palindrome.
...
\begin{compactitem}
  ...
  \item \inlinecode{eval(lo, hi)} returns the evaluation of a range [\inlinecode{lo}, \inlinecode{hi}) of already split-up expression tokens, where \inlinecode{lo} and \inlinecode{hi} must be random-access iterators.
