...
Common string functions, many of which already have standard STL equivalents. Most of the following implementations are presented for educational purposes, and are not heavily optimized. They often depend on certain \inlinecode{std::string} functions that have unspecified complexity.
...
\begin{compactitem}
  ...
  \item $O(1)$ auxiliary for \inlinecode{find\_all\_in()}.
\end{compactitem}
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ for storage of the suffix and LCP arrays.
  \item $O(n)$ auxiliary for the constructor.
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary stack space for destruction and \inlinecode{walk()}, where $n$ is the maximum length of any string that has been inserted so far.
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary stack space for \inlinecode{insert()}, destruction, and \inlinecode{walk()}, where $n$ is the maximum length of any string that has been inserted so far.
  \item $O(n)$ auxiliary stack space for \inlinecode{erase(s)}, where $n$ is the length of \inlinecode{s}.
  \item $O(1)$ auxiliary for all other operations.
\end{compactitem}
...
More precisely, state \inlinecode{v} contributes one distinct substring for each length greater than \inlinecode{st[st[v].link].len} and at most \inlinecode{st[v].len}. When a character is appended, \inlinecode{extend()} creates a state for the new full string and walks suffix links from the previous \inlinecode{last}, adding missing transitions to it. If an existing transition leads to a state whose maximum length is too large, a shorter clone of that state is inserted and the affected transitions and suffix links are redirected to it, splitting the ending-position classes without changing the recognized substrings. Summing these length ranges gives the number of distinct substrings. Membership queries follow transitions, while longest common substring queries retreat through suffix links when a match fails.
...
\begin{compactitem}
  \item $O(n)$ expected per call to the constructor for a string of length $n$.
  \item $O(1)$ expected amortized per call to \inlinecode{add()}.
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(\min\left(n, m\right))$ auxiliary heap space for \inlinecode{edit\_distance()}.
  ...
  \item $O(\log\left(\max\left(n, m\right)\right))$ auxiliary stack space and $O(\min\left(n, m\right))$ auxiliary heap space for \inlinecode{hirschberg\_align(s1, s2)}, where $n$ and $m$ are the lengths of \inlinecode{s1} and \inlinecode{s2}, respectively.
\end{compactitem}
...
A_GGCA
\end{exampleoutput}
\subsection{Palindrome Dynamic Programming}
...
Evaluate an expression according to the order of operations (parentheses, unary plus and minus signs, multiplication/division, addition/subtraction). The following is a minimalistic recursive descent implementation using iterators. Each precedence level is parsed by its own function that obtains operands by calling the next-tighter level, so the call structure itself enforces the order of operations. Literals are nonnegative decimal integers, division truncates toward zero, and the input must be a valid expression without whitespace. Every divisor must be nonzero, and all intermediate results must fit in \inlinecode{int}.
...
An arbitrary operand type is supported by changing the \inlinecode{Operand} alias and the static \inlinecode{is\_operand()} and \inlinecode{eval\_operand()} helpers. For maximum reliability, the string representation of operands should not use characters shared by any operator. For instance, the best practice instead of accepting \inlinecode{-1} as a valid operand (since the \inlinecode{-} sign may conflict with the identical binary operator), is to specify nonnegative numbers as operands alongside the unary operator \inlinecode{-}.

Operators may be non-empty strings of any length, but should not contain any parentheses or shared characters with the string representations of operands. When multiple operators match at the same position, the longest one is chosen, so \inlinecode{\textless{}=} remains one token when \inlinecode{\textless{}} is also an operator.

\begin{compactitem}
  \item \inlinecode{RecursiveDescentParser(unary\_ops, binary\_ops)} initializes a parser with operators specified by hash tables \inlinecode{unary\_ops} (of operator to unary function object) and \inlinecode{binary\_ops} (of operator to pair of binary function object and operator precedence). Operator precedences should be numbered upwards starting at $0$ (lowest precedence, evaluated last).
  \item \inlinecode{split(s)} returns a vector of tokens for the expression \inlinecode{s}, split on the given operators during construction. Each parenthesis, operator, and operand satisfying \inlinecode{is\_operand()} will be split into a separate token.
  \item \inlinecode{eval(lo, hi)} returns the evaluation of a range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ of already split-up expression tokens, where \inlinecode{lo} and \inlinecode{hi} must be random-access iterators.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(m \cdot k \cdot \log m)$ per call to the constructor, where $m$ is the total number of operators and $k$ is their maximum length.
...
Evaluates an infix expression with custom operand types, prefix unary operators, binary operators, and precedences. In infix notation, binary operators appear between operands; prefix and postfix notation place operators before and after their operands, respectively. Postfix notation is also called reverse Polish notation (RPN).

The shunting yard algorithm commonly converts infix expressions to postfix notation. This implementation instead maintains operator and value stacks and evaluates each operator when it would be emitted to the postfix output, avoiding an intermediate token list. Multiplication by juxtaposition is not supported.
...
\begin{compactitem}
  \item \inlinecode{ShuntingYardParser(unary\_ops, binary\_ops)} initializes a parser with operators specified by hash tables \inlinecode{unary\_ops} (of operator to unary function object) and \inlinecode{binary\_ops} (of operator to pair of binary function object and operator precedence). Operator precedences should be numbered upwards starting at $0$ (lowest precedence, evaluated last).
  \item \inlinecode{split(s)} returns a vector of tokens for the expression \inlinecode{s}, split on the given operators during construction. Each parenthesis, operator, and operand satisfying \inlinecode{is\_operand()} will be split into a separate token.
  \item \inlinecode{eval(lo, hi)} returns the evaluation of a range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ of already split-up expression tokens, where \inlinecode{lo} and \inlinecode{hi} must be random-access iterators.
  \item \inlinecode{eval(s)} returns the evaluation of expression \inlinecode{s}, after first calling \inlinecode{split(s)} to obtain the tokens.
\end{compactitem}
