...
\begin{compactitem}
  ...
  \item \inlinecode{find\_all(haystack, needle)} returns a vector of all positions where the string \inlinecode{needle} appears in the string \inlinecode{haystack}. Matches may overlap; e.g. \inlinecode{find\_all("aaa", "aa")} returns \inlinecode{\{0, 1\}}.
...
\begin{compactitem}
  ...
  \item \inlinecode{repeat(s, n)} returns \inlinecode{s} concatenated with itself \inlinecode{n} times, like Python's \inlinecode{s * n}. For a single repeated character, prefer \inlinecode{std::string(n, ch)}.
...
Specializations of \inlinecode{std::hash} for \inlinecode{std::pair} and \inlinecode{std::tuple} are also provided, letting those keys be used in \inlinecode{std::unordered\_map}/\inlinecode{std::unordered\_set} without an explicit hasher argument. Specializing \inlinecode{std::hash} for purely standard types is technically nonstandard, so the functor forms above are the portable alternative. Vector keys are left to the functors to avoid clashing with the standard \inlinecode{std::hash\textless{}std::vector\textless{}char\textgreater{}\textgreater{}}.
...
\begin{compactitem}
  \item \inlinecode{RollingHash\textless{}T, ValueHasher = RollingValueHasher\textless{}T\textgreater{}\textgreater{}(hasher = ValueHasher())} constructs an empty hash sequence using \inlinecode{hasher} to map values before mixing.
  \item \inlinecode{RollingHash\textless{}T, ValueHasher = RollingValueHasher\textless{}T\textgreater{}\textgreater{}(first, last, hasher = ValueHasher())} constructs prefix hashes for any iterator range of values accepted by the value hasher.
  \item \inlinecode{RollingHash\textless{}T, ValueHasher = RollingValueHasher\textless{}T\textgreater{}\textgreater{}(v, hasher = ValueHasher())} constructs prefix hashes for vector \inlinecode{v}.
  \item \inlinecode{get(l, r)} returns the hash of the half-open subsequence $[{\inlinecodemath{l}}, {\inlinecodemath{r}})$.
  \item \inlinecode{hash(first, last, hasher = ValueHasher())} returns the hash of an iterator range.
  \item \inlinecode{hash(v, hasher = ValueHasher())} returns the hash of vector \inlinecode{v}.
...
Maintain a polynomial rolling hash over a sequence of elements of an arbitrary type \inlinecode{T}, under insertion and deletion at either end, in $O(1)$ time per operation. Each element is first reduced to a nonnegative integer ``digit'' by a user-supplied hasher, so any comparable type (characters, integers, whole words, tuples) can be hashed. This is the deque generalization of the classic Rabin-Karp sliding window: pushing at the back and popping from the front is exactly a moving window, and this structure additionally supports pushing at the front and popping from the back.
...
\begin{compactitem}
  ...
  \item \inlinecode{a == b} (and \inlinecode{a != b}) compares two deques: it checks equal length, then equal hash pairs, so two deques hold equal sequences with high probability if and only if they compare equal.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{hash(lo, hi)} returns the pair of hashes of the half-open subsequence $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$. Two ranges are equal with high probability if and only if their \inlinecode{hash()} pairs are equal.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{distinct\_prefix\_hashes(lo, hi)} returns prefix hashes where each distinct key in the range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$ contributes only on its first occurrence.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(n + m)$ auxiliary for \inlinecode{find(haystack, needle)} where $n$ is the length of \inlinecode{haystack} and $m$ is the length of \inlinecode{needle}.
\end{compactitem}
...
Given two strings, determine their longest common substring: a contiguous run of characters that appears in both. A dynamic program computes, for each pair of prefixes, the length of their longest common suffix; the largest value over all pairs locates a longest common substring. Only the previous row of the table is retained, so the auxiliary is linear in the shorter string.
