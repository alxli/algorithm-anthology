...
\begin{compactitem}
  \item $O(n)$ auxiliary per call to operations that return a new string or vector of strings.
...
By default, each sequence value is cast to \inlinecode{uint64\_t} and mixed. For non-integer element types, pass a custom value hasher that maps each element to a stable nonzero value in [\inlinecode{1}, \inlinecode{HASH\_MOD}).

\begin{compactitem}
  \item \inlinecode{RollingHash\textless{}T, Hash = ValueHasher\textless{}T\textgreater{}\textgreater{}(hasher = Hash())} constructs an empty hash sequence using \inlinecode{hasher} to map values before mixing.
  \item \inlinecode{RollingHash\textless{}T, Hash = ValueHasher\textless{}T\textgreater{}\textgreater{}(lo, hi, hasher = Hash())} constructs prefix hashes from the values in the half-open iterator range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
  \item \inlinecode{RollingHash\textless{}T, Hash = ValueHasher\textless{}T\textgreater{}\textgreater{}(v, hasher = Hash())} constructs prefix hashes for vector \inlinecode{v}.
  \item \inlinecode{get(lo, hi)} returns the hash of the half-open subsequence $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
  \item \inlinecode{hash(lo, hi, hasher = Hash())} returns the hash of the half-open iterator range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
  \item \inlinecode{hash(v, hasher = Hash())} returns the hash of vector \inlinecode{v}.
...
\begin{compactitem}
  ...
  \item $O(1)$ per call to \inlinecode{get(lo, hi)} and \inlinecode{concat(left, right, right\_len)}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{HashDeque\textless{}T = int, Hash = IdentityDigit\textless{}T\textgreater{}\textgreater{}(hasher = Hash())} constructs an empty deque whose \inlinecode{hasher} maps each element to its nonnegative integer digit. For integer-like \inlinecode{T} the default hasher casts the element to \inlinecode{uint64\_t}, so \inlinecode{HashDeque\textless{}char\textgreater{}} and \inlinecode{HashDeque\textless{}int\textgreater{}} work with no hasher supplied.
  ...
  \item \inlinecode{a == b} (and \inlinecode{a != b}) compares the lengths and hash pairs of two deques. Equal sequences always compare equal; equality indicates equal sequences with high probability.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(1)$ auxiliary for \inlinecode{find\_in(haystack)}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(1)$ auxiliary for \inlinecode{find\_all\_in(haystack, report\_match)}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(1)$ per call to \inlinecode{is\_palindrome(lo, hi)}.
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(k)$ auxiliary for \inlinecode{lyndon\_factorization(s)}, where $k$ is the number of factors.
  \item $O(1)$ auxiliary for \inlinecode{min\_rotation(s)}.
\end{compactitem}
...
\textbf{Time Complexity}: $O(n \cdot m)$ per call, where $n$ and $m$ are the lengths of \inlinecode{s1} and \inlinecode{s2}, respectively.
...
\textbf{Time Complexity}: $O(n \cdot m)$ per call to \inlinecode{edit\_distance(s1, s2)}, \inlinecode{align\_sequences(s1, s2)}, and \inlinecode{hirschberg\_align\_sequences(s1, s2)}, where $n$ and $m$ are the lengths of \inlinecode{s1} and \inlinecode{s2}, respectively.
...
\begin{compactitem}
  \item \inlinecode{longest\_palindromic\_subsequence(s)} returns one longest palindromic subsequence of \inlinecode{s}.
  \item \inlinecode{min\_insertions\_palindrome(s)} returns the minimum number of characters that must be inserted anywhere in \inlinecode{s} to make it a palindrome.
\end{compactitem}
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n^{2})$ auxiliary for \inlinecode{longest\_palindromic\_subsequence(s)}.
  \item $O(n)$ auxiliary for \inlinecode{min\_insertions\_palindrome(s)}.
\end{compactitem}
...
\textbf{Space Complexity}: $O(n)$ auxiliary stack space per call, where $n$ is the length of \inlinecode{s}.
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(m)$ auxiliary for \inlinecode{byte\_frequencies(s)}.
...
\begin{compactitem}
  ...
  \item $O(n + b)$ to encode $n$ characters into $b$ bits, and $O(b)$ to decode $b$ bits.
\end{compactitem}
