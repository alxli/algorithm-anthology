...
\textbf{Time Complexity}: $O(\log ans)$ calls to \inlinecode{pred()}, where \inlinecode{ans} is the distance from \inlinecode{lo} to the first true value.
...
Maintains the minimum or maximum value in a sliding window using a monotone queue. This is useful for dynamic programming recurrences where each transition may only come from one of the last $w$ states, such as \inlinecode{dp[i] = a[i] + min(dp[j])} over \inlinecode{i - w \textless{}= j \textless{} i}.
...
\begin{compactitem}
  ...
  \item \inlinecode{add\_line(m, b)} inserts line $y = mx + b$. The slope \inlinecode{m} must be less than or equal to the slope of every line added so far.
...
\begin{compactitem}
  ...
  \item \inlinecode{add\_line(m, b)} inserts line $y = mx + b$ in arbitrary order.
...
\begin{compactitem}
  ...
  \item \inlinecode{add\_line(m, b)} inserts line $y = mx + b$.
...
\begin{compactitem}
  \item \inlinecode{simplex\_solve(a, b, c, \&x)} solves the linear programming problem for an $m$ by $n$ matrix \inlinecode{a} of real values, a length $m$ vector \inlinecode{b}, and a length $n$ vector \inlinecode{c}, returning 0 if a solution was found or $-1$ if there are no solutions. If a solution is found, then the vector pointed to by \inlinecode{x} is populated with the solution values for basic (nonzero) variables only; non-basic variables have value 0 and are omitted. The caller should zero-initialize \inlinecode{x} to length $n$ before calling if a dense solution vector is needed.
\end{compactitem}
