...
\begin{compactitem}
  ...
  \item \inlinecode{get(lo, hi)} returns the hash of the half-open subsequence $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
  \item \inlinecode{hash(lo, hi)} returns the hash of the half-open iterator range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to the constructor or \inlinecode{hash()}, where $n$ is the sequence length.
  \item $O(1)$ per call to \inlinecode{get()} and \inlinecode{concat()}.
\end{compactitem}
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(1)$ expected per call to \inlinecode{get()} and \inlinecode{toggle()}.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(L + z \log m)$ expected per call to the constructor, where $L$ is the total length of the $m$ needles and $z$ is the total number of inherited output entries.
...
Operators may be non-empty strings of any length, but should not contain any parentheses or shared characters with the string representations of operands. Ideally, operators should not be prefixes or suffixes of one another, else the tokenization process may be ambiguous. For example, if \inlinecode{++} and \inlinecode{+} are both operators, then \inlinecode{++} may be split into either [\inlinecode{"+"}, \inlinecode{"+"}] or [\inlinecode{"++"}] depending on the lexicographical ordering of conflicting operators.
