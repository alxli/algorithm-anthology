...
Common string functions, many of which already have standard STL equivalents. Most of the following implementations are presented for educational purposes, and are not heavy optimized. They often depend on certain \inlinecode{std::string} functions that have unspecified complexity.
...
\begin{compactitem}
  ...
  \item \inlinecode{ljust(s, width, ch)} returns \inlinecode{s} left-justified (padded on the right with \inlinecode{ch}) to at least \inlinecode{width} characters, like Python's \inlinecode{str.ljust}.
  \item \inlinecode{rjust(s, width, ch)} returns \inlinecode{s} right-justified (padded on the left with \inlinecode{ch}) to at least \inlinecode{width} characters, like Python's \inlinecode{str.rjust}.
  \item \inlinecode{center(s, width, ch)} returns \inlinecode{s} centered in a field of at least \inlinecode{width} characters by padding both sides with \inlinecode{ch}, like Python's \inlinecode{str.center}. If the padding is uneven, the extra character goes on the right.
...
\begin{compactitem}
  \item \inlinecode{RollingHash\textless{}T, ValueHasher\textgreater{}(first, last)} constructs prefix hashes for any iterator range of values accepted by the value hasher.
  \item \inlinecode{RollingHash\textless{}T, ValueHasher\textgreater{}(v)} constructs prefix hashes for vector \inlinecode{v}.
...
\begin{compactitem}
  \item \inlinecode{ZobristHash\textless{}T\textgreater{}(seed)} constructs a token generator with initial value \inlinecode{seed}.
...
\begin{compactitem}
  ...
  \item \inlinecode{min\_rotation(s)} returns the 0-indexed position \inlinecode{p} such that the rotation beginning at \inlinecode{p}, that is, \inlinecode{s[p]}, \inlinecode{s[(p + 1) \% n]}, ..., \inlinecode{s[(p + n - 1) \% n]}, is lexicographically smallest. If several rotations tie, the smallest such \inlinecode{p} is returned.
\end{compactitem}
...
Solves basic palindrome dynamic programming problems on a string. The core interval state considers substring $[{\inlinecodemath{l}}, {\inlinecodemath{r}}]$: if the two endpoints match, they can wrap an optimal answer for the inside substring; otherwise, at least one endpoint must be skipped or inserted around the other side. Since each state depends on shorter substrings, intervals are processed by increasing length.
