...
This is useful for answer-search problems over unbounded or very large integer domains. The predicate must become true before the exponentially growing probe overflows \inlinecode{Int}; otherwise, add an explicit limit.
...
Coefficient vectors must be nonempty and omit trailing zero coefficients. \inlinecode{find\_one\_root()} requires a polynomial of degree at least one and scales its coefficients by their largest magnitude so a common coefficient factor does not affect convergence.
...
Root-finding inputs must contain at least one nonzero coefficient. After normalization, trailing coefficients no larger than \inlinecode{ZERO\_EPS} relative to the largest coefficient are discarded. \inlinecode{eval\_with\_derivative()} requires a nonempty coefficient vector.
