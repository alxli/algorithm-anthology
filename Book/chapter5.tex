...
\begin{compactitem}
  \item \inlinecode{ternary\_search\_min(lo, hi, f)} and \inlinecode{ternary\_search\_max(lo, hi, f)} return a \inlinecode{double} \inlinecode{x} in \inlinecode{[lo, hi]} optimizing the continuous unimodal function \inlinecode{f}, to within the optional absolute error \inlinecode{eps}.
  ...
  \item \inlinecode{discrete\_ternary\_min(lo, hi, f)} and \inlinecode{discrete\_ternary\_max(lo, hi, f)} return the integer index in the inclusive range \inlinecode{[lo, hi]} optimizing the unimodal function \inlinecode{f}, breaking ties toward the smaller index.
\end{compactitem}
...
Given a continuous function $f(x, y)$ returning a real number and a (possibly arbitrary) starting guess \inlinecode{(x0, y0)}, search for a global minimum using the hill-climbing heuristic.
...
The matrix is supplied as a function \inlinecode{get(r, c)}, and the result reports column indices, so on ties the smallest qualifying column is chosen. For row maxima, negate the oracle or reverse its comparison.

\begin{compactitem}
  \item \inlinecode{smawk\_row\_minima(R, C, get)} returns a vector \inlinecode{arg} of length \inlinecode{R}, where \inlinecode{arg[r]} is the column index minimizing \inlinecode{get(r, c)} over $0 \leq {\inlinecodemath{c}} < {\inlinecodemath{C}}$. The matrix \inlinecode{get(r, c)} must be totally monotone, with \inlinecode{R} $\geq 0$ and \inlinecode{C} $\geq 1$.
\end{compactitem}
