...
Given a continuous function \inlinecode{f(x, y)} to \inlinecode{double} and a (possibly arbitrary) starting guess \inlinecode{(x0, y0)}, return a potential global minimum found through hill-climbing. Optionally, two \inlinecode{double} pointers \inlinecode{critical\_x} and \inlinecode{critical\_y} may be passed to store the input points to \inlinecode{f} at which the returned minimum value is attained.
