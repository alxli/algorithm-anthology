...
\begin{compactitem}
  \item $O(\log n)$ calls to \inlinecode{pred()} per call to \inlinecode{binary\_search\_first\_true()} and \inlinecode{binary\_search\_last\_true()}, where $n$ is the distance between \inlinecode{lo} and \inlinecode{hi}.
  \item $O(n)$ calls to \inlinecode{pred()} per call to \inlinecode{fbinary\_search()}, where $n$ is the number of iterations.
\end{compactitem}
