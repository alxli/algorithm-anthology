...
\section{\texorpdfstring{Binary Search}{5.1 Binary Search}}
...
\textbf{Time Complexity}: $O(\log n)$ comparisons per call, where $n$ is the size of \inlinecode{a}.
...
\textbf{Time Complexity}: $O(\log d)$ per call to \inlinecode{add\_line()} and \inlinecode{query()}, where $d$ is the distance between \inlinecode{lo} and \inlinecode{hi}.
...
\textbf{Time Complexity}: $O(n)$ calls to \inlinecode{f()} per call to \inlinecode{bisection\_root()} and \inlinecode{falsi\_illinois\_root()}, where $n$ is the number of iterations.
