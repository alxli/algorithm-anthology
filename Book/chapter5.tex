...
\begin{compactitem}
  \item \inlinecode{anneal\_min(f, x0, y0, best\_x = nullptr, best\_y = nullptr)} returns a small value found by the randomized search starting from $({\inlinecodemath{x0}}, {\inlinecodemath{y0}})$. If the optional pointers \inlinecode{best\_x} and \inlinecode{best\_y} are supplied, the point attaining the returned value is stored through them.
...
Overflow warning: border updates subtract slopes/intercepts and \inlinecode{query()} evaluates \inlinecode{m * x + b}, so those intermediate values must fit in \inlinecode{int64\_t}.
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n \log d)$ worst-case node storage for $n$ lines inserted, usually much less.
