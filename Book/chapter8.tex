...
\begin{compactitem}
  ...
  \item PBDS priority queues: similar to standard priority queues for \inlinecode{top}/\inlinecode{push}/\inlinecode{pop}, but with extra non-standard handles for \inlinecode{modify} and \inlinecode{erase}, and \inlinecode{join} for melding. Exact bounds depend on the heap tag; for \inlinecode{PairingHeap}, \inlinecode{push} and \inlinecode{join} are $O(1)$ amortized, while \inlinecode{pop}, \inlinecode{modify}, and \inlinecode{erase} are $O(\log n)$ amortized.
\end{compactitem}
