...
\begin{compactitem}
  ...
  \item \inlinecode{floor\_div(a, b)} and \inlinecode{ceil\_div(a, b)} divide signed integers with mathematical rounding toward negative or positive infinity. Requires \inlinecode{b != 0} and a quotient representable by the result type; in particular, the minimum value cannot be divided by $-1$.
...
The generated test type must be copyable so each solution receives an independent input, and both solutions must return comparable results.
