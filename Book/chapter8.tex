...
\begin{compactitem}
  ...
  \item \inlinecode{rng.uniform\_int(lo, hi)} returns a random integer in range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$.
  \item \inlinecode{rng.uniform\_real(lo = 0.0, hi = 1.0)} returns a random real number in the half-open range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
...
\begin{compactitem}
  \item \inlinecode{FastInput in(file = stdin)} reads tokens from a \inlinecode{FILE*}.
  ...
  \item \inlinecode{FastOutput out(file = stdout)} writes to a \inlinecode{FILE*}.
...
\begin{compactitem}
  ...
  \item \inlinecode{benchmark(name, iterations, f)} runs \inlinecode{f()} \inlinecode{iterations} times and returns the average elapsed milliseconds per call with the given name. \inlinecode{iterations} must be positive.
\end{compactitem}
