...
\begin{compactitem}
  ...
  \item \inlinecode{clmp(x, a, b)} returns \inlinecode{x} clamped into the interval [\inlinecode{a}, \inlinecode{b}]. This is not needed on C++17 and later, where std::clamp() is available.
  ...
  \item \inlinecode{rng.uniform\_int(lo, hi)} returns a random integer in the inclusive range [\inlinecode{lo}, \inlinecode{hi}].
  \item \inlinecode{rng.uniform\_real(lo, hi)} returns a random real number in the half-open range [\inlinecode{lo}, \inlinecode{hi}).
  \item \inlinecode{rng.shuffle(lo, hi)} randomly shuffles the range [\inlinecode{lo}, \inlinecode{hi}).
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{OrderedSet\textless{}T\textgreater{}} behaves like a set with \inlinecode{find\_by\_order(k)} and \inlinecode{order\_of\_key(x)}.
  \item \inlinecode{OrderedMap\textless{}K, V\textgreater{}} behaves like a map with \inlinecode{find\_by\_order(k)} and \inlinecode{order\_of\_key(x)}.
  \item \inlinecode{HashMap\textless{}K, V\textgreater{}} and \inlinecode{HashSet\textless{}K\textgreater{}} are fast hash tables using \inlinecode{gp\_hash\_table}. The default \inlinecode{SplitMix64Hasher} is for integer-like keys; pass a custom hash for other key types. It randomizes the hash seed to avoid weak adversarial integer hashes.
  \item \inlinecode{OrderedMultiset\textless{}T\textgreater{}} stores duplicates by pairing each value with a unique ID.
  \item \inlinecode{OrderedMultimap\textless{}K, V\textgreater{}} stores duplicate keys by pairing each key with a unique ID.
