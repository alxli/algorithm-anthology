...
\begin{compactitem}
  ...
  \item \inlinecode{clmp(x, a, b)} returns \inlinecode{x} clamped into the interval \inlinecode{[a, b]}. This is not needed on C++17 and later, where std::clamp() is available.
  \item \inlinecode{ceil\_div(a, b)} and \inlinecode{floor\_div(a, b)} divide signed integers with mathematical rounding toward positive or negative infinity. Requires \inlinecode{b != 0}.
...
\begin{compactitem}
  ...
  \item \inlinecode{hash\_map\textless{}K, V\textgreater{}} and \inlinecode{hash\_set\textless{}K\textgreater{}} are fast hash tables using \inlinecode{gp\_hash\_table}. The default \inlinecode{splitmix64\_hash} is for integer-like keys; pass a custom hash for other key types. It randomizes the hash seed to avoid weak adversarial integer hashes.
...
Simple bump allocators for local contest code. These are useful when creating many small objects and individual \inlinecode{new}/\inlinecode{delete} overhead is measurable. They allocate from one fixed buffer and do not reclaim individual objects.

\begin{compactitem}
  \item The global \inlinecode{operator new} override sends all dynamic allocations through a static buffer. This is the shortest version, but it affects the whole program and should be used only when you are sure the fixed buffer is large enough.
