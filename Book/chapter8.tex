...
\begin{compactitem}
  ...
  \item \inlinecode{lb(a, x)} and \inlinecode{ub(a, x)} return the indices of the first elements in sorted container \inlinecode{a} that are not less than or are greater than \inlinecode{x}, respectively.
  \item \inlinecode{sort\_unique(v)} sorts a vector and removes duplicates.
  \item \inlinecode{indices(n)} returns the vector \inlinecode{\{0, 1, ..., n - 1\}}.
  \item \inlinecode{argsort(a)} returns the indices of \inlinecode{a} in ascending order of their values.
  ...
  \item \inlinecode{rng} is a global 64-bit Mersenne Twister seeded from the current \inlinecode{std::chrono::steady\_clock} reading; call \inlinecode{rng.seed(seed)} for reproducible results.
  \item \inlinecode{rand\_int(lo, hi)} returns a random integer in range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}}]$.
  \item \inlinecode{rand\_real(lo = 0.0, hi = 1.0)} returns a random real number in the half-open range $[{\inlinecodemath{lo}}, {\inlinecodemath{hi}})$.
...
File redirection and detected read/write failures throw \inlinecode{std::runtime\_error}. End-of-file is not an error, and the parser otherwise assumes valid input. Destruction flushes output on a best-effort basis; call \inlinecode{flush()} explicitly to detect a final write failure. Floating point input/output is provided for convenience, not as the main performance path.
...
\begin{compactitem}
  ...
  \item \inlinecode{pr(a, b, c)} prints only the values to \inlinecode{std::cerr} when \inlinecode{LOCAL} is defined; \inlinecode{pr()} prints a blank line.
...
All four printing macros evaluate to no output when \inlinecode{LOCAL} is not defined.
...
\begin{compactitem}
  ...
  \item \inlinecode{rand\_graph(n, m, directed = false)} returns a uniformly random simple graph with \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$ and exactly \inlinecode{m} edges. It uses expected $O(m)$ time and $O(m)$ space.
