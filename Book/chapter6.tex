...
The triangulation may not exist (e.g. for a set of collinear points), or may not be unique if it does exist. The following program assumes its existence and produces one such valid result using \inlinecode{TABLE\_DELAUNAY}, a divide and conquer algorithm with linear merging. Its fully documented version along with debugging messages for the current \inlinecode{assert()} calls may be found at the following link: http://people.sc.fsu.edu/\textasciitilde{}jburkardt/f\_src/table\_delaunay/table\_delaunay.html
