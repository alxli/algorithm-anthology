...
\begin{compactitem}
  \item \inlinecode{directed\_mst(n, root, edges)} returns the total weight of the minimum spanning arborescence rooted at \inlinecode{root} over \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$, or $-1$ if some node is unreachable from the root (no arborescence exists). Edges are given as $({\inlinecodemath{from}}, {\inlinecodemath{to}}, {\inlinecodemath{weight}})$ triples; parallel edges and self-loops are allowed, with self-loops simply ignored.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{gomory\_hu(n, edges)} returns the ${\inlinecodemath{n}} - 1$ edges of a Gomory-Hu tree as $({\inlinecodemath{u}}, {\inlinecodemath{v}}, {\inlinecodemath{weight}})$ tripes for an undirected, weighted graph with \inlinecode{n} nodes and \inlinecode{edges} of the same shape.
