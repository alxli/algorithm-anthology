...
\begin{compactitem}
  ...
  \item \inlinecode{floyd\_warshall()} updates the global adjacency matrix \inlinecode{dist} so \inlinecode{dist[u][v]} stores the shortest-path distance from $u$ to $v$, and updates \inlinecode{next\_node} for path reconstruction. If the graph contains negative-weight cycles, there is no shortest path and an error will be thrown.
