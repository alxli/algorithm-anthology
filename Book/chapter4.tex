...
\begin{compactitem}
  \item \inlinecode{dijkstra(start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated adjacency list \inlinecode{adj} whose nodes are numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Each edge is stored as $({\inlinecodemath{neighbor}}, {\inlinecodemath{weight}})$, where \inlinecode{weight} is nonnegative.
\end{compactitem}

For path reconstruction, \inlinecode{pred[v]} stores the node immediately before \inlinecode{v} on the shortest path from \inlinecode{start} to \inlinecode{v}, or $-1$ if \inlinecode{v} is \inlinecode{start} or unreachable. Follow \inlinecode{pred} backward from the destination to \inlinecode{start}, then reverse that sequence to recover the path.

\textbf{Time Complexity}: $O(n + m \log n)$ per call, where $n$ is the number of nodes and $m$ is the number of edges.
...
\begin{compactitem}
  \item \inlinecode{gomory\_hu(n, edges)} returns the \inlinecode{n - 1} edges of a Gomory-Hu tree as $({\inlinecodemath{u}}, {\inlinecodemath{v}}, {\inlinecodemath{weight}})$ triplets for an undirected, weighted graph with \inlinecode{n} nodes and \inlinecode{edges} of the same shape.
