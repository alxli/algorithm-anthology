...
\begin{compactitem}
  ...
  \item \inlinecode{operator[n]} returns a reference to the adjacency list (\inlinecode{std::vector\textless{}int\textgreater{}}) of node \inlinecode{n}.
...
Given a directed or undirected graph, find simple cycles encountered by a depth-first search. The free functions return one cycle of a simple graph as a list of vertices. For undirected graphs, an edge to a visited non-parent vertex closes a cycle. For directed graphs, only an edge to a vertex still on the recursion stack closes a cycle; edges to already-finished vertices do not.

Both functions remember the DFS parent of each vertex. When a closing edge \inlinecode{u} $\to$ \inlinecode{cycle\_start} is found, the answer is reconstructed by walking parents backward from \inlinecode{u} to \inlinecode{cycle\_start}. In \inlinecode{find\_cycle\_directed()}, \inlinecode{state[v]} is $0$ for unvisited vertices, $1$ for vertices currently on the recursion stack, and $2$ for finished vertices.

\begin{compactitem}
  \item \inlinecode{find\_cycle\_undirected(adj)} returns one vertex cycle in a simple undirected graph, or an empty vector if no cycle exists.
  \item \inlinecode{find\_cycle\_directed(adj)} returns one vertex cycle in a simple directed graph, or an empty vector if no cycle exists.
\end{compactitem}
...
\end{lstlisting}
The \inlinecode{CycleFinder} class is the more general multigraph variant. Its cycles are returned as edge IDs rather than vertex lists, which makes the representation robust for parallel edges. In an undirected graph, the cycles found this way form a DFS cycle basis: one cycle for each non-tree edge. In a directed graph, this finds cycles created by back edges to vertices still on the recursion stack. Here \inlinecode{state[v]} is $-1$ before discovery, $-2$ after finishing, and otherwise the index where the vertex first appeared in the current edge stack; that index marks which stack suffix forms a cycle.
...
\begin{compactitem}
  ...
  \item $O(k)$ auxiliary heap space for \inlinecode{cycle\_vertices()}, where $k$ is the number of edges in the cycle.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{subtree\_small\_to\_large\_merge(adj, root, init, merge, answer)} visits the tree rooted at \inlinecode{root}. The callback \inlinecode{init(u)} returns the initial summary for node \inlinecode{u}; \inlinecode{merge(big, small)} absorbs one child summary into another; and \inlinecode{answer(u, summary)} records the final subtree summary for \inlinecode{u}.
\end{compactitem}

The input must be a tree. Summary objects must support \inlinecode{.size()}, and \inlinecode{merge(big, small)} should iterate over \inlinecode{small} and insert into \inlinecode{big}.

\textbf{Time Complexity}: $O(n \log n)$ expected moved items per call, where $n$ is the number of nodes.

\textbf{Space Complexity}: $O(n)$ auxiliary stack space and $O(n)$ auxiliary heap space.
...
\begin{compactitem}
  \item \inlinecode{known\_eulerian\_path\_directed(adj, start)} returns a vertex trail using every directed edge in \inlinecode{adj} exactly once, assuming such a trail exists and begins at \inlinecode{start}.
  \item \inlinecode{eulerian\_path\_directed(adj, start = -1)} returns a vertex trail using every directed edge in \inlinecode{adj} exactly once, or an empty vector if no such trail exists. If \inlinecode{start} $= -1$, a valid start is chosen automatically; otherwise the trail must begin at \inlinecode{start}.
\end{compactitem}

Parallel directed edges are supported by storing duplicate neighbors in \inlinecode{adj}. Since these functions return only vertices, use \inlinecode{EulerianGraph} below when edge IDs are needed.
...
\begin{compactitem}
  \item \inlinecode{bfs(start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{bfs\_zero\_one(start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Each edge is stored as $({\inlinecodemath{neighbor}}, {\inlinecodemath{weight}})$, where \inlinecode{weight} is either $0$ or $1$.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{dijkstra(adj, start)} returns a pair of vectors \inlinecode{dist} and \inlinecode{pred} for an adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Each edge is stored as $({\inlinecodemath{neighbor}}, {\inlinecodemath{weight}})$, where \inlinecode{weight} is nonnegative.
\end{compactitem}

For path reconstruction, \inlinecode{pred[v]} stores the node immediately before \inlinecode{v} on the shortest path from \inlinecode{start} to \inlinecode{v}, or $-1$ if \inlinecode{v} is \inlinecode{start} or unreachable. Follow \inlinecode{pred} backward from the destination to \inlinecode{start}, then reverse that sequence to recover the path.

\textbf{Time Complexity}: $O(m \log n)$ per call, where $m$ is the number of edges and $n$ is the number of nodes.
...
\begin{compactitem}
  \item \inlinecode{spfa(start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Each edge is stored as $({\inlinecodemath{neighbor}}, {\inlinecodemath{weight}})$. The function returns \inlinecode{false} if it detects a reachable negative cycle, and returns \inlinecode{true} otherwise.
\end{compactitem}

For path reconstruction, \inlinecode{pred[v]} stores the node immediately before \inlinecode{v} on the shortest path from \inlinecode{start} to \inlinecode{v}, or $-1$ if \inlinecode{v} is \inlinecode{start} or unreachable. Follow \inlinecode{pred} backward from the destination to \inlinecode{start}, then reverse that sequence to recover the path.

\textbf{Time Complexity}: $O(n \cdot m)$ per call in the worst case, where $n$ is the number of nodes and $m$ is the number of edges.
...
\begin{compactitem}
  ...
  \item $O(n^{2})$ auxiliary for \inlinecode{init\_floyd()} and \inlinecode{floyd\_warshall()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{prim\_mst()} populates \inlinecode{mst} with the edge IDs in the minimum spanning tree (returning the total MST weight) for a global, bidirectionally pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Edges are stored as (\inlinecode{neighbor}, \inlinecode{weight}, \inlinecode{edge\_id}).
\end{compactitem}
...
\textbf{Time Complexity}: $O(m \log n)$ per call, where $m$ is the number of edges and $n$ is the number of nodes. The internal DSU uses path compression and union by size, making each DSU operation $O(\alpha\left(n\right))$ amortized.
...
\begin{compactitem}
  \item \inlinecode{KruskalReconstructionTree(n, edges)} builds the tree from weighted edges stored as \inlinecode{(weight, u, v)}.
...
\begin{compactitem}
  \item \inlinecode{directed\_mst(n, root, edges)} returns the total weight of the minimum spanning arborescence rooted at \inlinecode{root} over \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$, or $-1$ if some node is unreachable from the root (no arborescence exists). Edges are given as (\inlinecode{from}, \inlinecode{to}, \inlinecode{weight}) triples; parallel edges and self-loops are allowed, with self-loops simply ignored.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{max\_flow(source, sink)} modifies the global residual capacity matrix \inlinecode{cap} and returns maximum flow. Nodes are numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{cap.size()}.
\end{compactitem}
...
Given a directed network where each edge must carry at least \inlinecode{lo} and at most \inlinecode{hi} units of flow, find whether the lower bounds can be satisfied, and optionally optimize an \inlinecode{s}-to-\inlinecode{t} flow value. Subtract each lower bound from its edge capacity, then record the forced imbalance: \inlinecode{lo} units must leave \inlinecode{u} and enter \inlinecode{v}. A super-source supplies every node with net demand, and every node with net surplus sends that surplus to a super-sink. The bounds are feasible exactly when all such auxiliary edges can be saturated.

For \inlinecode{s}-to-\inlinecode{t} flow, add an infinite edge from \inlinecode{t} back to \inlinecode{s} before checking feasibility. The flow on that artificial edge is one feasible value. After removing the auxiliary nodes, augmenting from \inlinecode{s} to \inlinecode{t} maximizes the value; augmenting from \inlinecode{t} to \inlinecode{s} cancels as much flow as possible and minimizes it.
...
\begin{compactitem}
  ...
  \item \inlinecode{max\_flow(source, sink)} returns the maximum feasible \inlinecode{source}-to-\inlinecode{sink} flow, or $-1$ if no feasible flow exists.
  \item \inlinecode{min\_flow(source, sink)} returns the minimum feasible \inlinecode{source}-to-\inlinecode{sink} flow, or $-1$ if no feasible flow exists.
  \item \inlinecode{edge\_flows()} returns one feasible flow value for each original edge after a successful call.
\end{compactitem}

All capacities should be nonnegative integers and \inlinecode{lo} $\leq$ \inlinecode{hi}. Choose \inlinecode{INF} larger than any possible finite flow value.

\textbf{Time Complexity}: $O(n^{2} \cdot m)$ per feasibility check or optimized flow call, where $n$ is the number of nodes and $m$ is the number of edges.
...
\begin{compactitem}
  \item \inlinecode{gomory\_hu(n, edges)} returns the ${\inlinecodemath{n}} - 1$ edges of a Gomory-Hu tree as $({\inlinecodemath{u}}, {\inlinecodemath{v}}, {\inlinecodemath{weight}})$ triplets for an undirected, weighted graph with \inlinecode{n} nodes and \inlinecode{edges} of the same shape.
...
\textbf{Time Complexity}: $O(m \cdot (n_1 + n_2))$ per call, where $m$ is the number of edges.

\textbf{Space Complexity}: $O(n_1 + n_2)$ auxiliary stack space per call.
...
Hopcroft-Karp augments along many shortest augmenting paths per phase. Each phase first runs a breadth-first search to layer the graph by distance, then a depth-first search to find a maximal set of vertex-disjoint shortest augmenting paths to flip at once, for $O(m \cdot \sqrt{n_1 + n_2})$ running time overall.

\begin{compactitem}
  \item \inlinecode{bipartite\_matching\_hk(n2)} populates \inlinecode{match} and returns maximum matching size for a global, pre-populated adjacency list \inlinecode{adj} whose left-side nodes are numbered $[0, {\inlinecodemath{n}})$ and whose right-side neighbors are numbered $[0, {\inlinecodemath{n2}})$, where \inlinecode{n} is \inlinecode{adj.size()}.
\end{compactitem}
...
\textbf{Time Complexity}: $O(m \cdot (n_1 + n_2))$ per call, dominated by the matching, where $m$ is the number of edges.

\textbf{Space Complexity}: $O(n_1 + n_2)$ auxiliary.
...
\begin{compactitem}
  \item \inlinecode{min\_assignment\_cost(cost, \&assignment)} returns the minimum total assignment cost for a 0-based \inlinecode{cost} matrix, while populating \inlinecode{assignment} with $n$ values where \inlinecode{assignment[i]} is the chosen job for worker \inlinecode{i}, using 0-based job numbers.
\end{compactitem}

\textbf{Time Complexity}: $O(n^{2} \cdot m)$ per call, where \inlinecode{cost} has $n$ rows and $m$ columns.

\textbf{Space Complexity}: $O(n + m)$ auxiliary, not counting the input matrix and returned assignment.
...
\textbf{Time Complexity}: $O(n^{2} \cdot m)$ per call in the worst case, where $n$ is the number of nodes and $m$ the number of edges; substantially faster in practice.
