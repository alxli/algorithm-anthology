...
\begin{compactitem}
  \item $O(1)$ amortized per call to \inlinecode{add\_edge()} when no resize is required; $O(r)$ for a call that grows the graph to $r$ nodes.
...
An Eulerian trail is a path in a graph which contains every edge exactly once. An Eulerian cycle or circuit is an Eulerian trail which begins and ends on the same node. A directed graph has an Eulerian trail when all nonzero-degree nodes belong to one connected part of the underlying graph, and either every node has equal in-degree and out-degree or exactly one node has one extra outgoing edge and exactly one node has one extra incoming edge.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(T \log T + m \log T \log n)$ per call to \inlinecode{solve()}, where $T$ is the total number of operations (\inlinecode{add\_edge()}, \inlinecode{remove\_edge()}, and \inlinecode{count\_components()} combined), $m$ is the number of \inlinecode{add\_edge()} operations, and $n$ is the number of nodes.
...
\begin{compactitem}
  ...
  \item \inlinecode{get\_path(dest)} returns the path from a nearest starting node to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable, using the state left by the most recent call to \inlinecode{bfs\_zero\_one()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{floyd\_warshall()} updates the global adjacency matrix \inlinecode{dist} so \inlinecode{dist[u][v]} stores the shortest-path distance from $u$ to $v$, updates \inlinecode{next\_node} for path reconstruction, and returns whether the graph contains no negative-weight cycle.
...
\textbf{Space Complexity}: $O(n \log n)$ for the reconstruction tree and LCA table, plus $O(m)$ auxiliary during construction.
