...
\begin{compactitem}
  \item \inlinecode{build\_centroid\_tree()} populates \inlinecode{subtree\_size}, \inlinecode{removed}, and \inlinecode{centroid\_parent} for the global, bidirectionally pre-populated adjacency list \inlinecode{adj} which must form a valid tree with nodes numbered from 0 to \inlinecode{adj.size() - 1}.
...
\begin{compactitem}
  ...
  \item \inlinecode{find\_dominators(start)} returns a vector \inlinecode{idom} where \inlinecode{idom[start] == start}, \inlinecode{idom[v]} is the immediate dominator of reachable node \inlinecode{v}, and \inlinecode{idom[v] == -1} if \inlinecode{v} is unreachable.
\end{compactitem}
...
Both preference lists are given as \inlinecode{n} by \inlinecode{n} matrices. \inlinecode{men\_pref[m]} lists the women in \inlinecode{m}'s order of preference, most preferred first, and \inlinecode{women\_pref[w]} lists the men in \inlinecode{w}'s order of preference.
