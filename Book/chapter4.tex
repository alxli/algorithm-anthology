...
\begin{compactitem}
  ...
  \item \inlinecode{diameter()} returns a tuple (\inlinecode{length}, \inlinecode{u}, \inlinecode{v}), where \inlinecode{length} is the maximum distance between any two nodes in the tree and \inlinecode{u} and \inlinecode{v} are endpoints attaining that distance. It uses a well-known double DFS technique.
\end{compactitem}
...
Given a tree DP that can answer one rooted query with a recursive \inlinecode{dfs(u, p)}, compute that same answer for every possible root. Rerooting DP first stores parent pointers and preorder from an arbitrary root. A bottom-up pass computes the usual child-to-parent DFS values, then a top-down pass uses prefix/suffix aggregation to compute the missing parent-to-child values. The prefix/suffix aggregation requires \inlinecode{combine} to be associative for a running time of $O(n)$, which is usually preferred, but a segment-tree/exclusive-combine variant can support a left-fold style interface in $O(n \log n)$.

\begin{compactitem}
  \item \inlinecode{rerooting\_dp(identity, combine, finalize, lift)} returns the value that \inlinecode{dfs(root, p)} would produce for every possible \inlinecode{root} with sentinel parent $p = -1$. The global, bidirectionally pre-populated adjacency list \inlinecode{adj} must form a nonempty tree whose indices represent its nodes.
\end{compactitem}
