...
\begin{compactitem}
  \item \inlinecode{CentroidDecomposition(adj)} builds the centroid decomposition of the tree given by a bidirectional adjacency list \inlinecode{adj}, which must form a valid tree and whose indices represent the nodes.
...
\begin{compactitem}
  \item \inlinecode{encode\_prufer()} returns the prufer code for the global, bidirectionally pre-populated adjacency list \inlinecode{adj}, which must form a valid tree and whose indices represent the nodes.
...
\begin{compactitem}
  \item \inlinecode{bfs\_zero\_one(starts)} populates \inlinecode{dist} and \inlinecode{pred} for shortest paths from the nodes in \inlinecode{starts}, assigning each node its minimum distance from any start. The global, pre-populated adjacency list \inlinecode{adj} uses its indices as nodes. Each edge is stored as (\inlinecode{neighbor}, \inlinecode{weight}), where \inlinecode{weight} is either $0$ or $1$. Pass a singleton vector for a single source.
  \item \inlinecode{get\_path(dest)} returns the path from a nearest starting node to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable, after the latest call to \inlinecode{bfs\_zero\_one()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{push\_relabel(source, sink)} returns maximum flow for a global capacity matrix \inlinecode{cap}, whose row and column indices represent the nodes.
...
\begin{compactitem}
  \item \inlinecode{color\_graph()} populates \inlinecode{color} and returns minimum color count for a global, bidirectionally pre-populated adjacency matrix \inlinecode{adj}, whose row and column indices represent the nodes.
\end{compactitem}
