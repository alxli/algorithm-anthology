...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary stack and heap space for the DFS and returned cycle.
\end{compactitem}
...
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for storage of the graph.
  \item $O(\max\left(n, s\right))$ auxiliary stack and heap space for \inlinecode{find\_cycles()}, where $s$ is the total size of the returned cycles.
...
\textbf{Space Complexity}: $O(\max\left(n, m\right))$ auxiliary stack and heap space for \inlinecode{known\_eulerian\_path\_directed()} and \inlinecode{eulerian\_path\_directed()}.
