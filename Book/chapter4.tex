...
Given a weighted, directed graph with possibly negative weights, determine the minimum distance between all pairs of start and destination nodes in the graph. Optionally, output the shortest path between two nodes using the shortest-path tree precomputed into the \inlinecode{pred} matrix.

\begin{compactitem}
  \item \inlinecode{initialize(nodes)} initializes \inlinecode{dist} and \inlinecode{pred} for nodes numbered from 0 to \inlinecode{nodes - 1}.
  \item \inlinecode{floyd\_warshall()} updates the global adjacency matrix \inlinecode{dist} so \inlinecode{dist[u][v]} stores the shortest path from $u$ to $v$, and updates \inlinecode{pred} for path reconstruction. If the graph contains negative-weighted cycles, there is no shortest path and an error will be thrown.
\end{compactitem}
