...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n + m \log n)$ per call in the worst case, where $n$ is the number of nodes and $m$ is the number of edges, plus one call to \inlinecode{h()} per queue insertion and removal. A more informed heuristic lowers the constant by expanding fewer nodes, but does not improve the worst case.
  \item $O(p)$ per call to \inlinecode{get\_path()}, where $p$ is the number of nodes in the returned path.
\end{compactitem}
