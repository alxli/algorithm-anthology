...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O((T + m \log T) \log n)$ per call to \inlinecode{solve()}, where $T$ is the total number of operations (\inlinecode{add\_edge}, \inlinecode{remove\_edge}, and \inlinecode{count\_components} combined), $m$ is the number of \inlinecode{add\_edge} operations, and $n$ is the number of nodes.
...
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for storage of the graph, where $n$ is the number of nodes and $m$ is the number of edges.
  \item $O(\max\left(n, m\right))$ auxiliary heap space and $O(\log n)$ auxiliary stack space.
\end{compactitem}
