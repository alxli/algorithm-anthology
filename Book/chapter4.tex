...
A graph consists of a set of nodes and a set of connections (edges) between pairs of nodes. A graph may be stored as an adjacency list, which is a space-efficient representation that is also efficient for traversals.
...
\begin{compactitem}
  ...
  \item \inlinecode{operator[n]} returns a mutable or const reference to the adjacency list (\inlinecode{std::vector\textless{}int\textgreater{}}) of node \inlinecode{n}.
...
\begin{compactitem}
  ...
  \item $O(n)$ auxiliary stack space for the DFS, plus $O(n)$ auxiliary heap space for the returned cycle.
\end{compactitem}
...
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for storage of the graph.
  \item $O(n)$ auxiliary stack space plus $O(s)$ auxiliary heap space for \inlinecode{find\_cycles()}, where $s$ is the total size of the returned cycles.
  \item $O(k)$ auxiliary heap space for \inlinecode{cycle\_nodes()}, where $k$ is the number of edges in the cycle.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{find\_centroid(u = 0, p = -1)} returns the node where all of its subtrees have a size less than or equal to $n / 2$, where $n$ is the number of nodes in the tree.
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(n)$ auxiliary heap space for \inlinecode{find\_centers()}, where $n$ is the number of nodes.
...
\begin{compactitem}
  \item \inlinecode{RerootingTree(n)} constructs a tree with \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$.
  \item \inlinecode{add\_edge(u, v)} adds an undirected edge between nodes \inlinecode{u} and \inlinecode{v}.
  \item \inlinecode{build\_rooted\_tree(root = 0)} populates \inlinecode{parent}, \inlinecode{depth}, \inlinecode{subtree\_size}, \inlinecode{order}, \inlinecode{tin}, and \inlinecode{tout} for the tree rooted at \inlinecode{root}. This must be called only after the added edges define a connected tree of size \inlinecode{n}.
  \item \inlinecode{is\_ancestor(u, v)} returns whether \inlinecode{u} is an ancestor of \inlinecode{v} in the last rooted tree.
  \item \inlinecode{rerooting\_dp(identity, combine, finalize, lift)} returns the value that \inlinecode{dfs(root, p)} would produce for every possible \inlinecode{root} with sentinel parent $p = -1$. This must be called only after the added edges define a connected tree of size \inlinecode{n}.
\end{compactitem}
...
\textbf{Time Complexity}: $O(n \log n)$ expected moved items per call, where $n$ is the number of nodes.
...
\begin{compactitem}
  \item \inlinecode{CentroidDecomposition(adj)} builds the centroid decomposition of the tree given by a bidirectional adjacency list \inlinecode{adj}, which must form a valid tree with \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}.
...
\begin{compactitem}
  \item \inlinecode{encode\_prufer()} returns the prufer code for the global, bidirectionally pre-populated adjacency list \inlinecode{adj} which must form a valid tree with nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}.
  \item \inlinecode{decode\_prufer()} takes a Prufer code and returns the corresponding tree edges.
\end{compactitem}

The encoded tree must contain at least two nodes, since the empty code represents the unique tree on two nodes.
...
\textbf{Space Complexity}: $O(n)$ auxiliary per call to \inlinecode{encode\_prufer()} or \inlinecode{decode\_prufer()}.
...
A Eulerian trail is a path in a graph which contains every edge exactly once. A Eulerian cycle or circuit is an Eulerian trail which begins and ends on the same node. A directed graph has a Eulerian trail when all nonzero-degree nodes belong to one connected part of the underlying graph, and either every node has equal in-degree and out-degree or exactly one node has one extra outgoing edge and exactly one node has one extra incoming edge.
...
\textbf{Space Complexity}: $O(m)$ auxiliary stack space and $O(\max\left(n, m\right))$ auxiliary heap space for \inlinecode{known\_eulerian\_path\_directed()} and \inlinecode{eulerian\_path\_directed()}.
...
\end{lstlisting}
The \inlinecode{EulerianGraph} class is the more general edge-ID variant. An undirected graph has a Eulerian trail when all nonzero-degree nodes are connected and either zero or two nodes have odd degree. This class stores and returns edge IDs, which supports directed graphs, undirected graphs, and multigraphs: parallel edges are distinct because each edge receives its own ID.
...
\begin{compactitem}
  ...
  \item \inlinecode{add\_edge(u, v)} adds an edge and returns its edge ID.
  \item \inlinecode{eulerian\_path(start = -1)} returns a trail using every edge exactly once, or a result with \inlinecode{start = -1} if no such trail exists. If \inlinecode{start = -1}, a valid start is chosen automatically.
...
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for storage of the graph, where $n$ is the number of nodes and $m$ is the number of edges.
  \item $O(n)$ auxiliary stack space per call.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{Dominators(n)} constructs a directed graph of \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$.
...
\textbf{Space Complexity}:
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for the graph and auxiliary heap arrays.
...
\begin{compactitem}
  \item \inlinecode{FunctionalGraph(f)} builds the structure for the successor function \inlinecode{f} over nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{f.size()}.
...
This offline method supports arbitrary undirected graphs. When updates must be processed online and the graph remains a forest, see the Euler Tour Tree in Section 2.7.9.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O((T + m \log T) \log n)$ for \inlinecode{solve()}, where $T$ is the total number of operations (\inlinecode{add\_edge}, \inlinecode{remove\_edge}, and \inlinecode{count\_components} combined), $m$ is the number of \inlinecode{add\_edge} operations, and $n$ is the number of nodes.
  \item $O(1)$ amortized per call to \inlinecode{add\_edge()}, \inlinecode{remove\_edge()}, and \inlinecode{count\_components()}.
\end{compactitem}

\textbf{Space Complexity}: $O(n + m \log T)$ auxiliary.
...
\begin{compactitem}
  \item \inlinecode{bfs(starts)} populates \inlinecode{dist} and \inlinecode{pred} from every node in \inlinecode{starts}, assigning each node its minimum distance from any start. The global, pre-populated adjacency list \inlinecode{adj} must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Pass a singleton vector for single-source BFS.
  \item \inlinecode{get\_path(dest)} returns the path from a nearest starting node to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable, after the latest call to \inlinecode{bfs()}.
\end{compactitem}
...
Given a starting node in a weighted graph whose edge weights are only $0$ or $1$, compute the shortest distance to every reachable node. Optionally, reconstruct the shortest path to a specific destination node using the shortest-path tree from the predecessor array \inlinecode{pred}.
...
\begin{compactitem}
  \item \inlinecode{bfs\_zero\_one(start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Each edge is stored as (\inlinecode{neighbor}, \inlinecode{weight}), where \inlinecode{weight} is either $0$ or $1$.
  \item \inlinecode{get\_path(dest)} returns the path from \inlinecode{start} to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable, after the latest call to \inlinecode{bfs\_zero\_one()}.
\end{compactitem}
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ per call, where $n$ is the number of nodes and $m$ is the number of edges.
  \item $O(p)$ per call to \inlinecode{get\_path()}, where $p$ is the number of nodes in the returned path.
\end{compactitem}
...
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for storage of the graph, where $n$ is the number of nodes and $m$ is the number of edges.
  \item $O(n)$ auxiliary deque space per call.
  \item $O(p)$ for the path returned by \inlinecode{get\_path()}.
\end{compactitem}
...
Given a starting node in a weighted, directed graph with nonnegative weights only, visit every connected node and determine the minimum distance to each such node. Optionally, reconstruct the shortest path to a destination node using the shortest-path tree from the predecessor array \inlinecode{pred}.
...
\begin{compactitem}
  \item \inlinecode{dijkstra(start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated adjacency list \inlinecode{adj} whose nodes are numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Each edge is stored as (\inlinecode{neighbor}, \inlinecode{weight}), where \inlinecode{weight} is nonnegative.
  \item \inlinecode{get\_path(dest)} returns the path from \inlinecode{start} to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable, after the latest call to \inlinecode{dijkstra()}.
\end{compactitem}
...
Given a starting node in a weighted, directed graph with possibly negative weights, visit every connected node and determine the minimum distance to each such node. Optionally, reconstruct the shortest path to a destination node using the shortest-path tree from the predecessor array \inlinecode{pred}.

Bellman-Ford relaxes every edge in the graph up to $n - 1$ times, stopping early if a pass makes no changes. Since any shortest path uses at most $n - 1$ edges, all distances are correct after these passes unless a negative-weight cycle keeps reducing them, which a further pass detects. Bellman-Ford will also detect whether the graph contains a negative-weight cycle reachable from the start node, in which case the affected shortest paths are undefined and an error will be thrown. (To detect a negative cycle anywhere in the graph, add a virtual source with zero-weight edges to every node and start from it.)

\begin{compactitem}
  \item \inlinecode{bellman\_ford(n, start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated edge list \inlinecode{edges} whose endpoints must be numbered $[0, {\inlinecodemath{n}})$.
  \item \inlinecode{get\_path(dest)} returns the path from \inlinecode{start} to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable, after the latest call to \inlinecode{bellman\_ford()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{spfa(start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Each edge is stored as (\inlinecode{neighbor}, \inlinecode{weight}). The function returns \inlinecode{false} if it detects a reachable negative cycle, and returns \inlinecode{true} otherwise.
  \item \inlinecode{get\_path(dest)} returns the path from \inlinecode{start} to \inlinecode{dest} after a successful call to \inlinecode{spfa()}, or an empty vector if \inlinecode{dest} is unreachable.
\end{compactitem}
...
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for storage of the graph, where $n$ is the number of nodes and $m$ is the number of edges.
  \item $O(n)$ auxiliary queue space per call.
  \item $O(p)$ for the path returned by \inlinecode{get\_path()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{dag\_shortest\_path(start)} populates \inlinecode{dist} and \inlinecode{pred} for a global, pre-populated adjacency list \inlinecode{adj} whose nodes are numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Each edge is stored as (\inlinecode{neighbor}, \inlinecode{weight}) and may have any sign. \inlinecode{dist[v]} is set to \inlinecode{INF} for nodes not reachable from \inlinecode{start}, and \inlinecode{pred} stores the shortest-path tree for path reconstruction.
  \item \inlinecode{get\_path(dest)} returns the path from \inlinecode{start} to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable, after the latest call to \inlinecode{dag\_shortest\_path()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{get\_path(start, dest)} returns the shortest path from \inlinecode{start} to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable from \inlinecode{start}, after the latest call to \inlinecode{floyd\_warshall()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(1)$ auxiliary per call to \inlinecode{floyd\_warshall()}.
  \item $O(p)$ for the path returned by \inlinecode{get\_path()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{johnson\_all\_pairs(n)} populates \inlinecode{dist} and \inlinecode{next\_node} for a global, pre-populated edge list \inlinecode{edges} whose nodes are numbered $[0, {\inlinecodemath{n}})$. Each edge is stored as (\inlinecode{u}, \inlinecode{v}, \inlinecode{weight}). Afterward, \inlinecode{dist[u][v]} is the shortest distance from $u$ to $v$, or \inlinecode{INF} if $v$ is unreachable from $u$. An error is thrown if the graph contains a negative-weight cycle.
  \item \inlinecode{get\_path(start, dest)} returns the shortest path from \inlinecode{start} to \inlinecode{dest}, or an empty vector if \inlinecode{dest} is unreachable from \inlinecode{start}, after the latest call to \inlinecode{johnson\_all\_pairs()}.
\end{compactitem}
...
All potentials, reduced weights, and path distances must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  \item \inlinecode{prim\_mst()} populates \inlinecode{mst} with the edges in the minimum spanning forest and returns the total MST weight for a global, bidirectionally pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. Adjacency entries are stored as (\inlinecode{neighbor}, \inlinecode{weight}), while each MST edge is stored as (\inlinecode{from}, \inlinecode{to}, \inlinecode{weight}).
\end{compactitem}
...
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for storage of the graph, where $n$ is the number of nodes and $m$ is the number of edges.
  \item $O(\max\left(n, m\right))$ auxiliary heap space and $O(\log n)$ auxiliary stack space per call.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{KruskalReconstructionTree(n, edges)} builds the tree from weighted edges stored as (\inlinecode{weight}, \inlinecode{u}, \inlinecode{v}).
  \item \inlinecode{max\_edge\_on\_path(u, v)} returns the maximum edge weight on the MST path between original nodes \inlinecode{u} and \inlinecode{v}.
  \item \inlinecode{root()} returns the reconstruction-tree root.
\end{compactitem}

The input graph must be connected. Parallel edges are supported and are handled naturally by Kruskal's algorithm.
...
\textbf{Space Complexity}: $O(n \log n + m)$ auxiliary.
...
\begin{compactitem}
  \item \inlinecode{max\_flow(source, sink)} modifies the global residual capacity matrix \inlinecode{cap} and returns maximum flow. Nodes are numbered $[0, n)$, where $n$ is \inlinecode{cap.size()}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{push\_relabel(source, sink)} returns maximum flow for a global capacity matrix \inlinecode{cap} whose nodes are numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{cap.size()}.
...
For $s$-$t$ flow, add an infinite edge from $t$ back to $s$ before checking feasibility. The flow on that artificial edge is one feasible value. After the feasibility check, augmenting from $s$ to $t$ maximizes the value; augmenting from $t$ to $s$ cancels as much flow as possible and minimizes it.
...
All capacities should be nonnegative integers and \inlinecode{lo} $\leq$ \inlinecode{hi}. Choose \inlinecode{INF} larger than any possible finite flow value. Each feasibility or optimization call starts from the original network, so different variants may be solved successively on the same instance.
...
Overflow warning: the returned cost accumulates products of sent flow and edge cost. Ensure each product and the total cost fit in the cost type \inlinecode{C}.
...
\begin{compactitem}
  \item \inlinecode{bipartite\_matching(n2)} populates \inlinecode{match\_left} and \inlinecode{match\_right}, then returns maximum matching size for a global, pre-populated adjacency list \inlinecode{adj} whose left-side nodes are numbered $[0, {\inlinecodemath{n}})$ and whose right-side neighbors are numbered $[0, {\inlinecodemath{n2}})$, where \inlinecode{n} is \inlinecode{adj.size()}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item $O(n_1 + n_2)$ auxiliary heap space and $O(n_1)$ auxiliary stack space per call.
\end{compactitem}
...
\begin{compactitem}
  \item $O(\max\left(n, m\right))$ for storage of the graph, where $n$ is the number of nodes and $m$ is the number of edges.
  \item $O(n_1 + n_2)$ auxiliary stack space and $O(n_1 + n_2)$ auxiliary heap space per call.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{maximum\_matching(adj)} returns a matching for a bidirectionally pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}. The returned vector \inlinecode{match} has \inlinecode{match[u] == v} and \inlinecode{match[v] == u} when \inlinecode{u} and \inlinecode{v} are matched, or \inlinecode{match[u]} is $-1$ when \inlinecode{u} is unmatched.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{WeightedGeneralMatching(n)} creates a graph of \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$.
...
\begin{compactitem}
  ...
  \item $O(n \cdot m)$ auxiliary per call to \inlinecode{max\_assignment()} for the transformed matrix.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{max\_clique()} returns the maximum clique size for a global, bidirectionally pre-populated adjacency matrix \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}.
  \item \inlinecode{max\_clique\_weighted()} additionally uses global \inlinecode{w} and returns the maximum clique weight.
\end{compactitem}

These implementations use \inlinecode{mask\_t}, which is \inlinecode{uint64\_t} by default, so the number of nodes must be less than \inlinecode{MASK\_BITS}. Node weights must be nonnegative.
...
\begin{compactitem}
  \item \inlinecode{color\_graph()} populates \inlinecode{color} and returns minimum color count for a global, bidirectionally pre-populated adjacency matrix \inlinecode{adj} which must consist of nodes numbered $[0, {\inlinecodemath{n}})$, where \inlinecode{n} is \inlinecode{adj.size()}.
\end{compactitem}
...
\begin{compactitem}
  \item $O(n^{2})$ for storage of the graph, where $n$ is the number of nodes.
  \item $O(n)$ auxiliary stack space and $O(n)$ auxiliary heap space per call.
\end{compactitem}
...
\textbf{Time Complexity}: $O(n^{3} \cdot m)$ per call in the worst case, where $n$ is the number of nodes and $m$ the number of edges; substantially faster in practice.
