...
\begin{compactitem}
  \item \inlinecode{directed\_mst(n, edges, root)} returns the total weight of the minimum spanning arborescence rooted at \inlinecode{root} over \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$, or \inlinecode{std::nullopt} if no arborescence exists. Edges are given as (\inlinecode{from}, \inlinecode{to}, \inlinecode{weight}) triples with valid node indices; parallel edges and self-loops are allowed, with self-loops simply ignored.
\end{compactitem}

Overflow warning: selected edge weights, adjusted weights, and their total must fit in \inlinecode{int64\_t}.
...
\begin{compactitem}
  \item \inlinecode{match\_bipartite(n2)} populates \inlinecode{match\_left} and \inlinecode{match\_right}, then returns maximum matching size for a global, pre-populated adjacency list \inlinecode{adj}, whose indices represent left-side nodes and whose entries contain right-side neighbors numbered $[0, {\inlinecodemath{n2}})$. The two sides are given as separate node numberings. Use the bipartite check of section \secref{4.6.1} to produce them for a graph that does not already arrive presented as two sets.
\end{compactitem}

\textbf{Time Complexity}: $O(n_1 \cdot m)$ per call, where $m$ is the number of edges.
...
\begin{compactitem}
  \item \inlinecode{match\_bipartite\_hk(n2)} populates \inlinecode{match\_left} and \inlinecode{match\_right}, then returns maximum matching size for a global, pre-populated adjacency list \inlinecode{adj}, whose indices represent left-side nodes and whose entries contain right-side neighbors numbered $[0, {\inlinecodemath{n2}})$.
\end{compactitem}

\textbf{Time Complexity}: $O(m \cdot \sqrt{n_1 + n_2})$ per call, where $m$ is the number of edges.
