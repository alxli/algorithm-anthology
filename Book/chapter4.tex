...
\begin{compactitem}
  ...
  \item \inlinecode{build\_scc()} computes the strongly connected components.
  \item \inlinecode{components()} returns the strongly connected components from the last \inlinecode{build\_scc()} call.
  \item \inlinecode{component\_id(v)} returns the component ID containing node \inlinecode{v}. Component IDs are in topological order: for every edge from component $a$ to a different component $b$, $a < b$.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{build\_scc()} computes the strongly connected components.
  \item \inlinecode{components()} returns the strongly connected components from the last \inlinecode{build\_scc()} call.
  \item \inlinecode{component\_id(v)} returns the component ID containing node \inlinecode{v}. Component IDs are in reverse topological order: for every edge from component $a$ to a different component $b$, $a > b$.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{build\_bcc()} computes the articulation points and vertex-biconnected components.
  \item \inlinecode{articulation\_points()} and \inlinecode{components()} return those results.
  \item \inlinecode{build\_block\_cut\_forest()} computes the block-cut forest using the results of the previous \inlinecode{build\_bcc()} call.
  \item \inlinecode{block\_cut\_forest()} returns that forest. Its component nodes are numbered in the range $[0, {\inlinecodemath{components().size()}})$.
...
\begin{compactitem}
  ...
  \item \inlinecode{components()} and \inlinecode{bridge\_forest()} return those results.
  \item \inlinecode{component\_id(u)} returns the 2-edge-connected component ID containing node \inlinecode{u}.
\end{compactitem}
...
Since the search stops as soon as \inlinecode{dest} is expanded, only \inlinecode{dist[dest]} and the \inlinecode{pred} chain back to \inlinecode{start} are final; other entries may remain tentative, so use Dijkstra's algorithm in section 4.3.3 when every distance is needed.
...
\begin{compactitem}
  \item \inlinecode{bipartite\_matching(n2)} populates \inlinecode{match\_left} and \inlinecode{match\_right}, then returns maximum matching size for a global, pre-populated adjacency list \inlinecode{adj}, whose indices represent left-side nodes and whose entries contain right-side neighbors numbered $[0, {\inlinecodemath{n2}})$.
\end{compactitem}

The two sides are given as separate node numberings. Use the bipartite check of section 4.6.1 to produce them for a graph that does not already arrive presented as two sets.
...
\begin{compactitem}
  \item \inlinecode{bipartite\_matching\_hk(n2)} populates \inlinecode{match\_left} and \inlinecode{match\_right}, then returns maximum matching size for a global, pre-populated adjacency list \inlinecode{adj}, whose indices represent left-side nodes and whose entries contain right-side neighbors numbered $[0, {\inlinecodemath{n2}})$.
\end{compactitem}
