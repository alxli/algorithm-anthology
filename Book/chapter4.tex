...
It is a dynamic program over intermediate nodes: for each node \inlinecode{k} in turn, every pair \inlinecode{(i, j)} is relaxed by considering a path through \inlinecode{k}, so once all \inlinecode{k} have been processed the matrix holds the all-pairs shortest distances.
