...
\begin{compactitem}
  \item \inlinecode{RerootingTree(n)} constructs a tree with \inlinecode{n} nodes numbered from 0 to \inlinecode{n - 1}.
...
\begin{compactitem}
  \item \inlinecode{Data} is the return type of a normal fixed-root DFS call \inlinecode{dfs(u, p)}. It summarizes the component containing \inlinecode{u} after removing edge \inlinecode{u-p}, with all values measured from node \inlinecode{u}.
...
\begin{compactitem}
  \item \inlinecode{CentroidDecomposition(adj)} builds the centroid decomposition of the tree given by a bidirectional adjacency list \inlinecode{adj}, which must form a valid tree with nodes numbered from 0 to \inlinecode{adj.size() - 1}.
...
\begin{compactitem}
  \item \inlinecode{CycleFinder(n, directed)} constructs a graph on \inlinecode{n} nodes, directed if \inlinecode{directed} is true and undirected otherwise.
...
\begin{compactitem}
  \item \inlinecode{EulerianGraph(n, directed)} constructs a graph on \inlinecode{n} nodes, directed if \inlinecode{directed} is true and undirected otherwise.
  \item \inlinecode{add\_edge(u, v)} adds an edge and returns its edge ID.
...
\begin{compactitem}
  ...
  \item \inlinecode{build\_scc()} populates \inlinecode{scc} with the strongly connected components and \inlinecode{component[v]} with the component ID containing vertex \inlinecode{v}. Component IDs are in topological order: for every edge from component \inlinecode{a} to a different component \inlinecode{b}, \inlinecode{a \textless{} b}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{add\_edge(u, v)} adds the directed edge from \inlinecode{u} to \inlinecode{v}.
  \item \inlinecode{build\_scc()} populates \inlinecode{scc} with the strongly connected components and \inlinecode{component[v]} with the component ID containing vertex \inlinecode{v}. Component IDs are in reverse topological order: for every edge from component \inlinecode{a} to a different component \inlinecode{b}, \inlinecode{a \textgreater{} b}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{Dominators(n)} constructs a directed graph on nodes numbered from 0 to \inlinecode{n - 1}.
...
\begin{compactitem}
  ...
  \item For a non-articulation point \inlinecode{v}, \inlinecode{block\_cut\_id[v]} is $-1$.
\end{compactitem}
...
\section{\texorpdfstring{BFS and Shortest Paths}{4.3 BFS and Shortest Paths}}
...
Given a starting node in a weighted graph whose edge weights are only 0 or 1, compute the shortest distance to every reachable node. 0-1 BFS is a specialized version of Dijkstra's algorithm. Because every relaxation changes the distance by either $0$ or $1$, a deque maintains nodes in nondecreasing distance order: push weight-0 relaxations to the front and weight-1 relaxations to the back.
...
\begin{compactitem}
  \item \inlinecode{dijkstra(start, adj, dist, pred)} returns a pair of vectors \inlinecode{dist} and \inlinecode{pred} for an adjacency list \inlinecode{adj} which must consist of nodes numbered from 0 to \inlinecode{adj.size() - 1}. Each edge is stored as \inlinecode{(neighbor, weight)}, where \inlinecode{weight} is nonnegative.
\end{compactitem}
...
Given a starting node in a weighted directed graph, compute shortest paths even when some edge weights are negative. The Shortest Path Faster Algorithm is a queue-based optimization of Bellman-Ford: instead of relaxing every edge in every round, it keeps a queue of nodes whose distances have improved and relaxes only their outgoing edges. It is often fast on benign inputs, but it still has Bellman-Ford's worst-case behavior and can be forced to run in $O(n \cdot m)$. Prefer Dijkstra for nonnegative weights, and use SPFA mainly when negative edges are present and the input is not adversarial.
...
Floyd-Warshall's algorithm is a dynamic program over intermediate nodes: for each node \inlinecode{k} in turn, every pair \inlinecode{(i, j)} is relaxed by considering a path through \inlinecode{k}, so once all \inlinecode{k} have been processed the matrix holds the all-pairs shortest distances.

\begin{compactitem}
  \item \inlinecode{init\_floyd(nodes)} initializes \inlinecode{dist} and \inlinecode{next\_node} for nodes numbered from 0 to \inlinecode{nodes - 1}.
...
\begin{compactitem}
  \item \inlinecode{prim\_mst()} populates \inlinecode{mst} with the edge IDs in the minimum spanning tree (returning the total MST weight) for a global, bidirectionally pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered from 0 to \inlinecode{adj.size() - 1}. Each edge is stored as \inlinecode{(neighbor, weight, edge\_id)}.
\end{compactitem}

The priority queue stores candidate edges as \inlinecode{(weight, from, to, edge\_id)} and uses \inlinecode{std::greater} to make it a min-heap. To find a maximum spanning tree instead, use the default max-heap ordering.
...
\begin{compactitem}
  \item \inlinecode{kruskal\_mst(nodes)} populates \inlinecode{mst} with the edge IDs in the minimum spanning tree (returning the total MST weight) for a global, pre-populated edge list \inlinecode{edges} whose endpoints must be numbered from 0 to \inlinecode{nodes - 1}. Each edge is stored as \inlinecode{(weight, u, v)}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{directed\_mst(n, root, edges)} returns the total weight of the minimum spanning arborescence rooted at \inlinecode{root} over \inlinecode{n} nodes numbered from 0 to \inlinecode{n - 1}, or $-1$ if some node is unreachable from the root (no arborescence exists). Edges are given as \inlinecode{(from, to, weight)} triples; parallel edges and self-loops are allowed, with self-loops simply ignored.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{EdmondsKarp(n)} constructs an empty flow network with nodes numbered from 0 to \inlinecode{n - 1}.
...
\begin{compactitem}
  \item \inlinecode{Dinic(n)} constructs an empty flow network with nodes numbered from 0 to \inlinecode{n - 1}.
  \item \inlinecode{add\_edge(u, v, cap)} adds a directed residual-network edge from \inlinecode{u} to \inlinecode{v}.
  \item \inlinecode{max\_flow(source, sink)} modifies the residual network and returns maximum flow.
  \item \inlinecode{clear\_flow()} resets all edge flows to zero, allowing a fresh recomputation on the same graph.
  \item \inlinecode{min\_cut(source)} returns the source side of a minimum cut after \inlinecode{max\_flow()} has been called.
...
\begin{compactitem}
  \item \inlinecode{MinCostMaxFlow(n)} constructs an empty residual network with nodes numbered from 0 to \inlinecode{n - 1}.
  ...
  \item \inlinecode{min\_cost\_flow(source, sink, target\_flow)} sends up to \inlinecode{target\_flow} additional units of flow and returns \inlinecode{(flow, cost)} for the flow sent by that call. If the returned flow is smaller than \inlinecode{target\_flow}, the residual network cannot carry the requested amount.
\end{compactitem}
...
Given an undirected graph with nonnegative edge weights, build a Gomory-Hu tree: a weighted tree on the same vertices that represents every pairwise minimum s-t cut value. For any two vertices \inlinecode{u} and \inlinecode{v}, the minimum cut value between them in the original graph is the minimum edge weight on the path from \inlinecode{u} to \inlinecode{v} in the Gomory-Hu tree.

The algorithm starts with every vertex attached to vertex 0, then performs \inlinecode{n - 1} max-flow/min-cut computations. After computing the minimum cut between vertex \inlinecode{s} and its current parent, vertices on the \inlinecode{s} side of that cut are reparented under \inlinecode{s}, gradually refining the cut-equivalent tree.

\begin{compactitem}
  \item \inlinecode{gomory\_hu(n, edges)} returns the \inlinecode{n - 1} edges of a Gomory-Hu tree as \inlinecode{(u, v, weight)} triples.
  ...
  \item The input \inlinecode{edges} are undirected weighted edges \inlinecode{(u, v, weight)}. Parallel edges are allowed.
\end{compactitem}
...
The graph is given as a symmetric adjacency matrix \inlinecode{adj}, where \inlinecode{adj[i][j]} is the weight of the edge between \inlinecode{i} and \inlinecode{j} (0 if absent), and the diagonal is 0. The graph must have at least two vertices.

\begin{compactitem}
  \item \inlinecode{stoer\_wagner(adj)} returns a pair \inlinecode{(weight, side)}, where \inlinecode{weight} is the total weight of the global minimum cut and \inlinecode{side} lists the vertices on one side of that cut.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{blossom()} returns a matching for a global, bidirectionally pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered from 0 to \inlinecode{adj.size() - 1}. The returned vector \inlinecode{match} has \inlinecode{match[u] = v} and \inlinecode{match[v] = u} when \inlinecode{u} and \inlinecode{v} are matched, or \inlinecode{match[u] = -1} when \inlinecode{u} is unmatched.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{WeightedGeneralMatching(n)} creates a graph on \inlinecode{n} nodes numbered 0 to \inlinecode{n - 1}.
  \item \inlinecode{add\_edge(u, v, w)} adds an undirected edge of weight \inlinecode{w \textgreater{} 0} between \inlinecode{u} and \inlinecode{v}. If several edges join the same pair, the last weight set is used.
...
Both preference lists are given as $n$ by $n$ matrices. \inlinecode{men\_pref[m]} lists the women in \inlinecode{m}'s order of preference, most preferred first, and \inlinecode{women\_pref[w]} lists the men in \inlinecode{w}'s order of preference.
...
\begin{compactitem}
  \item \inlinecode{edge\_coloring(n, edges)} returns a vector of colors, one per entry of \inlinecode{edges}, using colors in the range $[0, D]$ where $D$ is the maximum degree. Nodes are numbered from 0 to \inlinecode{n - 1}, and \inlinecode{edges[i]} is an undirected edge \inlinecode{u}-\inlinecode{v}.
\end{compactitem}
...
Given a complete, weighted, directed graph, find a minimum-length Hamiltonian path or cycle. A Hamiltonian path visits each node exactly once. A Hamiltonian cycle additionally returns to its starting node; this cycle version is the traveling salesman problem.
