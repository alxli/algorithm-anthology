...
The priority queue stores candidate edges as (\inlinecode{weight}, \inlinecode{from}, \inlinecode{to}, \inlinecode{edge\_id}) and uses \inlinecode{std::greater} to make it a min-heap. To find a maximum spanning tree instead, use the default max-heap ordering.
...
\begin{compactitem}
  \item \inlinecode{gomory\_hu(n, edges)} returns the \inlinecode{n - 1} edges of a Gomory-Hu tree as (\inlinecode{u}, \inlinecode{v}, \inlinecode{weight}) tripes for an undirected, weighted graph specificed by \inlinecode{edges} of the same shape.
