...
Given a directed acyclic graph, find one of possibly many orderings of the nodes such that for every edge from node $u$ to $v$, $u$ comes before $v$ in the ordering. Depth-first search is used to traverse all nodes in post-order. A node finishes only after every node it points to has finished, so the reverse of the finish order is a valid topological ordering.
...
A Eulerian trail is a path in a graph which contains every edge exactly once. An Eulerian cycle or circuit is an Eulerian trail which begins and ends on the same node. A directed graph has an Eulerian cycle if and only if every node has an in-degree equal to its out-degree, and all of its nodes with nonzero degree belong to a single strongly connected component. An undirected graph has an Eulerian cycle if and only if every node has even degree, and all of its nodes with nonzero degree belong to a single connected component. The cycles are found with Hierholzer's algorithm: walk unused edges until returning to a node with none left, emit that node, and back up, which splices every closed detour into one cycle output in reverse.
...
\begin{compactitem}
  ...
  \item \inlinecode{euler\_cycle\_undirected(adj, u)} returns a vector of all nodes in an undirected graph that are reachable from the starting node \inlinecode{u} in an order which forms an Eulerian cycle. The first node will be repeated as the last element of the vector. The undirected graph given by adjacency list \inlinecode{adj} must consist of bidirectional edges with nodes numbered from 0 to \inlinecode{adj.size() - 1}.
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{init(n)} clears the formula and resets it to \inlinecode{n} variables.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to the constructor or \inlinecode{init(n)}, where $n$ is the number of variables.
  \item $O(1)$ per call to \inlinecode{literal()}, \inlinecode{add\_implication()}, \inlinecode{add\_or()}, \inlinecode{add\_true()}, and \inlinecode{add\_false()}.
...
Given a weighted, directed graph with possibly negative weights, determine the minimum distance between all pairs of start and destination nodes in the graph. Optionally, output the shortest path between two nodes using the next-hop matrix precomputed into \inlinecode{next\_node}.
...
\begin{compactitem}
  \item \inlinecode{initialize(nodes)} initializes \inlinecode{dist} and \inlinecode{next\_node} for nodes numbered from 0 to \inlinecode{nodes - 1}.
...
\begin{compactitem}
  \item \inlinecode{prim\_mst()} populates \inlinecode{mst} with the minimum spanning tree edges (returning the total MST weight) for a global, bidirectionally pre-populated adjacency list \inlinecode{adj} which must consist of nodes numbered from 0 to \inlinecode{adj.size() - 1}. Each edge is stored as \inlinecode{(neighbor, weight)}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{kruskal\_mst(nodes)} populates \inlinecode{mst} with the minimum spanning tree edges (returning the total MST weight) for a global, pre-populated edge list \inlinecode{edges} whose endpoints must be numbered from 0 to \inlinecode{nodes - 1}. Each edge is stored as \inlinecode{(weight, u, v)} and will be sorted by weight after the call.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{directed\_mst(n, root, edges)} returns the total weight of the minimum spanning arborescence rooted at \inlinecode{root} over \inlinecode{n} nodes numbered from 0 to \inlinecode{n - 1}, or $-1$ if some node is unreachable from the root (no arborescence exists).
\end{compactitem}
...
\begin{compactitem}
  ...
  \item \inlinecode{init(nodes)} clears the residual network and resets it to contain \inlinecode{nodes} nodes.
...
\textbf{Time Complexity}:
\begin{compactitem}
  \item $O(n)$ per call to the constructor or \inlinecode{init(n)}, where $n$ is the number of nodes.
...
It is solved by the Held-Karp dynamic program over subsets: \inlinecode{dp[S][i]} is the length of the shortest path that starts at a fixed node, visits exactly the set \inlinecode{S} of nodes, and ends at node \inlinecode{i}, built up by iterating over the subsets in increasing order of their bitmask.
...
It uses the same Held-Karp subset dynamic program as the traveling salesman cycle, where \inlinecode{dp[S][i]} is the shortest path visiting exactly the nodes in \inlinecode{S} and ending at node \inlinecode{i}. The answer is the minimum over all end nodes for the full set, with no return edge added.
