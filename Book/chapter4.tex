...
\begin{compactitem}
  ...
  \item \inlinecode{eulerian\_path\_directed(adj, start)} returns a vertex trail using every directed edge in \inlinecode{adj} exactly once, or an empty vector if no such trail exists. If \inlinecode{start} $= -1$, a valid start is chosen automatically; otherwise the trail must begin at \inlinecode{start}.
\end{compactitem}
...
\begin{compactitem}
  \item \inlinecode{KosarajuSCC(n)} constructs a directed graph of \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$.
...
\begin{compactitem}
  \item \inlinecode{TarjanSCC(n)} constructs a directed graph of \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$.
  \item \inlinecode{add\_edge(u, v)} adds the directed edge from \inlinecode{u} to \inlinecode{v}.
...
\begin{compactitem}
  \item \inlinecode{BiconnectedComponents(n)} constructs an undirected graph of \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$.
...
\begin{compactitem}
  \item \inlinecode{BridgeDecomposition(n)} constructs an undirected graph of \inlinecode{n} nodes numbered $[0, {\inlinecodemath{n}})$.
  \item \inlinecode{add\_edge(u, v)} adds the undirected edge \inlinecode{u}-\inlinecode{v}. Parallel edges are supported.
...
\begin{compactitem}
  \item \inlinecode{OnlineBridges(n)} constructs a graph with \inlinecode{n} isolated nodes numbered $[0, {\inlinecodemath{n}})$.
...
\begin{compactitem}
  \item \inlinecode{BoundedFlow(n)} constructs a directed lower-bound flow network with nodes in $[0, n)$.
